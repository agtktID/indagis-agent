...
profile's INDAGIS_HOME and EXECUTE under it.
...
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active INDAGIS_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever INDAGIS_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    INDAGIS_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import hermes_constants

    monkeypatch.setattr(
        hermes_constants, "_get_platform_default_indagis_home", lambda: root
    )
    monkeypatch.setenv("INDAGIS_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile INDAGIS_HOME (<root>/profiles/<name>), the cron store
    ...
    assert hermes_constants.get_indagis_home().resolve() == profile_home.resolve()
    assert hermes_constants.get_default_indagis_root().resolve() == root.resolve()

    # cron/jobs.py computes HERMES_DIR from get_indagis_home() at import, so a
