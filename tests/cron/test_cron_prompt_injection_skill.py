...
def cron_env(tmp_path, monkeypatch):
    """Isolated INDAGIS_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `INDAGIS_HOME` alone doesn't reach it. We also patch the
    ...
    monkeypatch.setattr(_skills_tool, "INDAGIS_HOME", hermes_home)
