...
($INSTALL_DIR), not into the shared $INDAGIS_HOME.

Background (shared-$INDAGIS_HOME bug)
------------------------------------
$INDAGIS_HOME is a data directory users frequently bind-mount into a Docker
...
$INDAGIS_HOME the two installs would fight over one slot — and the container,
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$INDAGIS_HOME/\.install_method"', text), (
        "install.sh must not stamp $INDAGIS_HOME/.install_method — that data "
