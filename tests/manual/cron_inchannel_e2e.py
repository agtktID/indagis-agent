...
Uses a throwaway INDAGIS_HOME so it never touches ~/.hermes. No real names.
"""
...
def _fresh_home():
    """Point INDAGIS_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_indagis_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["INDAGIS_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER INDAGIS_HOME is set.
...
# a different get_indagis_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"INDAGIS_HOME (throwaway): {HOME}")
