"""ensure_indagis_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("INDAGIS_HOME", str(first))
    cfg.ensure_indagis_home()

    # Profile switch: INDAGIS_HOME moves → the new path is ensured too.
    monkeypatch.setenv("INDAGIS_HOME", str(second))
    cfg.ensure_indagis_home()
