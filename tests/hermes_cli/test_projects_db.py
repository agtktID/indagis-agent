...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Indagis Agent", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "indagis-agent"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "indagis-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' INDAGIS_HOME.
