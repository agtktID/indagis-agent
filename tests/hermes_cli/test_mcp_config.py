...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_cli.config.get_indagis_home", lambda: tmp_path
    )
...
class TestMcpRemove:
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_indagis_home in the mcp_config module namespace
        monkeypatch.setattr(
            "hermes_cli.mcp_config.get_indagis_home", lambda: tmp_path
        )
