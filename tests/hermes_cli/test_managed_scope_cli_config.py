...
def _load_cli_config(home):
    ...
    cli.py binds ``_hermes_home = get_indagis_home()`` at import time (module
    singleton), so monkeypatching INDAGIS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap INDAGIS_HOME.)
    """
    import cli
