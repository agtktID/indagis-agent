...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``INDAGIS_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` INDAGIS_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir INDAGIS_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="INDAGIS_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="INDAGIS_HOME=/tmp/hermes-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="INDAGIS_HOME={tmp_path}/hermes-home"\n'
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_indagis_home", lambda: tmp_path)
...
class TestSystemUnitHermesHome:
    """INDAGIS_HOME in system units must reference the target user, not root."""
    ...
    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's INDAGIS_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_indagis_home().resolve())
        assert f'INDAGIS_HOME={hermes_home}' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        # Correct installed unit (operator's INDAGIS_HOME + drain timeout).
        monkeypatch.setenv("INDAGIS_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited INDAGIS_HOME (falls back to root).
        monkeypatch.setenv("INDAGIS_HOME", str(root_hermes))
        ...
        assert os.environ["INDAGIS_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned INDAGIS_HOME *before* it reads/compares the unit.
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("INDAGIS_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_indagis_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_indagis_home", lambda: profile_dir)
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        # WorkingDirectory is anchored at the target user's INDAGIS_HOME (stable,
...
class TestLegacyHermesUnitDetection:
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "indagis gateway migrate-legacy" in out
...
class TestServiceWorkingDirIsStable:
    ...
    (INDAGIS_HOME), never the source checkout / worktree, so a relocated or
