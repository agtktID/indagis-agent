...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-INDAGIS_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root INDAGIS_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `indagis gateway run --replace` (no -p flag).
    assert "indagis gateway run --replace" in default_text
    assert "indagis -p default" not in default_text
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside INDAGIS_HOME the way
    ...
    env = os.environ.copy()
    env["INDAGIS_HOME"] = str(hermes_home)
