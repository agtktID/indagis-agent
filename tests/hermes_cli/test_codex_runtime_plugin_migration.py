...
# ---- Bug C: INDAGIS_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``INDAGIS_HOME``
    ...
    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate INDAGIS_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("INDAGIS_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("INDAGIS_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When INDAGIS_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever INDAGIS_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "INDAGIS_HOME" not in env, (
            f"INDAGIS_HOME should not be set when env var is unset, got: "
            f"{env.get('INDAGIS_HOME')!r}"
        )
