...
def isolated_home(tmp_path, monkeypatch):
    """Isolate INDAGIS_HOME + reset any module-level catalog cache per test."""
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        # ``INDAGIS_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` INDAGIS_HOME directly instead.
        import importlib
        from hermes_cli import model_catalog
        ...
        try:
            ...
            active_home = Path(os.environ["INDAGIS_HOME"])
