...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Hermes layout and point INDAGIS_HOME at
    ...
    monkeypatch.setenv("INDAGIS_HOME", str(sec_home))

    import hermes_constants
    monkeypatch.setattr(hermes_constants, "get_default_indagis_root", lambda: root)
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up INDAGIS_HOME (set in
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        # Don't set INDAGIS_HOME — falls back to default.
        import agent.file_safety as fs
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        ...
        assert "Active Indagis profile" in src
        assert "cross_profile=True" in src
        assert "~/.indagis/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Indagis profile: default" in src
        assert "Active Indagis profile: {active_profile}" in src
