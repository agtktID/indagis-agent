"""Contract tests for the Docker image's immutable /opt/indagis install tree."""
...
def test_dockerfile_makes_opt_hermes_readonly_for_hermes_user() -> None:
    ...
    assert "chown -R root:root /opt/indagis" not in text
    assert "chmod -R a+rX /opt/indagis" not in text
    assert "chmod -R a-w /opt/indagis" not in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hermes:hermes\s+/opt/indagis/\.venv",
        r"chown\s+-R\s+hermes:hermes\s+/opt/indagis/ui-tui",
        r"chown\s+-R\s+hermes:hermes\s+/opt/indagis/gateway",
        r"chown\s+-R\s+hermes:hermes\s+/opt/indagis/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/indagis must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/indagis/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $INDAGIS_HOME data volume (which a host install may also use).
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/indagis tree.
    ...
    assert "ENV HERMES_LAZY_INSTALL_TARGET=/opt/indagis" not in text
    ...
    assert '"$INDAGIS_HOME/lazy-packages"' in stage2, (
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+hermes:hermes\s+/opt/indagis/plugins", text
    )
