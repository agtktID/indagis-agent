...
Older images made /opt/indagis/ui-tui and /opt/indagis/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/indagis/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:hermes /opt/indagis/ui-tui" not in text
    assert "chown -R hermes:hermes /opt/indagis/node_modules" not in text
