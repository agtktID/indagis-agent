...
Build the real image and verify: a config.yaml present in $INDAGIS_HOME
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $INDAGIS_HOME must be migrated on boot by
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $INDAGIS_HOME: {r.stdout}"
    )
