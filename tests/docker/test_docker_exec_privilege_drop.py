...
3. Files written under $INDAGIS_HOME from a ``docker exec`` session land
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``$INDAGIS_HOME/logs/container-boot.log``: it is written by
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `hermes config set` which writes config.yaml under INDAGIS_HOME.
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    # provoke a write into INDAGIS_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # INDAGIS_HOME that docker exec just wrote.
    r = subprocess.run(
