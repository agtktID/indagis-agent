...
The fix is ``ENV HERMES_TUI_DIR=/opt/indagis/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    ...
    inner = (
        "source /opt/indagis/.venv/bin/activate && "
        "cd /opt/indagis && "
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/indagis/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/indagis/ui-tui)"
    )
