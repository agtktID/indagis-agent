...
the whole pytest session's logging to `<INDAGIS_HOME>/logs/agent.log`.

If INDAGIS_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets INDAGIS_HOME at module scope for that reason - this
...
class TestLogIsolation:
    def test_hermes_home_is_sandboxed_before_imports(self):
        ...
        # fixture has sandboxed INDAGIS_HOME, so reading it here would pass even
        ...
        assert home, "conftest must set INDAGIS_HOME before test modules import"
        assert Path(home).resolve() != _real_hermes_home().resolve(), (
            f"INDAGIS_HOME pointed at the operator's real home ({home}) when "
