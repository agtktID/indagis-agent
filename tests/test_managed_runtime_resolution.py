...
Hermes installs runtimes for itself — ``uv`` at ``$INDAGIS_HOME/bin/uv``, Node at
``$INDAGIS_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
...
# Runtimes Hermes provisions into INDAGIS_HOME and must therefore resolve
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        ...
        + "\n\n$INDAGIS_HOME/bin (uv) and $INDAGIS_HOME/node are not on an "
