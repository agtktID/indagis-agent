...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate INDAGIS_HOME under tmp_path."""
    monkeypatch.setenv("INDAGIS_HOME", str(tmp_path / ".hermes"))
    (tmp_path / ".hermes").mkdir()

    # Force the constants/image cache helpers to re-read INDAGIS_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under INDAGIS_HOME — gateway cleanup
