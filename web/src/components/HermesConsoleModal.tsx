...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("indagis> ");
  const inputPromptRef = useRef("indagis> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "indagis> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Indagis Console...\x1b[0m");
