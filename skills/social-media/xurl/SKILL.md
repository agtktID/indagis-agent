...
> **Docker HOME pitfall:** In the official Indagis Docker layout, `/opt/data` is `HERMES_HOME`, but Indagis tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Indagis-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
