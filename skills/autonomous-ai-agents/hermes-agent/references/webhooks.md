...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Indagis agent runs by POSTing events to a URL.
...
Add to `${INDAGIS_HOME:-~/.indagis}/.env`:
```bash
...
All management is via the `indagis webhook` CLI command:

### Create a subscription
```bash
indagis webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.indagis/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.indagis/scripts/` (path traversal is blocked).

```bash
indagis webhook subscribe todoist-hermes \
...
```bash
indagis webhook remove <name>
```

### Test a subscription
```bash
indagis webhook test <name>
indagis webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
indagis webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
indagis webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
indagis webhook subscribe alerts \
...
- Subscriptions persist to `~/.indagis/webhook_subscriptions.json`

## How It Works

1. `indagis webhook subscribe` writes to `~/.indagis/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.indagis/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `indagis webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `indagis webhook test <name>` to verify the route works.
