# Themes / Skins — Author an Indagis Color Theme

Author an Indagis **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.indagis/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Indagis itself.
...
- Write access to the Indagis home dir — `~/.indagis` by default, or `$INDAGIS_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `indagis config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   indagis config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `indagis config set display.skin default` (or they can `/skin default`).
...
```
indagis skin set <key> <hex>      # e.g. indagis skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `indagis skin set background "#08201f"` changes
only the background; `indagis skin use <name>` / `indagis skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.indagis`** when a profile is active — resolve the real home
  from `$INDAGIS_HOME` first, falling back to `~/.indagis`.
...
- **Never hand-edit `config.yaml` to activate.** Use `indagis config set
...
- **You apply it, not the user.** `indagis config set display.skin <name>` is
...
- Run `indagis config get display.skin` and confirm it reports `<name>`.
