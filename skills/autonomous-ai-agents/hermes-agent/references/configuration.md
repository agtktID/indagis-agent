...
Edit with `indagis config edit` or `indagis config set section.key value`.
...
`indagis config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `indagis tools` (interactive) or `indagis tools enable/disable NAME`.
