...
Author widget apps for the Indagis TUI (`indagis --tui`): glanceable ambient
...
- The TUI must be in use (`indagis --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.indagis/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.indagis/logs/tui_gateway_crash.log` if a widget never appears.
