...
## Layer 1 — Is this thing an Indagis plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Indagis process**. Its LLM calls go through Indagis's already-configured provider. | Already uses Portal if user's Indagis is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Indagis. That LLM call is already authenticated
via Indagis's provider config. The plugin's own server auth (e.g.
...
What Indagis Agent has that *feels* like OAuth — `indagis login --provider nous`
opening a browser, user signs in, token lands in `~/.indagis/auth.json` — is a
**Indagis-specific browser flow**. Under the hood it produces a credential
Indagis uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Indagis's existing Portal credential out of `~/.indagis/auth.json`.
...
rides on the credential Indagis already holds.

Where this could live in Indagis:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Indagis itself.
...
1. First decide: Indagis plugin (runs inside Indagis) or separate app? If plugin,
   it already uses Portal via Indagis's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Indagis agent itself") is fully supported. Indagis treats agent
...
pitch in the Tool Gateway announcement is specifically about Indagis Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Indagis is configured to use Portal as its provider. It is
