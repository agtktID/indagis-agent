...
Indagis runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`AF_INET` socket. Root cause is usually Indagis's env scrubber dropping
...
Indagis-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Indagis tool and most Windows APIs. Prefer forward slashes in code
