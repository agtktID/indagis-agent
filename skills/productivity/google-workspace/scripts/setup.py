#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Indagis Agent.
...
from _hermes_home import display_indagis_home, get_indagis_home

INDAGIS_HOME = get_indagis_home()
TOKEN_PATH = INDAGIS_HOME / "google_token.json"
CLIENT_SECRET_PATH = INDAGIS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = INDAGIS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Indagis profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Indagis Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Indagis Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Indagis home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_indagis_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Indagis")
