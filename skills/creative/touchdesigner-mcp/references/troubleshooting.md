...
## 5. Indagis Configuration

### Config location

`$INDAGIS_HOME/config.yaml` (defaults to `~/.indagis/config.yaml` when `INDAGIS_HOME` is unset)
...
Restart the Indagis session for changes to take effect. The MCP connection is
