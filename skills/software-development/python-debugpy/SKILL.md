...
- A long-running process (indagis gateway, tui_gateway) misbehaves and you can't restart it
...
The indagis test runner and pytest both support this:
...
For long-lived processes: Indagis gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Indagis you have two practical options:
...
## Debugging Indagis-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `indagis` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `indagis --tui`)
...
debugpy.wait_for_client()
```
Start `indagis --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Indagis subagents, debug one process at a time.
