...
/**
 * User widget apps — Indagis authors its own TUI widgets, mirroring the
 ...
 *  every change, so a widget Indagis writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Indagis) ever writes must hot-load too — a 10s poll here read
