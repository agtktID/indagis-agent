...
describe('createGatewayEventHandler', () => {
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mIndagis here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `indagis model` to select a provider, or run `indagis setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
