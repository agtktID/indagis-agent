...
/** Why a known Indagis command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Indagis profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Indagis Agent version', surface: exec() },
 ...
 * Indagis' built-in slash commands — i.e. skill commands (`/gif-search`,
 ...
 * activity — the ones that ship with Indagis and were never asked for. It is
