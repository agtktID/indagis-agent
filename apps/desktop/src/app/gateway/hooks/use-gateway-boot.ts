...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Indagis...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Indagis…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Indagis gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Indagis couldn't start"
