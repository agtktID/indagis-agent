...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Indagis Cloud: one portal login powers discovery + silent per-agent
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Indagis Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Indagis Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Indagis Cloud org (slug or id) to
...
// --- Indagis Cloud (cloud-auto-discovery Phase 3) ---
...
// A discovered Indagis Cloud agent — the trimmed DTO from NAS GET /api/agents.
