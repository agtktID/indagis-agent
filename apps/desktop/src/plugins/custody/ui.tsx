  ...
  return (
    <div className="border-b border-(--ui-stroke-secondary) py-2.5 last:border-b-0">
      ...
      <div
        ...
        title={entry.public_key ?? undefined}
      >
    ...
    <div className={cn('mx-auto flex max-w-2xl flex-col gap-4 p-6')}>
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Signing keys for evidence exports — names and public keys only. Generate one with{' '}
          <code>indagis custody keygen</code>.
        </p>
      </div>
      ...
      {error && (
        <ErrorState
          description={error instanceof Error ? error.message : 'Failed to load keys.'}
      ...
      {!isLoading && !error && data && data.keys.length === 0 && (
        <EmptyState
          description="Generate a signing key with indagis custody keygen to get started."
          title="No keys yet"
        />
      )}
