...
INDAGIS_HOME="${INDAGIS_HOME:-/opt/data}"
...
# $INDAGIS_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# --- Bootstrap INDAGIS_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hermes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom INDAGIS_HOME paths whose parents only
# root can create (e.g. `INDAGIS_HOME=/home/hermes/.hermes` in a Compose
...
# When HERMES_UID is remapped or the top-level $INDAGIS_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $INDAGIS_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$INDAGIS_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $INDAGIS_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $INDAGIS_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$INDAGIS_HOME"; then
        :
    else
        chown hermes:hermes "$INDAGIS_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $INDAGIS_HOME failed (rootless container?) — continuing"
    fi
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$INDAGIS_HOME/$sub" ] && tree_has_non_hermes_owner "$INDAGIS_HOME/$sub"; then
            chown_hermes_tree "$INDAGIS_HOME/$sub"
        fi
    done
fi
...
# $INDAGIS_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
...
# $INDAGIS_HOME/lazy-packages dir on the data volume (Dockerfile sets
...
# Always reset ownership of $INDAGIS_HOME/profiles to hermes on every
...
if [ -d "$INDAGIS_HOME/profiles" ] && tree_has_non_hermes_owner "$INDAGIS_HOME/profiles"; then
    chown_hermes_tree "$INDAGIS_HOME/profiles"
fi

# Always reset ownership of $INDAGIS_HOME/cron on every boot for the same
...
# gate as profiles/).
if [ -d "$INDAGIS_HOME/cron" ] && tree_has_non_hermes_owner "$INDAGIS_HOME/cron"; then
    chown_hermes_tree "$INDAGIS_HOME/cron"
fi
...
# top-level $INDAGIS_HOME is mis-owned, so a warm volume with hermes-owned
# INDAGIS_HOME but root-owned logs/gateways would otherwise leave
...
if [ -d "$INDAGIS_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$INDAGIS_HOME/logs/gateways"; then
        :
    else
        chown hermes:hermes "$INDAGIS_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# data-volume chown above only runs when the top-level $INDAGIS_HOME is
...
# and cron/.
if [ -d "$INDAGIS_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$INDAGIS_HOME/platforms/pairing"; then
    chown_hermes_tree "$INDAGIS_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$INDAGIS_HOME/pairing" ] && tree_has_non_hermes_owner "$INDAGIS_HOME/pairing"; then
    chown_hermes_tree "$INDAGIS_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $INDAGIS_HOME
...
# sweep so host-owned files in a bind-mounted $INDAGIS_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$INDAGIS_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$INDAGIS_HOME/$f"; then
            :
        else
            chown hermes:hermes "$INDAGIS_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$INDAGIS_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$INDAGIS_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$INDAGIS_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$INDAGIS_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $INDAGIS_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hermes mkdir -p \
    "$INDAGIS_HOME/backups" \
    "$INDAGIS_HOME/cron" \
    "$INDAGIS_HOME/sessions" \
    "$INDAGIS_HOME/logs" \
    "$INDAGIS_HOME/logs/gateways" \
    "$INDAGIS_HOME/hooks" \
    "$INDAGIS_HOME/memories" \
    "$INDAGIS_HOME/skills" \
    "$INDAGIS_HOME/skins" \
    "$INDAGIS_HOME/plans" \
    "$INDAGIS_HOME/workspace" \
    "$INDAGIS_HOME/home" \
    "$INDAGIS_HOME/pairing" \
    "$INDAGIS_HOME/platforms/pairing" \
    "$INDAGIS_HOME/lazy-packages"
...
# $INDAGIS_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $INDAGIS_HOME: it is a shared DATA volume, commonly
...
# stale 'docker' stamp from $INDAGIS_HOME if one is present (the host install's
...
if [ -f "$INDAGIS_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$INDAGIS_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$INDAGIS_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$INDAGIS_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$INDAGIS_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$INDAGIS_HOME/$dest"
        fi
    fi
}
...
if [ -f "$INDAGIS_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$INDAGIS_HOME/.env"; then
        :
    else
        chown hermes:hermes "$INDAGIS_HOME/.env" 2>/dev/null || true
        chmod 600 "$INDAGIS_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $INDAGIS_HOME on the mounted volume. Run the same safe, non-interactive
...
if [ ! -f "$INDAGIS_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$INDAGIS_HOME/auth.json"; then
        :
    else
        printf '%s' "$HERMES_AUTH_JSON_BOOTSTRAP" > "$INDAGIS_HOME/auth.json"
        chown hermes:hermes "$INDAGIS_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$INDAGIS_HOME/auth.json"
    fi
fi
...
if [ -f "$INDAGIS_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$INDAGIS_HOME/auth.json"; then
        :
    else
        s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$INDAGIS_HOME/auth.json" \
...
if [ ! -f "$INDAGIS_HOME/gateway_state.json" ] && \
        [ "${HERMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$INDAGIS_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$INDAGIS_HOME/gateway_state.json"
        chown hermes:hermes "$INDAGIS_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$INDAGIS_HOME/gateway_state.json"
    fi
fi
