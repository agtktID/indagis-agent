...
def _hermes_root(home: Path) -> Path:
    """Root ~/.indagis for both the default profile and named profiles."""
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    try:
        ...
        resolved = Path(str(home)) if home else get_indagis_home()
...
def _remote_paragraph(root: Path) -> str:
    ...
    connected to the user's Desktop (local, remote URL, SSH, Indagis Cloud,
...
def _build_section(home: Path) -> str:
    ...
    return (
        f"{_PROTOCOL_HEADING}\n"
        "This install runs Bot Mode: each Indagis profile is an agent teammate with "
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient INDAGIS_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    from hermes_constants import get_indagis_home

    resolved = str(home) if home else str(get_indagis_home())
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    try:
        ...
        token = set_indagis_home_override(str(resolved))
        try:
