...
def register_credential_file(
    ...
    *relative_path* is relative to ``INDAGIS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside INDAGIS_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because INDAGIS_HOME is exactly where
    ...
    # Reject absolute paths — they bypass the INDAGIS_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to INDAGIS_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape INDAGIS_HOME.
    ...
    # inside INDAGIS_HOME and therefore pass the containment check above.
...
# Each tuple is (new_subpath, old_name) matching hermes_constants.get_indagis_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_indagis_dir()`` for backward compatibility with old directory layouts.
    """
    from hermes_constants import get_indagis_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_indagis_dir(new_subpath, old_name)
        if host_dir.is_dir():
