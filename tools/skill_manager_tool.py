...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch INDAGIS_HOME and
    ...
    live profile-scoped INDAGIS_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`indagis curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`indagis curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `indagis curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_indagis_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `indagis curator unpin <name>`. "
