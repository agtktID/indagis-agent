...
This means ``indagis dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``indagis kanban
...
def list_diagnostics(
    ...
    AND for ``indagis kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``indagis kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``indagis kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``indagis kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.indagis/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Indagis profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.indagis/profiles/*`` on disk and every
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Indagis
...
def list_kanban_projects():
    """List first-class Indagis projects for board scoping.
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``indagis profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``indagis kanban decompose <task_id>``.
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.indagis/config.yaml.
