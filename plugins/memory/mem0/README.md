...
```bash
indagis memory setup    # select "mem0"
```
...
Behavioral settings live in `$HERMES_HOME/mem0.json` (set them via `indagis memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.indagis/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Indagis session and call `mem0_search` — it connects to your server.
...
```bash
indagis memory setup mem0 --mode oss \
...
```bash
indagis memory setup mem0 --mode oss --oss-llm-key sk-...
```
...
```json
{
  ...
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.indagis/mem0_qdrant"}}
  }
}
```
...
```bash
indagis memory setup mem0 --mode platform --api-key sk-...
```
