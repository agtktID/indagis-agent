...
Or via $INDAGIS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $INDAGIS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_indagis_home() / "hindsight" / "config.json"
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $INDAGIS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
