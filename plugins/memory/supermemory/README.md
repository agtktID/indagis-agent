...
- Hosted: API key from [app.supermemory.ai/integrations?connect=indagis](http://app.supermemory.ai/integrations?connect=indagis)
...
Before running `indagis memory setup`, add the local endpoint to
...
Then run `indagis memory setup` and enter the local server's API key. Configuring
...
| `container_tag` | `indagis` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hermes-{identity}` → `hermes-coder`). |
...
`https://api.supermemory.ai`. Indagis resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: indagis`, and document writes stamp
`metadata.sm_source: indagis`. This is a **functional routing key, not telemetry**:
it groups Hermes-written memories into a dedicated "Indagis" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Indagis profile:
