...
Config in $INDAGIS_HOME/config.yaml (profile-scoped):
  plugins:
    hermes-memory-store:
      db_path: $INDAGIS_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_indagis_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _hermes_home = str(get_indagis_home())
