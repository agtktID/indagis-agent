...
``indagis://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Indagis OAuth client. IP-literal so
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Indagis.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local INDAGIS_HOME override can't reach.
