...
Resolution order for config file:
  1. $INDAGIS_HOME/honcho.json  (instance-local, enables isolated Indagis instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for an Indagis profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Indagis profile.
    ...
    Resolution order:
      ...
      4. Fallback: ``"indagis"`` (default profile)
    """
    ...
    # but applying it before profile resolution makes every named Indagis
    # profile share that same host.  Keep named profiles isolated; only the
    # default Indagis profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $INDAGIS_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.indagis/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_indagis_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $INDAGIS_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Indagis profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Indagis session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Indagis session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'indagis honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `indagis honcho setup` to configure)."
            )
