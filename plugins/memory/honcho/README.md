...
```bash
indagis memory setup honcho   # configure Honcho directly (works on a fresh install)
indagis memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
indagis config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.indagis/.env
```

> `indagis honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `indagis memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `indagis model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Indagis instances) |
| 2 | `~/.indagis/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Indagis profile: `indagis` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `indagis honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
- **`per-directory`** — basename of `$PWD`. Opening indagis in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Indagis session ID (timestamp + hex). Every `indagis` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Indagis profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`indagis`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Indagis profile: `indagis` (default) or `hermes_<profile>` (e.g. `indagis -p coder` -> host key `hermes_coder`). Older `indagis.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `indagis memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `indagis honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `indagis memory setup`) |
| `indagis honcho status` | Show resolved config for active profile |
| `indagis honcho enable` / `disable` | Toggle Honcho for active profile |
| `indagis honcho mode <mode>` | Change recall or observation mode |
| `indagis honcho peer --user <name>` | Update user peer name |
| `indagis honcho peer --ai <name>` | Update AI peer name |
| `indagis honcho tokens --context <N>` | Set context token budget |
| `indagis honcho tokens --dialectic <N>` | Set dialectic max chars |
| `indagis honcho map <name>` | Map current directory to a session name |
| `indagis honcho sync` | Create host blocks for all Indagis profiles |
