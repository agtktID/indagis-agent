...
Handles: indagis honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "indagis" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Indagis profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'indagis honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'indagis honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `indagis update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Indagis profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Indagis persistent cross-session memory.")
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'indagis honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'indagis honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'indagis honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'indagis honcho setup' again once you have a key.\n")
                return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "indagis")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "indagis")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        hermes_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Indagis GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Indagis gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: indagis config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    indagis honcho status     -- show full config")
    print("    indagis honcho mode       -- change recall/observation mode")
    print("    indagis honcho tokens     -- tune context and dialectic budgets")
    print("    indagis honcho peer       -- update peer names")
    print("    indagis honcho map <name> -- map this directory to a session name\n")
...
def _active_profile_name() -> str:
    """Return the active Indagis profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: indagis honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'indagis honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'indagis honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: indagis honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        indagis = hosts.get(_host_key(), {})
        user = indagis.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = indagis.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = indagis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = indagis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Indagis' identity in Honcho. Seed with 'indagis honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: indagis honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: indagis honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = indagis.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = indagis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = indagis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Indagis asks Honcho's AI peer a question")
        ...
        print("\n  Set with: indagis honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'indagis honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'indagis honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    indagis honcho identity --show        — show both peer representations")
        print("    indagis honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Indagis + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Indagis\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Indagis persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  indagis honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'indagis honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'indagis honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  indagis honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'indagis'")
        print("  with Honcho configured and no prior session history.")
        print("  (Indagis calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    indagis honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'indagis honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Indagis, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'indagis honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    indagis honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  indagis honcho identity <path/to/SOUL.md>")
    ...
    print("    Indagis:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Indagis:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Indagis:   Honcho observes every message and updates representations")
    ...
    print("    Indagis:   per-session by default — each run gets its own session")
    print("              Map a custom name:  indagis honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. indagis honcho setup              — configure API key (required)")
        print("  2. indagis honcho migrate            — re-run this walkthrough")
    else:
        print("  1. indagis honcho status             — verify Honcho connection")
        print("  2. indagis                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. indagis honcho identity --show    — verify AI peer representation")
        print("  4. indagis honcho tokens             — tune context and dialectic budgets")
        print("  5. indagis honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'indagis memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``indagis honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``indagis honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to indagis memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Indagis Honcho",
    )
