...
``{title, url, description, position}`` rows every other Indagis web
provider produces.
...
prefers Hermes-managed xAI Grok OAuth (via ``indagis auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.indagis/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``indagis tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `indagis auth` to sign in with "
