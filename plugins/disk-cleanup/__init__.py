"""disk-cleanup plugin — auto-cleanup of ephemeral Indagis session files.
...
1. ``post_tool_call`` hook — inspects ``write_file`` and ``terminal``
   ...
   under ``INDAGIS_HOME`` and tracks them silently.  Zero agent
   compliance required.
...
2. ``on_session_end`` hook — when any test files were auto-tracked
   ...
   logs a single line to ``$INDAGIS_HOME/disk-cleanup/cleanup.log``.
...
All operations are scoped to INDAGIS_HOME and /tmp/hermes-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside INDAGIS_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Indagis session files.",
    )
