...
Optional Indagis observability plugin that configures exporters and maps
Hermes-specific observer hooks to NeMo Relay marks and ATIF state. Indagis core
...
not replace Indagis Agent's planner, tools, memory, model provider routing, or
CLI UX. Indagis core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Indagis Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Indagis core.
- Add Indagis session, turn, approval, and subagent mark events.
...
```bash
indagis plugins enable observability/nemo_relay
```
...
`HERMES_HOME` is the Indagis profile/config home used by both
`indagis plugins enable ...` and the later `indagis chat ...` run. If unset,
Indagis uses the user's default home, usually `~/.indagis`. For isolated smoke
...
indagis plugins enable observability/nemo_relay
indagis chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `indagis` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run indagis plugins enable observability/nemo_relay
uv run indagis chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Indagis installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Indagis shared client metrics.
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Indagis does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Indagis keeps the direct env-var fallbacks active for that run.

Indagis core routes provider and tool execution through NeMo Relay managed APIs
...
Indagis uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
...
the Python binding does not yet expose that resolver. Indagis rejects
...
Indagis activates these plugins before registering its managed LLM and tool
...
For the full generic Indagis middleware contract, see
...
with Indagis and a local Ollama model served through the OpenAI-compatible API.
...
This run starts a parent Indagis session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Indagis Agent E2E'
...
{"kind":"mark","name":"indagis.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Indagis Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Indagis dispatches the read-only tools as one batch, and NeMo Relay
...
- Indagis sessions map to `agent` scopes.
- Indagis core managed provider calls map to `llm` scope start/end events.
- Indagis core managed tool calls map to `tool` scope start/end events.
...
Indagis core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Hermes-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Indagis calls the
...
```text
Indagis provider call
  -> nemo_relay.llm.execute(...)
    -> Indagis provider adapter callback(...)

Indagis tool call
  -> nemo_relay.tools.execute(...)
    -> Indagis authorization and dispatch callback(...)
```
...
middleware for a local Indagis run. This path requires a NeMo Relay runtime that
...
agent_name = "Indagis Middleware E2E"
agent_version = "local"
