"""langfuse — Indagis plugin for Langfuse observability.

Traces Indagis conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Indagis plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``indagis plugins enable
observability/langfuse`` or ``indagis tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``indagis tools`` or ~/.indagis/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Indagis plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older Indagis paths only expose ``task_id``/``session_id``. Newer paths
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Indagis turn",
                tags=["indagis", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Indagis branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Indagis also has a turn-scoped
    ...
    # messages is a list only for legacy Indagis branches that fired
    # pre_llm_call with API messages directly. Current Indagis fires
...
def register(ctx) -> None:
    ...
    # Indagis versions.  pre_api_request / post_api_request fire per API
