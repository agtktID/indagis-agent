...
2. User-installed providers: ``$INDAGIS_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$INDAGIS_HOME/plugins/`` or None if unavailable."""
    try:
        from hermes_constants import get_indagis_home
        d = get_indagis_home() / "plugins"
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$INDAGIS_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
