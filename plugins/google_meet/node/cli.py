"""`indagis meet node ...` subcommand tree.

Wired into the existing ``indagis meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``indagis meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``indagis meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             indagis meet node approve <name> ws://<host>:{args.port} {token}")
        try:
