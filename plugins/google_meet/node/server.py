...
Launched by ``indagis meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$INDAGIS_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``indagis meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_indagis_home()) / "workspace" / "meetings" / "node_token.json"
