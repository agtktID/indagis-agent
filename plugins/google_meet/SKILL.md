...
indagis meet install                 # pip deps + Chromium (transcribe only)
indagis meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
indagis meet auth                    # optional; skips guest-lobby wait
indagis meet setup                   # preflight checks
```

`indagis meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.indagis/.env
```
...
pip install playwright websockets && python -m playwright install chromium
indagis plugins enable google_meet
indagis meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
indagis meet node approve my-mac ws://<mac-ip>:18789 <token>
indagis meet node ping my-mac                   # confirm reachable
```

Run `indagis meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "An Indagis agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `indagis meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `indagis meet node approve`.
