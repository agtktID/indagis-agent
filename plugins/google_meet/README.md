...
Let the indagis agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where indagis runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `indagis meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `indagis meet` CLI |
...
| `cli.py` | `indagis meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `indagis meet node {run,list,approve,remove,status,ping}` |
...
indagis meet install                                      # pip + Chromium
indagis meet setup                                        # preflight
indagis meet auth                                         # optional
indagis meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
indagis meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.indagis/.env
indagis meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
indagis meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
indagis meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.indagis/.env
indagis meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, indagis will **not** switch your system audio input automatically — the
...
indagis plugins enable google_meet
indagis meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
indagis meet node approve my-mac ws://<mac-ip>:18789 <token>
indagis meet node ping my-mac
