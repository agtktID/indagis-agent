...
# identify HermesAgent traffic — matching other Indagis outbound surfaces
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `indagis slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `indagis gateway setup` "
                "or in the active profile's ~/.indagis/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `indagis gateway setup` "
                "or add it to your active profile's ~/.indagis/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `indagis gateway setup` "
                "or in the active profile's ~/.indagis/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `indagis gateway setup` "
                "or add it to your active profile's ~/.indagis/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            # Indagis has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Indagis can "
                    "be removed from the Slack app manifest via "
                    "`indagis slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # manifest (see `indagis slack manifest`). In Socket Mode, Slack
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'indagis slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Indagis handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Indagis session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Indagis owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer Indagis agents in Socket Mode on some
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Indagis",
                bot_description="Your Indagis agent on Slack",
            )
            ...
            print_info(
                "   Re-run `indagis slack manifest --write` anytime to refresh after "
                "Indagis adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `indagis update`)",
                True,
            ):
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `indagis setup` to install Slack support.",
