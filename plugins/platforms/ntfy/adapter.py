"""ntfy platform adapter (Indagis plugin).
...
an Indagis dependency.

This adapter ships as an Indagis platform plugin under
``plugins/platforms/ntfy/``. The Indagis plugin loader scans the
...
async def _standalone_send(
    ...
    *,
    ...
    ``indagis cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already an Indagis dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `indagis gateway status` without
