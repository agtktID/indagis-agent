"""
LINE Messaging API platform adapter for Indagis Agent.
...
to Indagis Agent. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``indagis status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``indagis status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``indagis setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.indagis/.env`` via ``hermes_cli.config``.
    """
    print()
