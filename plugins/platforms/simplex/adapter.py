"""SimpleX Chat platform adapter (Indagis plugin).
...
This adapter ships as an Indagis platform plugin under
``plugins/platforms/simplex/``. The Indagis plugin loader scans the
...
discoverable and ``indagis setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Indagis — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``indagis setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.indagis/.env`` via
    ``hermes_cli.config``.
    """
    print()
