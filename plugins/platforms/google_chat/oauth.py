...
- Per-user tokens (keyed by sender email):
    ``${INDAGIS_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${INDAGIS_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${INDAGIS_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${INDAGIS_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${INDAGIS_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's INDAGIS_HOME helper so the token follows the user's
# profile (e.g. tests can override via INDAGIS_HOME=/tmp/...).
try:
    from hermes_constants import display_indagis_home, get_indagis_home
...
def _hermes_home() -> Path:
    """Resolve INDAGIS_HOME at call time (NOT module import).

    Tests and ``INDAGIS_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.indagis/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `indagis setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `indagis setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into INDAGIS_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_indagis_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_indagis_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Indagis (native attachment delivery)"
    )
