...
In single-bot mode (what Indagis currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_indagis_home() / "feishu_seen_message_ids.json"
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Indagis gateway is already using this Feishu app_id"
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_indagis_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Indagis does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Indagis webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Indagis' SessionSource fields.
...
# Called by `indagis gateway setup` via _setup_feishu() in hermes_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=indagis&tp=indagis"
    else:
        qr_url += "?from=indagis&tp=indagis"
    return {
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `indagis setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `indagis pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `indagis setup` to install Feishu support.",
