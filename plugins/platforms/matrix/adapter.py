...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Indagis command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Indagis accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Indagis ``/command`` text."""
...
# Uses get_indagis_home() so each profile gets its own Matrix store.
from hermes_constants import get_indagis_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Indagis
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "indagis"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Indagis Agent" become "Agent".
        """
        if not body:
            return ""
...
def interactive_setup() -> None:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Indagis delivers cron job results and notifications.")
