"""
IRC Platform Adapter for Indagis Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Indagis agent.  Zero external dependencies — uses
...
def check_requirements() -> bool:
    ...
    # indagis setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `indagis gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Indagis to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #indagis — comma-separate for multiple)",
    ...
    print_info("Restart the gateway for changes to take effect: indagis gateway restart")
