...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.indagis/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Indagis gateway
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Indagis response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Indagis work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Indagis/gateway outage notices."""
        text = (content or "").lower()
        subject = r"(?:indagis|the agent|agent|the gateway|gateway|bmo)"
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Indagis manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Indagis pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Indagis session")
        ...
        @tree.command(name="status", description="Show Indagis session status")
        ...
        @tree.command(name="stop", description="Stop the running Indagis agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.indagis/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Indagis Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Indagis gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start an Indagis session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Indagis in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run an Indagis skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Indagis session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Indagis: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Indagis has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Indagis"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Indagis: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Indagis semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Indagis session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Indagis handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Indagis to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Indagis needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Indagis needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``indagis update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Indagis could not create a Discord thread for "
...
def _component_check_auth(
    ...
    the pairing store (``indagis pairing approve``).
    ...
    # so users approved via ``indagis pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``indagis update`` prompts.
...
# is not in this process (e.g. ``indagis cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    print()
    print_info("📬 Home Channel: where Indagis delivers cron job results,")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `indagis setup` to install Discord support.",
