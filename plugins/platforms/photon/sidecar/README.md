...
Small Node helper that bridges Indagis Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Indagis is Python; Photon has no public HTTP
...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Indagis can receive inbound messages
...
The Indagis plugin's `indagis photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Indagis is Python, so replies go through this sidecar
