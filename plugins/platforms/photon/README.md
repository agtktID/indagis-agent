...
This plugin connects Indagis Agent to iMessage (and other Spectrum
...
TypeScript-only, Indagis runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Indagis channel,
...
Run `indagis photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.indagis/.env` (the same place every other
...
Management metadata lives in `~/.indagis/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `indagis photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Indagis
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Indagis coalesces
...
- **Native polls are supported.** Indagis posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `indagis send`) cannot spawn the sidecar;
...
6. Verify end-to-end: `indagis photon status`, a DM and a group roundtrip,
