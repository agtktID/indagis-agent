"""
``indagis photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Indagis
...
# Writable sidecar runtime dir (mirrors to INDAGIS_HOME on immutable
...
# wiring imports this module for `indagis --help`).
...
# when called later (gateway start, indagis status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `indagis photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Indagis Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``indagis photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Indagis Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.indagis/.env,
    ...
    print("  Start the gateway:  indagis gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `indagis photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`indagis photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.indagis/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `indagis photon telemetry on` / `indagis photon telemetry off`.")
        return 0
    try:
    ...
    print("  Restart the gateway for the sidecar to pick it up:  indagis gateway restart")
    return 0
...
#
# `indagis gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``indagis photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `indagis gateway setup` wizard."""
