...
Credential storage mirrors every other Indagis channel:

    * runtime SDK creds  -> ``~/.indagis/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.indagis/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is an indagis dependency
...
# registers Indagis as its own client_id.
...
# Default name of the project Indagis provisions for the operator.
DEFAULT_PROJECT_NAME = "Indagis Agent"
...
def _auth_json_path() -> Path:
    """Resolve ``~/.indagis/auth.json`` honouring the active Indagis profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.indagis/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.indagis/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.indagis/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `indagis photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `indagis photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `indagis photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `indagis photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `indagis photon setup`)"
