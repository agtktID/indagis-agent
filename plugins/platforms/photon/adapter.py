"""
Photon Spectrum (iMessage) platform adapter for Indagis Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already an Indagis dep
...
#
# Out-of-process senders (cron subprocesses, `indagis send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_indagis_home() honors profile overrides — never hardcode ~/.indagis.
    from hermes_constants import get_indagis_home

    return get_indagis_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (indagis status, test collection, plugin discovery).
...
# artifacts so Indagis sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `indagis photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `indagis photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `indagis setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, indagis status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: indagis photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: indagis photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `indagis update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``indagis photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Indagis does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Indagis defaults).
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Indagis checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `indagis photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `indagis photon setup`)"
                )
        # A `indagis update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `indagis send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`indagis send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Indagis gateway (which spawns the sidecar and "
...
def register(ctx) -> None:
    """Called by the Indagis plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `indagis photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: indagis photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `indagis gateway setup` alongside every other
    ...
    # Register CLI subcommands — `indagis photon ...`
