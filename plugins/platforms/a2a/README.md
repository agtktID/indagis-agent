# A2A — Agent-to-Agent protocol for Indagis
...
peer (another Indagis, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
indagis gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Indagis serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.indagis/a2a_audit.jsonl`.
- Conversations persist to `~/.indagis/a2a_conversations/` — they survive context
