...
``https://api.deepinfra.com/v1/openai/models?filter=true&sort_by=indagis``
...
models show up in ``indagis tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from indagis the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `indagis tools` → Image "
                    "Generation → DeepInfra to configure, or `indagis setup` "
                    "to add the key."
                ),
