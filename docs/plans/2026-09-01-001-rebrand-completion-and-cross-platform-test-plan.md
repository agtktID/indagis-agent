...
2. **`hermes_cli/banner.py`** — `_UPSTREAM_REPO_URL`, `_OFFICIAL_REPO_CANONICAL`, `_RELEASE_URL_BASE` still point at the old repo, feeding the "N commits behind" banner shown on every CLI startup with wrong data.
