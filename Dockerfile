...
# Do not write .pyc files at runtime: /opt/indagis is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/indagis/.playwright
...
# Non-root user for runtime; UID can be overridden via INDAGIS_UID at runtime
...
# for the same reason: it lazy-installs into /opt/indagis/.venv at first
...
# Wire the exec shim and install-method stamp.  Files under /opt/indagis are
...
# `s6-setuidgid hermes` in its run script. If INDAGIS_UID is unset, services
...
# /opt/indagis/.hermes_build_sha at build time, and have
...
RUN if [ -n "${HERMES_GIT_SHA}" ]; then \
        printf '%s\n' "${HERMES_GIT_SHA}" > /opt/indagis/.hermes_build_sha; \
    fi
...
# slots from $INDAGIS_HOME/profiles/<name>/ after a container restart
...
# ---------- Runtime ----------
ENV HERMES_WEB_DIST=/opt/indagis/hermes_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/indagis/ui-tui/dist/entry.js`) and skip the
...
ENV HERMES_TUI_DIR=/opt/indagis/ui-tui
ENV INDAGIS_HOME=/opt/data
ENV HERMES_WRITE_SAFE_ROOT=/opt/data
ENV HERMES_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/indagis (root-owned, read-only) so a runtime
...
#
# /opt/indagis/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/indagis/bin:/opt/indagis/.venv/bin:/opt/data/.local/bin:${PATH}"
...
ENTRYPOINT [ "/opt/indagis/docker/entrypoint-dispatch.sh" ]
CMD [ ]
