...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `indagis skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Indagis Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Indagis Agent ships with a large built-in skill library copied into `~/.indagis/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Indagis Agent also syncs bundled skills on `indagis update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.indagis/skills/` tree, it is still shipped with Indagis Agent; restore it with `indagis skills reset <name> --restore`.",
        "",
