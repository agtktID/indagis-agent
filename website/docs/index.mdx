...
description: "A self-improving AI agent for cybersecurity investigation. A built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
A self-improving AI agent for cybersecurity investigation — OSINT, threat intel, and DFIR. It has a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
To easily install the command-line and desktop applications, [download the Indagis Desktop installer](https://indagis-agent.example.com/) from our website and run it.
...
After installing, run `indagis setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Nous Portal](/integrations/nous-portal).
:::
...
| 🧭 **[Use MCP with Indagis](/guides/use-mcp-with-hermes)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Indagis](/guides/use-voice-mode-with-hermes)** | Hands-on setup and usage patterns for Indagis voice workflows          |
...
- **Model-agnostic** — Works with [Nous Portal](https://portal.nousresearch.com), [OpenRouter](https://openrouter.ai), OpenAI, or any endpoint you already use
