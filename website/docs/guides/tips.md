...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `hermes sessions list` and resume with `indagis -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.
