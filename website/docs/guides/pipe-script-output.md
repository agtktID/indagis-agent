...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `indagis send`."
---

# Pipe Script Output to Messaging Platforms

`indagis send` is a small, scriptable CLI that pushes a message to any
messaging platform Indagis is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | indagis send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `indagis
...
indagis send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
indagis send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
indagis send --list

# Filter by platform
indagis send --list telegram
```
...
Any platform Indagis ships adapters for works as a target:
...
`indagis send` resolves the message body in this order:

1. **Positional argument** — `indagis send --to telegram "hi"`
2. **`--file PATH`** — `indagis send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | indagis send --to telegram`

When stdin is a TTY (no pipe), Indagis does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  indagis send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `indagis send` reuses your Indagis config, the same script works on
any host where Indagis is installed — no need to export bot tokens into
...
`indagis send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  indagis send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | indagis send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.hermes/bin/indagis send \
...
```bash
./train.py --epochs 200 && \
  indagis send --to telegram "training done" || \
  indagis send --to telegram "training failed (exit $?)"
```
...
indagis send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(indagis send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `indagis send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `indagis send` calls
the platform's REST endpoint directly using credentials from
`~/.indagis/.env` and `~/.indagis/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `indagis gateway start` and retry.
...
# Machine-readable
indagis send --list --json
```

The listing is built from `~/.indagis/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`indagis gateway
...
| Approach | Multi-platform | Reuses Indagis creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `indagis send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`indagis send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `indagis send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `indagis send` shares with cron delivery.
