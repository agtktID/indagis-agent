---
id: desktop-native-signin
title: "Desktop Native Sign-In (RFC 8252)"
sidebar_position: 1
description: "When the Indagis Desktop app connects to a **gated gateway** (a hosted or"
---
# Desktop Native Sign-In (RFC 8252)

When the Indagis Desktop app connects to a **gated gateway** (a hosted or
...
For Indagis specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          Indagis Cloud (IDP)
...
Indagis Cloud). This is required because the upstream `client_id` and permitted
redirect URIs are bound to the gateway's own origin — a desktop app can't be a
direct client of the Indagis Cloud. The desktop still gets the full RFC 8252
...
- [Run Indagis with Indagis Cloud](./run-indagis-with-indagis-cloud.md)
