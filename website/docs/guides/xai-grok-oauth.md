...
3. Hermes polls xAI until approval, then saves tokens to `~/.indagis/auth.json`.
4. From then on, Hermes refreshes the access token in the background — you stay signed in until you `indagis auth logout xai-oauth` or revoke access from your xAI account settings.
