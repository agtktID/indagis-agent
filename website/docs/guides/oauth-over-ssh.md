...
description: "How to complete browser-based OAuth (Spotify, MCP servers) when Indagis runs on a remote machine, container, or behind a jump box"
---
...
### Tokens land in the wrong `~/.indagis`

The tokens are written under the Linux user that ran `indagis auth add ...`. If your gateway / systemd service runs as a different user (e.g. `root` or a dedicated `indagis` user), authenticate as **that** user so the tokens land in their `~/.indagis/auth.json`. `sudo -u indagis -i` or equivalent.
