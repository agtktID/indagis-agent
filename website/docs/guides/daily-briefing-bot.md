---
id: daily-briefing-bot
title: "Tutorial: Daily Briefing Bot"
sidebar_position: 1
description: "In this tutorial, you'll build a personal briefing bot that wakes up every morning, researches topics you care about, summarizes the findings, and..."
---
...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Indagis Cloud subscription. The fastest setup is `indagis setup --cloud`. See [Indagis Cloud](/integrations/indagis-cloud).
:::
...
2. **Indagis spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.indagis/cron/output/` and you can read them anytime.
:::
...
Indagis will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Indagis Agent
```
...
Before creating cron jobs, ensure Indagis has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Indagis what you want:
...
Indagis will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Indagis to delegate each topic to a sub-agent:
...
Indagis will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
