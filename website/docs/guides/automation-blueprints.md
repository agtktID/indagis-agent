...
sidebar_position: 1
description: "Copy-paste blueprints for common automation patterns. Each blueprint uses Indagis's built-in cron scheduler for time-based triggers and webhook plat..."
---
# Automation Blueprints

Copy-paste blueprints for common automation patterns. Each blueprint uses Indagis's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`indagis webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `indagis webhook subscribe`) |
...
indagis cron create "0 2 * * *" \
  "You are a project manager triaging the Indagis Labs/indagis-agent GitHub repo.

1. Run: gh issue list --repo Indagis Labs/indagis-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
indagis webhook subscribe github-pr-review \
  --events "pull_request" \
...
indagis cron create "0 9 * * 1" \
  "Scan the Indagis Labs/indagis-agent repo for documentation drift.

1. Run: gh pr list --repo Indagis Labs/indagis-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
indagis cron create "0 6 * * *" \
  "Run a dependency security audit on the indagis-agent project.

1. cd ~/.indagis/indagis-agent && source .venv/bin/activate
...
```bash
indagis webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
indagis webhook subscribe alert-triage \
...
```python title="~/.indagis/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Indagis-Monitor/1.0"})
...
indagis cron create "every 30m" \
  ...
  --script ~/.indagis/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
indagis cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Indagis Agent development." \
...
```bash
indagis webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
indagis webhook subscribe stripe-payments \
...
```bash
indagis cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the indagis-agent codebase.
