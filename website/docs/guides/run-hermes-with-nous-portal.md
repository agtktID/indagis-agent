...
3. Sets `model.provider: nous` in `~/.indagis/config.yaml`
...
indagis setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
indagis auth add nous --type oauth
# Then re-run `indagis setup --portal` to wire the provider + gateway
```
...
You should see Indagis call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`indagis setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `indagis tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Indagis runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Indagis will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Indagis profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.indagis/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `indagis portal info` shows "not logged in" after `indagis setup --portal`
...
```bash
indagis config set model.provider nous
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Indagis doesn't replay it endlessly. Just log in again:

```bash
indagis auth add nous
```
...
`indagis portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Indagis configurations
