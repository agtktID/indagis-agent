---
id: terminal-environment-plugin
title: "Terminal Environment Provider Plugins"
sidebar_position: 1
description: "Indagis runs shell commands through a pluggable set of **terminal backends**."
---
# Terminal Environment Provider Plugins

Indagis runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.indagis/plugins/`, registering a backend the user selects
...
| `indagis setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `indagis status` / `indagis doctor` | `doctor_checks()` |
...
```python title="~/.indagis/plugins/acmebox/__init__.py"
...
```yaml title="~/.indagis/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
indagis plugins enable acmebox
indagis config set terminal.backend acmebox
```
