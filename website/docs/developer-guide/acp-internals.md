...
title: "ACP Internals"
sidebar_position: 1
description: "The ACP adapter wraps Indagis' synchronous AIAgent in an async JSON-RPC stdio server."
---
# ACP Internals

The ACP adapter wraps Indagis' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
indagis acp / hermes-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.indagis/.env
  -> configure stderr logging
  -> construct IndagisACPAgent
...
- `allow_once` -> Indagis `once`
- `allow_always` -> Indagis `always`
- reject options -> Indagis `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Indagis tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Indagis' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Indagis provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run ACP clients can open Indagis' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.indagis/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `hermes_cli/main.py` — `indagis acp` CLI subcommand
