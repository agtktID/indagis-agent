...
sidebar_position: 1
title: "Architecture"
description: "Indagis Agent internals — major subsystems, execution paths, data flow, and where to read next"
---
...
├── hermes_constants.py       # INDAGIS_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `indagis` subcommands (large file)
...
| **Profile isolation** | Each profile (`indagis -p <name>`) gets its own INDAGIS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
