...
compression:
  ...
  codex_app_server_auto: native  # native|indagis|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `indagis`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
notice is shown once per profile — a marker under `$INDAGIS_HOME`
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  ...
  events). Set `indagis` to let Indagis' compression threshold initiate
