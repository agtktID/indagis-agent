---
id: context-compression-and-caching
title: "Context Compression and Caching"
sidebar_position: 1
description: "Indagis Agent uses a dual compression system and Anthropic prompt caching to"
---
# Context Compression and Caching

Indagis Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `indagis plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Indagis has two separate compression layers that operate independently:
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Indagis raises the
...
```bash
indagis config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
indagis config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Indagis keeps the
...
`gpt-5.4-900k`). These are Indagis-side aliases: the suffix is stripped before
...
thread context, so Indagis' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Indagis
  records the resulting compaction events (compression counters, session
  events). Set `hermes` to let Indagis' compression threshold initiate
  app-server compaction, or `off` to disable Indagis-initiated automatic
  compaction entirely (codex may still compact natively).

Indagis' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Indagis' summary compressor.
...
encrypted `compaction` output item. Indagis captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Indagis
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Indagis can avoid; user-facing docs for `/model`, fallback providers, and
