...
When neither key is set, Hermes auto-detects the backend from whichever API key/URL is present in the environment. `indagis tools` walks users through selection.
...
| `requires_env` | Interactive credential prompt during `indagis plugins install` (see [Build a Hermes Plugin](/developer-guide/plugins#gate-on-environment-variables) for the rich format) |
