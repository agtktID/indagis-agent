...
sidebar_position: 1
description: "This guide walks through building a complete Indagis plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hoo..."
---
...
Indagis has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.indagis/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `indagis skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Indagis Labs/indagis-agent`. Users install them into `~/.indagis/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Indagis Labs Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/Indagis Labs/indagis-agent/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Indagis can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Indagis already owns. It does not replace native `plugin.yaml` plus
...
Indagis validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Indagis.
...
Portable `streamable-http` entries are routed through Indagis' existing native
...
same full-trust posture as other installed Indagis plugins.
...
records it as Published. Indagis keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Indagis does not expose a
...
continue to work after a normal Indagis upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Indagis inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Indagis releases can therefore load a plugin whose manifest contains
...
Indagis enforces this contract with frozen external-plugin fixtures discovered
...
```bash
mkdir -p ~/.indagis/plugins/calculator
cd ~/.indagis/plugins/calculator
```

### Validate with Plugin Doctor

`indagis plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Indagis itself. It reports invalid hook names, callbacks that do
...
This tells Indagis: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
Indagis reads these from installed metadata without importing your code, so
`indagis plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Indagis understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Indagis validates them, and `indagis plugins install` / `indagis plugins doctor` surface missing ones with a `pip install` hint, but Indagis **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Indagis' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/Indagis Labs/indagis-agent/issues/64165) and
[#15220](https://github.com/Indagis Labs/indagis-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Indagis may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `indagis my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Indagis continues fine
...
Use plugin-relative config keys for user-visible behavior. Indagis resolves them
...
```bash
HERMES_PLUGINS_DEBUG=1 indagis plugins list
```
...
The same logs are always written to `~/.indagis/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
indagis logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `indagis plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.indagis/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `indagis plugins update` / `remove` git-pull or delete it — your
...
# <indagis home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.indagis/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.indagis/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `indagis plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Indagis into installing arbitrary packages — only specs Indagis itself ships are eligible |
...
auditable in `~/.indagis/logs/agent.log`. Plugins load after built-in
...
does not ship with Indagis core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `indagis plugins list`
...
loader, the plugin is disabled and Indagis continues. The gate exists
...
You normally never edit this key by hand. `indagis plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Indagis injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: indagis-agent"}
...
- **The system prompt is Indagis's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Indagis sends them. |
...
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Indagis still loads on an older one.

### Register CLI commands

Plugins can add their own `indagis <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for indagis my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: indagis my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for indagis my-plugin."""
...
After registration, users can run `indagis my-plugin status`, `indagis my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `indagis name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `indagis chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:
...
For running a full `indagis <subcommand>` (e.g. `indagis kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "indagis kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Indagis from a hook.
...
Indagis has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.indagis/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `indagis gateway status` without SDK instantiation.
...
Indagis also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Indagis without any Python plugin. Declare them in `~/.indagis/config.yaml`:
...
Indagis connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.indagis/hooks/<name>/`:

```yaml
# ~/.indagis/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.indagis/hooks/long-task-alert/handler.py
...
indagis skills search my-workflow --source myorg/skills-repo
indagis skills install myorg/skills-repo/my-workflow
```
...
# Plugin auto-discovered on next indagis startup
```
...
```python
# Wrong — will break if Indagis passes extra context
def handler(args):
    ...
