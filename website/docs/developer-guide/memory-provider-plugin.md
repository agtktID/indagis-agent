---
id: memory-provider-plugin
title: "Memory Provider Plugins"
sidebar_position: 1
description: "Memory provider plugins give Indagis Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to b..."
---
# Memory Provider Plugins
# Building a Memory Provider Plugin

Memory provider plugins give Indagis Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `indagis plugins`.
:::

## Installation Layouts

Indagis discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Indagis. Closed to new providers — see [CONTRIBUTING](https://github.com/Indagis Labs/indagis-agent/blob/main/CONTRIBUTING.md). |
...
Indagis, in `$HERMES_HOME/plugins/<name>/` when installed by a user, or in
...
two files Indagis reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `indagis <provider>`
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `indagis memory setup` | **Yes** |
...
The gate binds to every compaction authority, not just the Indagis
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Indagis
...
`get_config_schema()` returns a list of field descriptors used by `indagis memory setup`:
...
the `messages` parameter; Indagis will continue calling them with the legacy
signature.
...
data_dir = Path("~/.indagis/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `indagis my-provider status`, `indagis my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `indagis <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `indagis --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: indagis my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the indagis my-provider argparse tree.
