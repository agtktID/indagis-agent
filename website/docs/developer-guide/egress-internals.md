...
sidebar_position: 1
description: "This page covers the architecture of the egress credential-injection firewall (indagis egress / iron-proxy) from a contributor / plugin author's per..."
---
# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`indagis egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
hermes_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `indagis egress {install,setup,start,stop,
...
hermes_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       ...
                                       `indagis proxy` subparser, which uses
                                       dest='proxy_command').
...
indagis egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.indagis/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.indagis/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

indagis egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.indagis/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `indagis egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `indagis egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `indagis egress status` warn about it:
...
# Live PTY smoke against `indagis egress`
...
- User-facing setup + troubleshooting: [Egress proxy](https://indagis-agent.indagis-labs.fr/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://indagis-agent.indagis-labs.fr/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`indagis secrets bitwarden`](https://indagis-agent.indagis-labs.fr/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`indagis egress`](https://indagis-agent.indagis-labs.fr/docs/reference/cli-commands#hermes-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://indagis-agent.indagis-labs.fr/docs/reference/environment-variables#egress-proxy-sandbox-injected)
