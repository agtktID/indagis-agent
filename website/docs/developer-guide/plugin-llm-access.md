---
id: plugin-llm-access
title: "Plugin LLM Access"
sidebar_position: 1
description: "ctx.llm is the supported way for a plugin to make an LLM call."
---
# Plugin LLM Access
...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Indagis already has applies. The plugin never sees a
...
[`hermes-example-plugins`](https://github.com/Indagis Labs/hermes-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
indagis-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/Indagis Labs/hermes-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Indagis provider id (e.g. "openrouter")
...
using Indagis built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Indagis provider
        # (must be one Indagis already knows about — same names as
        # `indagis model` and config.yaml model.provider).
        ...
        # plugin sends — Indagis does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.indagis/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Indagis' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Indagis subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Indagis Labs/indagis-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Indagis Labs/indagis-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/Indagis Labs/hermes-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/Indagis Labs/hermes-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
