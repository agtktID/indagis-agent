...
sidebar_position: 1
description: "The native Indagis Desktop app is contribution-driven: every"
---
...
A **desktop plugin** is a single ESM file that default-exports a `IndagisPlugin`.
...
desktop app** (`indagis desktop`) SDK — the `@hermes/plugin-sdk` module and
`$HERMES_HOME/desktop-plugins/`. The **web dashboard** (`indagis dashboard`) has
...
All three take the same `IndagisPlugin` contract, appear in **Settings → Plugins**,
...
[`hermes-example-plugins`](https://github.com/Indagis Labs/hermes-example-plugins)
repo.

## Quick start — your first plugin

Create `$HERMES_HOME/desktop-plugins/hello/plugin.js` (that's `~/.indagis/...`
...
A plugin default-exports a `IndagisPlugin`:

```ts
interface IndagisPlugin {
...
for requests; `targetProfile` is the backend Indagis profile served by that route.
...
ctx.os.notify({
  ...
  // Body click → focus Indagis + navigate. Same vocabulary as OS deep links:
...
```
~/.indagis/plugins/<id>/           # ONE installable folder
...
a package into `~/.indagis/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.indagis/plugins` is not reachable as a filesystem —
...
```html
<a href="hermes://plugin/install?repo=owner/repo&enable=1">Install in Indagis</a>
```
...
indagis-agent codebase directly (`hermes_state`, `hermes_cli.config`, …). See
...
export a `IndagisPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
| Plugin contract | `IndagisPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `IndagisOpenTarget`, `Contribution` |
