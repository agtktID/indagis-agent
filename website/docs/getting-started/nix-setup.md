...
description: "Install and deploy Indagis Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Indagis Agent ships a Nix flake & a NixOS module.
...
After `nix profile install`, `indagis`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `indagis setup` walks you through provider selection, `indagis gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.indagis/`.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o indagis /dev/stdin /var/lib/hermes/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `indagis` CLI on your system PATH **and** sets `INDAGIS_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `indagis` in your shell creates a separate `~/.indagis/` directory.
:::
...
Set `container.hostUsers` to create a `~/.indagis` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$INDAGIS_HOME/config.yaml` on each activation.
...
| Override the indagis package | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$INDAGIS_HOME/.env` at activation time (`nixos-rebuild switch`). Indagis reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Indagis looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Indagis loads its primary `SOUL.md` from `$INDAGIS_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$INDAGIS_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$INDAGIS_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u indagis` (native):
...
# Native mode
sudo -u indagis INDAGIS_HOME=/var/lib/hermes/.hermes \
  indagis mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
# Ensure: chown indagis:indagis, chmod 0600
```
...
2. **`.managed` marker file** in `INDAGIS_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent indagis config set ...` is also blocked)
...
~/.indagis -> /var/lib/hermes/.hermes       (symlink bridge, per hostUsers)
...
Plugins are symlinked into `$INDAGIS_HOME/plugins/` at activation time. Indagis discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
| Check | What it tests |
|---|---|
| `package-contents` | `indagis` and `hermes-agent` binaries exist and `indagis version` runs |
...
| `user` | `str` | `"indagis"` | System user |
| `group` | `str` | `"indagis"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`INDAGIS_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `indagis` CLI to system PATH and set `INDAGIS_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$INDAGIS_HOME/.env` at activation time |
...
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$INDAGIS_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.indagis` symlink to the service stateDir and are auto-added to the `indagis` group |
...
```
/var/lib/hermes/                     # stateDir (owned by indagis:indagis, 0750)
├── .hermes/                         # INDAGIS_HOME
...
```bash
# Native mode
sudo -u indagis cat /var/lib/hermes/.hermes/.env
...
| `indagis version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 indagis:indagis` | Use `docker exec` or `sudo -u indagis` |
...
| `unable to find user indagis` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
