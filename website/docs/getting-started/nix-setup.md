...
Indagis Agent ships a Nix flake, a NixOS module, and a Home Manager module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`indagis setup`, `indagis gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage indagis the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:agtktID/indagis-agent#desktop

# Or install persistently
nix profile install github:agtktID/indagis-agent#desktop

# run the tui
nix run github:agtktID/indagis-agent -- setup
nix run github:agtktID/indagis-agent -- --tui

# or install it in your profile
nix profile install github:agtktID/indagis-agent
indagis setup
indagis --tui
```

After `nix profile install`, `indagis`, `indagis-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `indagis setup` walks you through provider selection, `indagis gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.indagis/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries indagis-agent might need. if you want a smaller variant, check the other flake outputs. 
...
:::note
This module needs NixOS. Indagis is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    indagis-agent.url = "github:agtktID/indagis-agent";
  };

  outputs = { nixpkgs, indagis-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        indagis-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.indagis-agent = {
    enable = true;
    settings.model.default = "anthropic/claude-sonnet-4";
    environmentFiles = [ config.sops.secrets."indagis-env".path ];
    addToSystemPackages = true;
  };
}
```

That's it. `nixos-rebuild switch` creates the `indagis` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o indagis /dev/stdin /var/lib/indagis/env
```

```nix
services.indagis-agent.environmentFiles = [ "/var/lib/indagis/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `indagis` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `indagis` in your shell creates a separate `~/.indagis/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `indagis` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `indagis chat`, `indagis sessions list`, `indagis --version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the indagis codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `indagis` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo indagis chat` manually.
:::
...
systemctl status indagis-agent

# Watch logs (Ctrl+C to stop)
journalctl -u indagis-agent -f

# If addToSystemPackages is true, test the CLI
indagis --version
indagis config       # shows the generated config
```
...
```nix
# base.nix
services.indagis-agent.settings = {
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Indagis defaults to OpenRouter.
:::
...
```nix
services.indagis-agent.configFile = /etc/indagis/config.yaml;
```
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."indagis-env".path ]` |
...
| Override the indagis package | `package` | `inputs.indagis-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/indagis"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Indagis reads this file on every startup, so changes take effect with a `systemctl restart indagis-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/indagis.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."indagis-env" = { format = "yaml"; };
  };

  services.indagis-agent.environmentFiles = [
    config.sops.secrets."indagis-env".path
  ];
}
```
...
```yaml
# secrets/indagis.yaml (encrypted with sops)
indagis-env: |
...
```nix
{
  age.secrets.hermes-env.file = ./secrets/indagis-env.age;
...
{
  services.indagis-agent = {
    authFile = config.sops.secrets."indagis/auth.json".path;
...
Indagis reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
```nix
{
  services.indagis-agent = {
    ...
    workingDirectory = "/var/lib/indagis/workspace";
    documents = {
...
`hermesHomeFiles` installs into **`HERMES_HOME`**. Indagis reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. Indagis does not load that file as the identity:

```nix
{
  services.indagis-agent.hermesHomeFiles = {
...
```nix
{
  services.indagis-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.indagis-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. Indagis implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.indagis-agent.mcpServers.my-oauth-server = {
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Indagis prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u hermes` (native):

```bash
# Container mode
docker exec -it indagis-agent \
  indagis mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u indagis HERMES_HOME=/var/lib/indagis/.indagis \
  indagis mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
indagis mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.indagis/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/indagis/.hermes/mcp-tokens/
...
```nix
{
  services.indagis-agent.mcpServers.analysis = {
    command = "npx";
...
When indagis runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `indagis setup` | Config is declarative — edit `settings` in your Nix config |
| `indagis config edit` | Config is generated from `settings` |
| `indagis config set <key> <value>` | Config is generated from `settings` |
| `indagis gateway install` | The systemd service is managed by NixOS |
| `indagis gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **The `HERMES_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `HERMES_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it indagis-agent indagis config set ...`.
...
The flake also exports `homeManagerModules.default`. Indagis is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.indagis-agent`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:
...
| CLI on the PATH | `addToSystemPackages`, which exports `HERMES_HOME` for the full system | `programs.indagis-agent.enable`, which exports it for your session only |
| Desktop application | not supported, because a system service cannot own a user session | `programs.indagis-agent.desktop.enable` |
...
```nix
{
  imports = [ indagis-agent.homeManagerModules.default ];
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. Indagis Desktop and the web dashboard connect to a *different* process, which is `indagis serve` or `indagis dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that Indagis Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `HERMES_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status indagis-agent
journalctl --user -u indagis-agent -f

# macOS
launchctl list | grep indagis
tail -f ~/Library/Logs/indagis-agent.log

indagis --version
indagis config     # shows the configuration that Nix wrote
```
...
When container mode is enabled, indagis runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-indagis-agent-0.1.0  ──►  /nix/store/... (ro)
~/.indagis -> /var/lib/indagis/.indagis       (symlink bridge, per hostUsers)
/var/lib/indagis/                    ──►  /data/          (rw)
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the indagis package itself do **not** trigger recreation.
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current indagis package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `indagis plugins install` needed.
...
```nix
services.indagis-agent.extraPlugins = [
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Indagis discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
```nix
services.indagis-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the indagis wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in indagis-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.indagis-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.indagis-agent.url = "github:agtktID/indagis-agent";
  outputs = { indagis-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ indagis-agent.overlays.default ];
    # Then:
    #   pkgs.indagis-agent.override { extraPythonPackages = [...]; }
    #   pkgs.indagis-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.indagis-agent.settings.plugins.enabled = [
  "hermes-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core indagis dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `indagis` and `indagis-agent` binaries exist and `indagis --version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `indagis --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true indagis config set ...` prints the NixOS error |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the indagis-agent service |
| `package` | `package` | `indagis-agent` | The indagis-agent package to use |
...
| `addToSystemPackages` | `bool` | `false` | Add `indagis` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| `hermesHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `HERMES_HOME`. `SOUL.md` and `memories/` must be here, or Indagis does not load them. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `indagis gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the indagis user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
### Backend (`indagis serve` / `indagis dashboard`)

This option runs the process that Indagis Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
### `programs.indagis-agent` (Home Manager only)

Home Manager separates "install this application for me" from "run this
daemon". `services.indagis-agent` keeps the state, the configuration and the
daemons. `programs.indagis-agent` installs what you use, and reads
`hermesHome` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `indagis` CLI to `home.packages`, and export `HERMES_HOME` for your shells |
| `package` | `package` | `services.indagis-agent.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the Indagis Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.hermesDesktop` | The desktop package. The default follows `package`, so the application and the services run one Indagis runtime. |

```nix
programs.indagis-agent = {
  enable = true;
  desktop.enable = true;
};
...
profile, so the value that `programs.indagis-agent.enable` exports with
...
`services.indagis-agent.installPackage` was removed by this split. A
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.hermes` symlink to the service stateDir and are auto-added to the `indagis` group |
...
```
/var/lib/indagis/                     # stateDir (owned by hermes:hermes, 0750)
├── .hermes/                         # HERMES_HOME
...
```
~/.indagis/                           # hermesHome (HERMES_HOME), 0700
...
├── auth.json                        # OAuth credentials: seeded, then Indagis owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Indagis binary + all Nix deps |
...
cd /etc/nixos && nix flake update indagis-agent
...
```bash
systemctl status indagis-agent
docker ps -a --filter name=indagis-agent
docker inspect indagis-agent --format='{{.State.Status}}'
docker exec -it indagis-agent bash
docker exec indagis-agent readlink /data/current-package
docker exec indagis-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop indagis-agent
docker rm -f indagis-agent
sudo rm /var/lib/indagis/.container-identity
sudo systemctl start indagis-agent
```
...
```bash
# Native mode
sudo -u indagis cat /var/lib/indagis/.hermes/.env

# Container mode
docker exec indagis-agent cat /data/.hermes/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec indagis-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u indagis-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `indagis --version` shows old version | Container not restarted | `systemctl restart indagis-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 hermes:hermes` | Use `docker exec` or `sudo -u hermes` |
| `nix-collect-garbage` removed indagis | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "indagis-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
...
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart indagis-agent` |
