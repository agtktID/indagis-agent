...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `INDAGIS_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.indagis/` directory.
:::
...
- The routing is transparent: `indagis chat`, `hermes sessions list`, `indagis version`, etc. all exec into the container under the hood
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$INDAGIS_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$INDAGIS_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Plugins are symlinked into `$INDAGIS_HOME/plugins/` at activation time. Hermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `hermes-agent` binaries exist and `indagis version` runs |
...
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `INDAGIS_HOME` system-wide |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.indagis` symlink to the service stateDir and are auto-added to the `hermes` group |
