...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Indagis Agent — from install to chatting in under 5 minutes"
---
...
To easily install the command-line and desktop applications, [download the Indagis Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `indagis model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `indagis model` |
...
# ~/.indagis/hermes-agent on Linux/macOS or %LOCALAPPDATA%\indagis\hermes-agent on Windows):
cd ~/.indagis/hermes-agent
...
(If you installed without `[all]`, run `cd ~/.indagis/hermes-agent && uv pip install -e ".[acp]"` first.)
