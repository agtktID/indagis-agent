---
id: quickstart
title: "Indagis Agent Quickstart"
sidebar_position: 2
description: "Indagis Agent Quickstart"
---
# Indagis Agent Quickstart

# Indagis Agent Quickstart

This guide gets you from zero to a working Indagis setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Indagis Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Indagis Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Indagis for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Indagis working on my machine | `indagis setup` | Run a real chat and verify it responds |
| I already know my provider | `indagis model` | Save the config, then start chatting |
| I want a bot or always-on setup | `indagis gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `indagis model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `indagis model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Indagis cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Indagis Agent
### With the Indagis Desktop installer on macOS or Windows (recommended)
...
The single most important setup step. Use `indagis model` to walk through the choice interactively:

```bash
indagis model
```

:::tip Easiest path: Indagis Cloud
...
That logs you in, sets Indagis as your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `indagis setup` offers three modes:

- **Quick Setup (Indagis Cloud)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `indagis update`. Re-enable anything later with `indagis tools`, seed skills with `indagis skills opt-in --sync`, or tune settings with `indagis setup agent`.
:::
...
|----------|-----------|---------------|
| **Indagis Cloud** | Subscription-based, zero-config | OAuth login via `indagis model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `indagis model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `indagis model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `indagis model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `indagis model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `indagis model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `indagis model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `indagis model` (requires `copilot` CLI + `copilot login`) |
...
Indagis Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `indagis model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Indagis separates secrets from normal config:

- **Secrets and tokens** → `~/.indagis/.env`
- **Non-secret settings** → `~/.indagis/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
indagis config set model anthropic/claude-opus-4.6
indagis config set terminal.backend docker
indagis config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
indagis            # classic CLI
indagis --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Indagis ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `indagis` vs `indagis --tui`.
:::
...
```bash
indagis --continue    # Resume the most recent session
...
```bash
indagis gateway setup    # Interactive platform configuration
```
...
- `indagis tools` — tune tool access per platform
- `indagis skills` — browse and install reusable workflows
...
```bash
indagis config set terminal.backend docker    # Docker isolation
indagis config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `indagis egress setup && indagis egress start`; `indagis setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Indagis install directory (the curl installer placed it at
# ~/.indagis/indagis-agent on Linux/macOS or %LOCALAPPDATA%\hermes\indagis-agent on Windows):
...
Skills are on-demand instruction documents that teach Indagis how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Indagis ships with a catalog of bundled skills already installed in `~/.indagis/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
indagis skills browse                      # list everything available
indagis skills search kubernetes           # find skills by keyword
indagis skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `indagis skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Indagis ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.indagis/indagis-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Indagis opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `indagis model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `indagis gateway setup` and check `indagis gateway status` |
| `indagis --continue` can't find old session | Switched profiles or session never saved | Check `indagis sessions list` and confirm you're in the right profile |
...
| `indagis doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `indagis model`
3. `indagis setup`
4. `indagis sessions list`
5. `indagis --continue`
6. `indagis gateway status`
...
| `indagis tools` | Configure which tools are enabled per platform |
| `indagis setup` | Full setup wizard (configures everything at once) |
| `indagis doctor` | Diagnose issues |
| `indagis update` | Update to latest version |
| `indagis gateway` | Start the messaging gateway |
| `indagis --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `indagis backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
