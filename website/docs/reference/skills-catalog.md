...
Hermes ships with a large built-in skill library copied into `~/.indagis/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Hermes also syncs bundled skills on `indagis update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.indagis/skills/` tree, it is still shipped with Hermes; restore it with `indagis skills reset <name> --restore`.
