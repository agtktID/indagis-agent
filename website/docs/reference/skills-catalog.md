...
| [`hermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) | Use, configure, theme, extend, and orchestrate Indagis Agent. | `autonomous-ai-agents/hermes-agent` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`domain-intel`](/docs/user-guide/skills/bundled/security/security-domain-intel) | Passive recon of subdomains, SSL certs, WHOIS, and DNS. | `security/domain-intel` |
| [`osint-investigation`](/docs/user-guide/skills/bundled/security/security-osint-investigation) | Follow the money via public records and sanctions data. | `security/osint-investigation` |
| [`sherlock`](/docs/user-guide/skills/bundled/security/security-sherlock) | Find accounts for a username across 400+ platforms. | `security/sherlock` |
| [`shodan-search`](/docs/user-guide/skills/bundled/security/security-shodan-search) | Search Shodan for internet-connected devices, services, and exposure by host, IP, or query. | `security/shodan-search` |
| [`sigma-rule-search`](/docs/user-guide/skills/bundled/security/security-sigma-rule-search) | Search the SigmaHQ rule repository for detection rules by keyword, MITRE ATT&CK technique, or log source, and convert matches to a target SIEM query language. | `security/sigma-rule-search` |
| [`virustotal-lookup`](/docs/user-guide/skills/bundled/security/security-virustotal-lookup) | Look up file hashes, URLs, domains, and IPs on VirusTotal for AV-engine verdicts, reputation, and relationship data. | `security/virustotal-lookup` |
| [`yara-scan`](/docs/user-guide/skills/bundled/security/security-yara-scan) | Scan files, directories, or a running process against YARA rules to identify or classify malware. | `security/yara-scan` |
...
| [`inspecting-hermes-desktop-dom`](/docs/user-guide/skills/bundled/software-development/software-development-inspecting-hermes-desktop-dom) | Read the live Indagis desktop DOM/CSS over CDP. | `software-development/inspecting-hermes-desktop-dom` |
