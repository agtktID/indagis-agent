...
sidebar_position: 9
description: "Profile Commands Reference"
---
# Profile Commands Reference

# Profile Commands Reference

This page covers all commands related to [Indagis profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `indagis profile` without a subcommand shows help.
...
```bash
indagis profile use <name>
```

Sets `<name>` as the active profile. All subsequent `indagis` commands (without `-p`) will use this profile.
...
indagis profile use default
```

## `indagis profile create`

```bash
indagis profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `indagis profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `indagis update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.hermes`), use `indagis skills opt-out` / `indagis skills opt-in`. |
...
indagis profile create work2 --clone-from work

# Clone everything from a specific profile
indagis profile create work2-backup --clone-from work --clone-all
```

## `indagis profile describe`

```bash
indagis profile describe [<name>] [options]
```
...
indagis profile describe researcher

# Set it explicitly
indagis profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
indagis profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
indagis profile describe --all --auto
```

## `indagis profile delete`

```bash
indagis profile delete <name> [options]
```
...
```bash
indagis profile delete mybot
indagis profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.hermes`) cannot be deleted — use `indagis uninstall` to remove everything.
:::

## `indagis profile show`

```bash
indagis profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Indagis home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ indagis profile show work
Profile: work
Path:    ~/.indagis/profiles/work
...
## `indagis profile alias`

```bash
indagis profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Indagis installation.
...
```bash
indagis profile alias work
# Creates/updates ~/.local/bin/work

indagis profile alias work --name mywork
# Creates ~/.local/bin/mywork

indagis profile alias work --remove
# Removes the wrapper script
```

## `indagis profile rename`

```bash
indagis profile rename <old-name> <new-name>
```
...
```bash
indagis profile rename mybot assistant
# ~/.indagis/profiles/mybot → ~/.indagis/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `indagis profile export`

```bash
indagis profile export <name> [options]
```
...
```bash
indagis profile export work
# Creates work.tar.gz in the current directory

indagis profile export work -o ./work-2026-03-29.tar.gz
```
...
## `indagis profile import`

```bash
indagis profile import <archive> [options]
```
...
```bash
indagis profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

indagis profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `indagis -p telemetry`). |
...
indagis profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
indagis profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
indagis profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
indagis profile install ./telemetry/
```

### `indagis profile update`

```bash
indagis profile update <name> [--force-config] [--yes]
```
...
### `indagis profile info`

```bash
indagis profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Indagis version, author, env var requirements, the source URL/path, and
...
`indagis profile list` also shows the distribution name and version in a
`Distribution` column, and `indagis profile show <name>` / `delete <name>`
...
indagis profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
indagis profile install https://github.com/your-org/internal-assistant.git
```
...
Indagis version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Indagis can clone from.
3. Tell recipients to run `indagis profile install <your-repo-url>`.
...
## `indagis -p` / `indagis --profile`

```bash
indagis -p <name> <command> [options]
indagis --profile <name> <command> [options]
```

Global flag to run any Indagis command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
indagis -p work chat -q "Check the server status"
indagis --profile dev gateway start
indagis -p personal skills list
indagis -p work config edit
```

## `indagis completion`

```bash
indagis completion <shell>
```
...
- `indagis profile <TAB>` — subcommands (list, use, create, etc.)
- `indagis profile use <TAB>` — profile names
- `indagis -p <TAB>` — profile names
