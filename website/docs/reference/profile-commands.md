...
:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.indagis`) cannot be deleted — use `indagis uninstall` to remove everything.
:::
...
:::info
`indagis profile export` / `import` are still the right commands for
