...
Set your provider with `hermes model` or by editing `~/.indagis/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
nohup hermes gateway run > ~/.indagis/logs/gateway.log 2>&1 &
```
