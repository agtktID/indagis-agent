...
description: "Frequently asked questions and solutions to common issues with Indagis Agent"
---
...
- [Use MCP with Indagis](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
...
cd ~/.indagis/hermes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
- [Use MCP with Indagis](/guides/use-mcp-with-hermes)
...
| **Scope** | Global (entire `~/.indagis` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='hermes-agent' ~/.indagis/ newmachine:~/.indagis/
```
