...
| Provider | Setup |
|----------|-------|
| **Nous Portal** | `indagis model` (OAuth, subscription-based) |
| **OpenAI Codex** | `indagis model` (ChatGPT OAuth, uses Codex models) |
...
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.indagis/.env` (provider: `alibaba`) |
...
[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run Indagis Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) — billed against your Nous subscription instead of separate per-provider accounts.

```bash
indagis setup --portal     # fresh install — OAuth + provider + gateway in one command
indagis model              # existing install — pick "Nous Portal" from the list
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Indagis Agent with Nous Portal guide](/guides/run-hermes-with-nous-portal).

**Client identification.** Every Portal request from Indagis Agent carries a `client=hermes-client-v<version>` tag (e.g. `client=hermes-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Indagis traffic from other clients. No config required; the tag updates automatically when you `indagis update`.

**JWT auth (automatic).** Indagis prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.
...
If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Indagis marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `indagis auth add openai-codex` (or `indagis model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `indagis setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `indagis model` or per-tool from `indagis tools`. Inspect routing at any time with `indagis portal info`.
:::
...
| **OpenAI Codex — ChatGPT plan OAuth** | ✅ Yes — `indagis model` → OpenAI Codex (ChatGPT OAuth device-code login, uses Codex models) | *Not currently documented* | *Not currently documented* | Docs cover auth and token refresh only; plan-quota semantics are not yet documented |
...
**OpenAI Codex.** Indagis authenticates via ChatGPT device-code OAuth, stores credentials in `~/.indagis/auth.json`, and can import existing Codex CLI credentials from `~/.codex/auth.json`. Which ChatGPT plan tiers are eligible, and how Indagis usage counts against your plan's Codex limits, are **not currently documented** — the Codex note under [Nous Portal](#nous-portal) covers authentication and token-refresh behavior only.
...
indagis chat --provider tencent-tokenhub --model hy3-preview
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `indagis model`, sign in through the browser, and Indagis persists the refresh token.
...
Supported parsers: `indagis` (Qwen 2.5, Indagis 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser indagis` |
