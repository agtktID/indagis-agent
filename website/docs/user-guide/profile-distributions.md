...
- **You just want to back up a profile on your own machine.** Use [`indagis profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — that's what those are for.
...
:::caution
**Hermes does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `indagis profile install` or `indagis profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
- [`indagis profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — local backup / restore (not distribution)
