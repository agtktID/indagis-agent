...
sidebar_position: 7
title: "Docker"
description: "Running Indagis Agent in Docker and using Docker as a terminal backend"
---

# Indagis Agent — Docker
...
:::tip
Inside the container, run `indagis setup --portal` once — the refresh token persists in the mounted `~/.indagis` volume. See [Nous Portal](/integrations/nous-portal).
:::
...
All mutable Indagis state belongs under `/opt/data`: config, `.env`, profiles, skills, memories, sessions, logs, dashboard uploads, plugins, and other user-managed files. The image also disables runtime `.pyc` writes and Indagis lazy dependency installs into `/opt/hermes`; optional platform dependencies needed by the published image should be baked into the image or installed through a new image build.
...
- Per-profile rotated logs at `${INDAGIS_HOME}/logs/gateways/<name>/current` (10 archives × 1 MB each).
...
**Indagis Desktop (and the web dashboard).** The Desktop app's **Remote Gateway** connection talks to a `indagis dashboard` backend (default **port 9119**, enabled by `HERMES_DASHBOARD=1`) — *not* the OpenAI API server. One dashboard backend serves **every** co-located profile: the app's profile switcher sends the target profile with each request and the backend opens that profile's `INDAGIS_HOME` on disk. So you do **not** need a second port — or a second connection — per profile for Desktop; one `:9119` connection covers them all through the switcher.
...
|---|---|---|
| **Per-profile gateway** (`indagis gateway run` and per-profile gateways under s6) | Tee'd to two places: `docker logs <container>` (real time, no extra prefix) **and** `${INDAGIS_HOME}/logs/gateways/<profile>/current` (rotated, ISO-8601 timestamped, 10 archives × 1 MB each) | `docker logs -f indagis` or `tail -F ~/.indagis/logs/gateways/default/current` on the host |
| **Dashboard** (when `HERMES_DASHBOARD=1`) | `docker logs <container>` (no prefix) | `docker logs -f indagis` — interleaved with gateway lines |
| **Boot reconciler** (records which profile gateways were restored on each container start) | `${INDAGIS_HOME}/logs/container-boot.log` (append-only audit log) | `tail -F ~/.indagis/logs/container-boot.log` |
| **Generic Indagis logs** (`agent.log`, `errors.log`) | `${INDAGIS_HOME}/logs/` (profile-aware) | `docker exec indagis indagis logs --follow [--level WARNING] [--session <id>]` |
...
Start with `docker compose up -d` and view logs with `docker compose logs -f`. The supervised gateway's stdout is also tee'd to `${INDAGIS_HOME}/logs/gateways/<profile>/current` on the volume — see [Where the logs go](#where-the-logs-go) for the full routing map.
...
```sh
docker exec indagis /opt/hermes/.venv/bin/python -c "import sounddevice as sd; print(sd.query_devices())"
```
...
The image treats `/opt/hermes` as an immutable install tree at runtime. Optional Python extras, Node workspaces, and TUI assets that must be available inside Docker need to be baked during the image build; runtime lazy installs are disabled so supervised gateways and `docker exec indagis …` commands do not try to write dependency artifacts back into the read-only source tree.
...
`docker exec indagis <cmd>` defaults to running as root inside the container, but the image ships a thin shim at `/opt/hermes/bin/hermes` (earliest on PATH) that detects root callers and transparently re-execs through `s6-setuidgid indagis`. So `docker exec indagis login`, `docker exec indagis profile create …`, `docker exec indagis setup`, etc. all write files owned by UID 10000 — i.e. readable by the supervised gateway — with no extra `--user` flag needed. Non-root callers (the supervised processes themselves, `docker exec --user indagis`, kanban subagents inside the container) hit a short-circuit that exec's the venv binary directly, so there's no overhead on the hot paths.
...
against the mounted `$INDAGIS_HOME/config.yaml` before starting the gateway.
...
If a tool is likely to be useful to most Indagis Agent users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [hermes-agent repository](https://github.com/NousResearch/hermes-agent) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.
