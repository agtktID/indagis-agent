...
By default the app starts and manages its own **local** backend. You can instead point it at an Indagis backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
`$INDAGIS_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Boot logs land in `INDAGIS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
INDAGIS_HOME=/tmp/throwaway npm run dev
