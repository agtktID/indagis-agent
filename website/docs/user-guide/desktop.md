...
- **CLI** (`indagis`) and **[TUI](./tui.md)** (`indagis --tui`) — terminal interfaces.
...
Download the app from the [Indagis Desktop product page](https://agent.indagis-labs.fr/desktop), or follow the [installation instructions for Indagis Desktop](../getting-started/installation.md).
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `indagis desktop --cwd <path>` (or the `HERMES_DESKTOP_CWD` environment variable).
...
- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with Indagis — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
roster where every [Indagis profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Indagis cron). The roster lives in the left
...
each other's Bot Chats directly (`indagis -p <bot> chat`). The backend teaches
...
The [manual update process](https://agent.indagis-labs.fr/docs/getting-started/updating) also works with the GUI.
...
:::note
Running `indagis uninstall --gui` from a **source checkout** (a `indagis desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `indagis desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `indagis desktop`

To launch via the CLI, simply run `indagis desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Indagis gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Indagis Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Indagis gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Indagis Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Indagis Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `indagis update` to every eligible gateway at once — Indagis Cloud entries are skipped (the platform updates them), and each instance reports its own result.
...
- **OAuth (Indagis Cloud) — preferred for anything reachable beyond your own machine.** Logins are verified against your Nous account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `indagis dashboard register` (or the Portal [`/local-dashboards`](https://portal.indagis-labs.fr/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with Indagis Labs**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Indagis Labs](./features/web-dashboard.md#default-provider-nous-research).

### On the backend (the remote machine)

Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.indagis/.env` (the secrets file, mode 0600):
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/indagis` work if you front it with a reverse proxy)
2. **Sign in** — the app detects which provider the backend advertises and adapts the button. For a username/password backend it shows a **Sign in** button that opens a credential form (enter the credentials from step 1). For an OAuth backend it shows **Sign in with `<provider>`** (e.g. *Sign in with Indagis Labs*), which runs the provider's browser sign-in. Either way the app ends up with an authenticated session against the backend.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.indagis/.env` and that the dashboard process actually loaded them.
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `indagis debug share --nous`; secrets are always redacted, the bundle is
  ...
  to GitHub Issues, Indagis Cloud Support, and Discord. On a remote or Cloud
...
rm "$HOME/.hermes/indagis-agent/.hermes-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.hermes/indagis-agent/venv"
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.hermes/indagis-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
(Desktop, then Downloads, then Documents, ...) as Indagis touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Indagis' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Indagis.app** if you use Desktop.
3. Fully quit and relaunch them once.

`indagis doctor` reports whether the current terminal context already has the
grant, and `indagis setup` shows this tip on macOS when it doesn't.
...
Indagis. Grants are stable from then on.
...
certificate once and tell Indagis to use it. The one-shot command does
...
```bash
indagis desktop --setup-tcc-identity
```
...
4. `indagis config set desktop.macos_signing_identity "Indagis Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Indagis Local Signing`). The command is idempotent —
