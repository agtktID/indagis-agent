...
Indagis runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Indagis
...
1. Indagis captures a baseline of current diagnostics for the file.
...
…). Indagis auto-detects the binary on PATH or in
`<INDAGIS_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Indagis at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<INDAGIS_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`indagis lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Indagis installs both packages together when you
run `indagis lsp install typescript` or auto-install fires on first
use.

## CLI

```
indagis lsp status          # service state + per-server install status
indagis lsp list            # registry, optionally --installed-only
indagis lsp install <id>    # eagerly install one server
indagis lsp install-all     # try every server with a known recipe
indagis lsp restart         # tear down running clients
indagis lsp which <id>      # print resolved binary path
```

`indagis lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <INDAGIS_HOME>/lsp/bin
...
When `install_strategy: auto`, Indagis installs binaries into
`<INDAGIS_HOME>/lsp/bin/`. NPM packages land in
`<INDAGIS_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
**`indagis lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<INDAGIS_HOME>/lsp/bin/`. Run
`indagis lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `indagis lsp status`**
...
When `indagis lsp status` shows a `Backend warnings` section, install
...
`~/.indagis/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.indagis/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `indagis lsp restart` to clear the set;
