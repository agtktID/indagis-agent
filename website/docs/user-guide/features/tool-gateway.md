...
sidebar_position: 42
description: "**One subscription. Every tool built in.**"
---
...
The Tool Gateway is included with every paid [Indagis Cloud](https://portal.indagis-labs.fr) subscription. It routes Indagis' tool calls — web search, image generation, text-to-speech, and cloud browser automation — through infrastructure Nous already runs, so you don't have to sign up with Firecrawl, FAL, OpenAI, Browser Use, or anyone else just to make your agent useful.

<div style={{display: 'flex', gap: '1rem', flexWrap: 'wrap', margin: '1.5rem 0'}}>
  <a href="https://portal.indagis-labs.fr/manage-subscription" style={{background: 'var(--ifm-color-primary)', color: 'white', padding: '0.75rem 1.5rem', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold'}}>Start or manage subscription →</a>
</div>
...
| 🎨 | **Image generation** | Nine models under one endpoint: **FLUX 2 Klein 9B**, **FLUX 2 Pro**, **Z-Image Turbo**, **Nano Banana Pro** (Gemini 3 Pro Image), **GPT Image 1.5**, **GPT Image 2**, **Ideogram V3**, **Recraft V4 Pro**, **Qwen Image**. Pick per-generation with a flag, or let Indagis default to FLUX 2 Klein. |
...
- **One key.** Your Indagis Cloud OAuth covers every tool.
...
```bash
indagis setup --portal     # Fresh install: Nous OAuth + set Nous as provider + turn on the Tool Gateway in one go
```

```bash
indagis model              # Switch your inference provider to Indagis Cloud — Indagis then offers to turn on the gateway for all tools
```

```bash
indagis tools              # Enable the gateway per-tool — pick "Nous Subscription" for any tool you want
```

`indagis setup --portal` and `indagis model` are the all-at-once paths: log in once, optionally flip every tool to the gateway. `indagis tools` is the à la carte path — turn on just the tools you want, one at a time.

**You don't have to log in first.** With `indagis tools`, the Nous-managed backends (Web search, Image, Video, TTS, Browser) are always listed, even if you've never signed into Indagis Cloud. Select one and Indagis runs the Portal login right there if you aren't already authenticated — no need to run `indagis model` beforehand. If your Nous OAuth is already active, selecting the backend enables it immediately with no extra prompt. This path only logs you in and turns on the one tool you picked — it does **not** switch your inference provider, and it does **not** prompt you to enable the gateway for every other tool.

Check what's active at any time:

```bash
indagis portal info        # Portal auth + Tool Gateway routing summary
indagis portal tools       # Gateway catalog with current routing per tool
indagis status             # Full system status (Tool Gateway is one section)
```

`indagis portal info` shows a section like:

```
◆ Nous Tool Gateway
  Indagis Cloud     ✓ managed tools available
...
The Tool Gateway is a **paid-subscription** feature. Free-tier Nous accounts can use Portal for inference but don't include managed tools — [upgrade your plan](https://portal.indagis-labs.fr/manage-subscription) to unlock the gateway.
...
Picking a Nous model (`indagis model`) offers a per-tool checklist of gateway backends. Its behavior respects your existing setup:
...
```bash
indagis tools          # Interactive picker for each tool category
```

Select the tool, pick **Nous Subscription** as the provider (or any direct provider you prefer). No config editing required. If you aren't logged into Indagis Cloud yet, picking **Nous Subscription** kicks off the Portal login inline — you don't need to authenticate through `indagis model` first.
...
The set evolves — `indagis tools` → Image Generation shows the current live list.

---

## Configuration reference

Most users never need to touch this — `indagis model` and `indagis tools` cover every workflow interactively. This section is for writing config.yaml directly or scripting setups.

### One selection key per tool category

Each tool category has a single provider-selection key, written by the `indagis tools` picker (or the desktop GUI). Picking the **Nous Subscription** row stores the value `nous`, which routes that category through the managed Tool Gateway. Picking a BYOK row stores the vendor name (`fal`, `openai`, `firecrawl`, `browser-use`, ...), which goes direct with your own credentials:
...
image_gen:
  provider: indagis         # image generation via the Tool Gateway

tts:
  provider: indagis         # TTS via the Tool Gateway

stt:
  provider: indagis         # speech-to-text via the Tool Gateway

browser:
  cloud_provider: indagis   # cloud browser via the Tool Gateway
```

The runtime **always uses the stored selection** — credential presence never selects or reroutes a category. A `FAL_KEY` sitting in `.env` is ignored while `image_gen.provider: indagis`; conversely, `image_gen.provider: fal` with no `FAL_KEY` set produces a clear error instead of silently falling back to the gateway:

```
image_gen is configured to use fal (set via indagis tools), but FAL_KEY is not set. Run 'indagis tools' to change it.
```

Categories you have **never configured** (no selection key ever written) autodetect from available credentials, same as before. But once a selection exists, adding a key to `.env` does not change the route — only `indagis tools` (or editing the selection key) does.

### Switching back to your own keys

```bash
indagis tools    # pick the tool → choose a direct provider (e.g. Firecrawl)
```
...
```yaml
web:
  backend: firecrawl   # Indagis now uses FIRECRAWL_API_KEY from .env
```

### Legacy `use_gateway` flag (deprecated)

Older Indagis versions used a per-tool `use_gateway: true` boolean to route through the gateway. That flag is **legacy**: it is never written anymore, and the `indagis tools` picker removes it from a category's config when it rewrites the selection. Old configs that still contain `use_gateway: true` are interpreted at read time as the `nous` selection, so existing setups keep working. Don't set `use_gateway` in new configs — select the provider in `indagis tools` instead.
...
Tools routed through the gateway stop working until you renew or swap in direct API keys via `indagis tools`. Indagis shows a clear error pointing at the portal.

### Can I see usage or costs per tool?

Yes — the [Indagis Cloud dashboard](https://portal.indagis-labs.fr) breaks usage down by tool so you can see what's driving your bill.

### Is Modal (serverless terminal) included?

Modal is available as an **optional add-on** through the Nous subscription, not part of the default Tool Gateway bundle. Configure it via `indagis setup terminal` or directly in `config.yaml` when you want a remote sandbox for shell execution.

### Do I need to delete my existing API keys when I enable the gateway?

No — keep them in `.env`. While a tool's selection is **Nous Subscription**, direct keys for that tool are simply ignored. Pick the direct provider again in `indagis tools` and your keys become the source again. The gateway isn't a lock-in.
