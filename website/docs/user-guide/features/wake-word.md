...
title: "Wake Word"
sidebar_position: 48
description: "The wake word turns Indagis into a hands-free assistant across the CLI, TUI, and"
---
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey indagis"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey indagis"** — a model for it ships with Indagis, so
...
```bash
cd ~/.indagis/indagis-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `indagis` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.indagis/config.yaml`,
...
wake_word:
  ...
  phrase: "hey indagis"        # cosmetic label only — detection is keyed by the model/keyword below
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey indagis" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey indagis" utterances are missed.
...
| `cli` | Only the classic `indagis` CLI. |
| `tui` | Only `indagis --tui`. |
...
command (`indagis -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.indagis/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("indagis" qualifies) beat common words like
"hello" or "stop".
:::
...
wake_word:
  ...
  phrase: "hey indagis"
  porcupine:
    keyword: ~/.indagis/wakewords/hey_hermes.ppn
```

Set your access key in `~/.indagis/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `indagis tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `indagis-agent[wake]`).
...
```bash
indagis config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
indagis config set wake_word.input_device null
```
