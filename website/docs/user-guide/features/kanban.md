...
1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `indagis kanban complete` *inside* the container, where `hermes` isn't installed and `~/.indagis/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.indagis/kanban.db` regardless of terminal backend.
