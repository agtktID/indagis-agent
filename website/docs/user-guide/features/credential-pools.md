...
sidebar_position: 11
description: "Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Indagis…"
---

# Credential Pools

Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Indagis automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Indagis Cloud](/integrations/indagis-cloud) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Indagis auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
indagis auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
indagis auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
indagis auth add anthropic --type oauth
...
Run `indagis auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `indagis auth` | Interactive pool management wizard |
| `indagis auth list` | Show all pools and credentials |
| `indagis auth list <provider>` | Show a specific provider's pool |
| `indagis auth add <provider>` | Add a credential (prompts for type and key) |
| `indagis auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `indagis auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `indagis auth remove <provider> <index>` | Remove credential by 1-based index |
| `indagis auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `indagis auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `indagis model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via indagis model:
indagis auth list
# Shows:
...
indagis auth add Together.ai --api-key sk-together-second-key
```
...
Indagis automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Indagis PKCE OAuth | `~/.indagis/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `indagis auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `indagis auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Indagis can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Indagis-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
Pool state is stored in `~/.indagis/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Indagis' credential store, so its token remains persistable.
