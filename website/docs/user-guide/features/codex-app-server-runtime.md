...
:::tip
Not using OpenAI Codex? `indagis setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
(Indagis will preserve your override on re-migration as long as it lives outside the `# managed by hermes-agent` markers.)
...
```toml
# managed by hermes-agent — `indagis codex-runtime migrate` regenerates this section
...
env = { INDAGIS_HOME = "/your/.hermes", PYTHONPATH = "...", HERMES_QUIET = "1" }
...
If you find a bug, [open an issue](https://github.com/NousResearch/hermes-agent/issues) with the output of `indagis logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
