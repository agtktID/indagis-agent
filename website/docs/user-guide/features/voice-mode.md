...
title: "Voice Mode"
sidebar_position: 47
description: "Indagis Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and…"
---

# Voice Mode

Indagis Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Indagis](../../guides/use-voice-mode-with-indagis.md).

For hands-free session start — saying "hey indagis" (or any phrase) to open a fresh voice session on the CLI, TUI, or desktop app — see [Wake Word](/user-guide/features/wake-word).

## Prerequisites

Before using voice features, make sure you have:

1. **Indagis Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `indagis model` or set your preferred provider credentials in `~/.indagis/.env`
3. **A working base setup** — run `indagis` to verify the agent responds to text before enabling voice

:::tip
The `~/.indagis/` directory and default `config.yaml` are created automatically the first time you run `indagis`. You only need to create `~/.indagis/.env` manually for API keys.
:::

:::tip Indagis Cloud covers both
A paid [Indagis Cloud](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `indagis setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`indagis chat`) and the **TUI** (`indagis --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.indagis/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
indagis                # Start the interactive CLI
```
...
1. Start the CLI with `indagis` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.indagis/config.yaml` (default: `ctrl+b`).
:::
...
When Indagis Desktop is connected to a **remote gateway**, audio does not need to be relayed through the gateway at all. At voice-session start the desktop fetches the active profile's resolved STT/TTS settings (provider, model, language/voice, and credential) from the gateway over the authenticated REST channel (`GET /api/audio/voice-config`) and then calls the providers **directly**:
...
```bash
indagis gateway        # Start the gateway (connects to configured platforms)
indagis gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.indagis/.env`:
```bash
...
```bash
indagis gateway        # Start with existing configuration
```
...
If you are running Indagis inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.indagis/.env`
...
- Review gateway logs: `tail -f ~/.indagis/logs/gateway.log`
