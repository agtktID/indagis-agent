...
sidebar_position: 35
description: "Indagis can show an animated **pet** — a small mascot sprite that reacts to what"
---

# Pets

Indagis can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Indagis renders the sprite at full fidelity when your
...
indagis pets list
indagis pets list cat

# Install a pet and make it active in one step
indagis pets install boba --select
...
## `indagis pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `indagis pets list [query] [--limit N]` |
| List installed pets | `indagis pets list --installed` |
| Install a pet | `indagis pets install <slug> [--select] [--force]` |
...
| Preview/animate | `indagis pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `indagis pets off` |
| Remove an installed pet | `indagis pets remove <slug>` |
| Diagnose setup | `indagis pets doctor` |

`indagis pets show` flags:
...
Beyond installing pre-made pets from the gallery, Indagis can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `indagis pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Indagis state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Indagis Cloud**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nous run a quality-first model chain by default.

- Resolution order prefers Indagis Cloud → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `indagis tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
...
always-on-top desktop window. Out there it stays visible while Indagis is
...
Run `indagis pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Indagis uses its own
  profile-scoped `<HERMES_HOME>/pets/` instead — install through `indagis pets`.

## See also

- The [`indagis-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-indagis-agent.md)
