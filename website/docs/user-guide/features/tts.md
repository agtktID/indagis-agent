...
Each provider has a documented per-request input-character cap. Indagis truncates text before calling the provider so requests never fail with a length error:
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `cd ~/.indagis/hermes-agent && uv pip install -e ".[mistral]"`.
...
If your configured provider isn't available, Indagis automatically falls back:
