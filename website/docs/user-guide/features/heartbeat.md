...
sidebar_position: 20
description: "`/heartbeat` gives the **current session** one recurring instruction. Whenever the session is idle and the interval has elapsed, the prompt fires as…"
---
...
Inspired by Prime-Agent's `/heartbeat`. The Indagis adaptation keeps the strict message-flow invariants: the heartbeat is injected only between turns (never mid-run), as a plain user-role message.

## Heartbeat vs cron: which one do I want?

They look similar but serve different jobs:

| | `/heartbeat` | [`indagis cron`](./cron) |
|---|---|---|
...
`/hb` is an alias. Works on the CLI and gateway platforms (on Slack, use `/indagis heartbeat …`).
...
Indagis: [Heartbeat — recurring instruction, fires every 15m]
