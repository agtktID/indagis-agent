...
sidebar_position: 29
description: "MCP lets Indagis Agent connect to external tool servers so the agent can use tools that live outside Indagis itself — GitHub, databases, file…"
---

# MCP (Model Context Protocol)

MCP lets Indagis Agent connect to external tool servers so the agent can use tools that live outside Indagis itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Indagis to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Indagis' `config.yaml` — and `indagis import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Indagis tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Indagis to see
...
4. Ask Indagis to use the MCP-backed capability.
...
Indagis will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Indagis ships a curated catalog of MCP servers that Nous staff has reviewed
...
```
n8n          available              Manage and inspect n8n workflows from Indagis
...
`optional-mcps/` in the indagis-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Indagis prompts at install time and writes the value to
...
- **OAuth** (third-party provider like Google/GitHub) — Indagis points you at
  `indagis auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Indagis probes the MCP server to list every
...
the indagis-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/agtktID/indagis-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Indagis
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Indagis`) for that entry instead of silently hiding it. Run `indagis update`
to install the latest Indagis when you see that.
...
[`optional-mcps/`](https://github.com/agtktID/indagis-agent/tree/main/optional-mcps).
...
rejected), and Indagis's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Indagis connects to directly.
...
- you do not want Indagis spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Indagis handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Indagis identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Indagis Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. Indagis auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
On first connect, Indagis prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.indagis/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Indagis runs on a different machine than your browser, the loopback callback can't reach your laptop. Ways to complete the flow:

- **Indagis Desktop (automatic):** when you run the OAuth sign-in from the Desktop app's MCP setup UI against a remote backend, Desktop hosts the callback listener on *your* machine and relays the authorization back to the gateway automatically — no tunnel, paste, or proxy needed. Requires both the Desktop app and the backend to be up to date.
- **Paste-back (no setup):** on an interactive terminal Indagis prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Indagis on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Indagis expects them.
...
Then run `indagis mcp login googledrive` — with the pre-registered client, Indagis skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.indagis/config.yaml` from inside a running Indagis session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `indagis mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Indagis passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Indagis profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Indagis reads MCP config from `~/.indagis/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Indagis skips the server entirely |
...
## How Indagis registers MCP tools

Indagis prefixes MCP tools so they do not collide with built-in names:
...
- Indagis only registers resource utilities if the MCP session actually supports resource operations
- Indagis only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Indagis, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Indagis skips the server completely and does not even attempt a connection.
...
You can also separately disable Indagis-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Indagis does not create an empty runtime MCP toolset for that server.
...
Indagis discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Indagis when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Indagis receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Indagis does not blindly pass your full shell environment.
...
Then verify your config and restart Indagis.
...
Because Indagis now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Indagis may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Indagis via the `sampling/createMessage` protocol. This allows an MCP server to ask Indagis to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Indagis as an MCP server

In addition to connecting **to** MCP servers, Indagis can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Indagis's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Indagis
- You want a single MCP server that bridges to all of Indagis's connected messaging platforms at once
- You already have a running Indagis gateway with connected platforms
...
Add Indagis to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Indagis in a specific location:

```json
{
  "mcpServers": {
    "indagis": {
      "command": "/home/user/.hermes/indagis-agent/venv/bin/indagis",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server includes a live event bridge that polls Indagis's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
- The embedded `indagis mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Indagis, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
