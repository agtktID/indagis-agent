...
description: "Connect Indagis Agent to external tool servers via MCP — and control exactly which MCP tools Indagis loads"
---
...
Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Indagis handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
**Remote / headless hosts.** When Indagis runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:
