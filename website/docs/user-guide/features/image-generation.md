...
sidebar_position: 23
description: "Indagis Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost…"
---
...
If you have a paid [Indagis Cloud](https://portal.indagis-labs.fr) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `indagis setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the image-gen backend via `indagis tools`.

If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `indagis tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
`image_gen.provider` is the single selection key: `nous` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: indagis`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via indagis tools), but FAL_KEY is not set. Run 'indagis tools' to change it.` rather than silently rerouting. Change providers via `indagis tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nous` when `true`, but never written anymore.)
...
new models appear in the picker as soon as OpenRouter serves them; no Indagis
...
Indagis Cloud proxies the chat-completions protocol only, so its picker offers
