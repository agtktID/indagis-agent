...
sidebar_position: 41
description: "The subscription proxy is a local HTTP server that lets external apps —"
---
...
chat completions — use your Indagis-managed provider subscription as their
...
| Use case | "Use Indagis as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Indagis Cloud OAuth flow. Indagis stores
the refresh token in `~/.indagis/auth.json` — the same place all Indagis
...
```
Starting Indagis proxy for Indagis Cloud
...
Model:      Indagis-4-70B    # or Indagis-4.3-36B, Indagis-4-405B
```
...
```bash
indagis proxy providers
```

Currently shipped: `nous` (Indagis Cloud) and `xai` (xAI / Grok). More
...
```bash
indagis proxy status
```

```
Indagis proxy upstream adapters

  [nous    ] Indagis Cloud — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `indagis portal`. If you see
...
supports; check `portal.indagis-labs.fr/models`.
...
INFERENCE_TEXT_MODEL=Indagis-4-70B
```
...
```bash
indagis proxy start --host 0.0.0.0 --port 8645
```
...
[portal.indagis-labs.fr](https://portal.indagis-labs.fr).
