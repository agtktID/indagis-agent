...
title: "API Server"
sidebar_position: 2
description: "The API server exposes indagis-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat,…"
---

# API Server

The API server exposes indagis-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to indagis-agent and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
Indagis itself needs a configured provider and tool backends for the API server to be useful. A [Indagis Cloud](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `indagis setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "indagis-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Tool calls in the `output` array were already executed server-side by the Indagis agent — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `indagis-agent` for the default profile). Required by most frontends for model discovery.
...
Indagis can route browser tools through an authenticated extension that controls
the browser session associated with the current Indagis session. The feature is
...
register only for an existing server session. Indagis derives the controller
...
Indagis preserves the existing browser backend. Once the gateway binds a
...
Indagis never retries the same action through another backend.
...
2. Indagis returns a single-use ticket with a 30-second TTL and the filtered,
...
Indagis sends `browser.controller.command` frames containing `command_id`,
...
For safely retryable creation, send an `Idempotency-Key` header (1–255 visible ASCII characters). Indagis durably reserves the key before starting work. An identical retry returns the original `run_id` with HTTP 202 and `Idempotency-Replayed: true`, including after a gateway restart and after the run has completed, failed, or been cancelled. Reusing the same key with a different JSON payload returns HTTP 409 with code `idempotency_key_conflict`. Keys are isolated by authenticated API profile/credential and retained for 24 hours after their last status update; clients should use unique, unguessable keys and must not reuse them for unrelated operations. Requests without the header retain the legacy behavior and always create a new run.

When `session_id` identifies an existing Indagis session and no explicit
...
## Long-term memory scoping (`X-Indagis-Session-Key`)

Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-Indagis-Session-Id` (which rotates on `/new`). Pass `X-Indagis-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and Indagis threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.

```http
POST /v1/chat/completions HTTP/1.1
Authorization: Bearer ***
X-Indagis-Session-Id: transcript-alpha
X-Indagis-Session-Key: agent:main:webui:dm:user-42
```

Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-Indagis-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior Indagis had before.

## System Prompt Handling

When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), indagis-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.indagis/profiles/<profile>/.env`). The default
...
:::warning Security
The API server gives full access to indagis-agent's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `indagis-agent` for default profile. |

### config.yaml

The same settings can live in `~/.indagis/config.yaml` under a nested `gateway.api_server:` section:
...
- **Simple OpenAI clients still see an alias** — `/v1/models` advertises the
  stable Indagis alias (`indagis-agent` or the active profile name). Richer
