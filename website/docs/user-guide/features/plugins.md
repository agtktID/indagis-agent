...
title: "Plugins"
sidebar_position: 36
description: "Indagis has a plugin system for adding custom tools, hooks, and integrations without modifying core code."
---

# Plugins

Indagis has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Indagis
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Indagis Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.indagis/plugins/` with a `plugin.yaml` and Python code:
...
Start Indagis — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.indagis/plugins/hello-world/plugin.yaml`**
...
**`~/.indagis/plugins/hello-world/__init__.py`**

```python
"""Minimal Indagis plugin — registers a tool and a hook."""
...
Drop both files into `~/.indagis/plugins/hello-world/`, restart Indagis, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Indagis does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Indagis.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `indagis <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `indagis plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Indagis — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.indagis/plugins/` | Personal plugins |
| Project | `.hermes/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hermes_agent.plugins` entry_points | Distributed packages |
| Nix | `services.indagis-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Indagis also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.indagis/plugins/model-providers/<name>/` and `~/.indagis/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `indagis plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.indagis/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
indagis plugins                    # interactive toggle (space to check/uncheck)
indagis plugins enable <name>      # add to allow-list
indagis plugins disable <name>     # remove from allow-list + add to disabled
```

After `indagis plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
indagis plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Indagis checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `indagis plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`indagis plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Indagis' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.indagis/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.indagis/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Indagis tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Indagis runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Indagis never
materializes the prompt on another surface.

Indagis still owns hardline blocks, sudo-stdin protection, user deny rules,
...
Indagis has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.indagis/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Indagis also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Indagis Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Indagis Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Indagis Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `indagis <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Indagis auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `indagis skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.indagis/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `indagis plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
indagis plugins                               # unified interactive UI
indagis plugins list                          # table: enabled / disabled / not enabled
indagis plugins search <term>                 # search the community plugin index
indagis plugins install <name>                # install by index name (resolved to repo @ pinned ref)
indagis plugins install user/repo             # install from Git, then prompt Enable? [y/N]
indagis plugins install user/repo --enable    # install AND enable (no prompt)
indagis plugins install user/repo --no-enable # install but leave disabled (no prompt)
indagis plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
indagis plugins remove my-plugin              # uninstall
indagis plugins enable my-plugin              # add to allow-list
indagis plugins disable my-plugin             # remove from allow-list + add to disabled
indagis plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Indagis Desktop registers the `indagis://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
indagis://plugin/install?repo=owner/repo            # main install link
indagis://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
indagis://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Indagis and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `indagis://plugin-agent/…` and
`indagis://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `hermes-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="indagis://plugin/install?repo=owner/repo&enable=1">Install in Indagis</a>
```

MCP servers have the equivalent link form — see
[Add to Indagis link](/reference/mcp-config-reference#add-to-hermes-link).
...
When a plugin declares capabilities, `indagis plugins install` (and
`indagis plugins enable`) shows the list with one-line risk descriptions and
...
granted, `indagis plugins update` surfaces the additions and asks again. New
...
`indagis plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
indagis plugins capabilities             # all plugins with declared/granted capabilities
indagis plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Indagis has not reviewed the plugin's code. Only install
...
`indagis plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
indagis plugins search telegram               # fuzzy search
indagis plugins search                        # browse the whole index
indagis plugins search --capability platform  # filter by declared capability
indagis plugins search media --json           # machine-readable output
indagis plugins search --refresh              # bypass the 24h local cache
```
...
```bash
indagis plugins install hermes-media-studio
```
...
(`https://raw.githubusercontent.com/Indagis Labs/hermes-plugin-index/main/index.json`,
overridable via `indagis config set plugins.index_url <url>`). Fetches are
cached under `~/.indagis/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Indagis — so search works fully offline.
...
```json
{
  ...
  "repo": "Indagis Labs/hermes-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/Indagis Labs/hermes-media-studio",
...
[hermes-plugin-index](https://github.com/Indagis Labs/hermes-plugin-index)
...
```bash
indagis plugins pack show ./hermes-pack.yaml     # dry-run review
indagis plugins pack install ./hermes-pack.yaml  # review → confirm → install
indagis plugins pack export > hermes-pack.yaml   # snapshot the current install
indagis plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `indagis plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `indagis plugins install`.
...
provenance (installed via `indagis plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`indagis skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `indagis plugins install` and `indagis plugins update` runs a static
...
On `indagis plugins update`, a dangerous verdict on the updated tree
...
Running `indagis plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `indagis plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Indagis reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Indagis rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Indagis can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Indagis drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Indagis checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Indagis' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
