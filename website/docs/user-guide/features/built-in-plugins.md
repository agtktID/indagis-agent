...
description: "Plugins shipped with Indagis Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build an Indagis Plugin](/developer-guide/plugins) to write your own.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `INDAGIS_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under INDAGIS_HOME | always | Never |
...
**State** — everything lives at `$INDAGIS_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `INDAGIS_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
- Unlock state is persisted to `$INDAGIS_HOME/plugins/hermes-achievements/state.json`
...
**State files** — live under `$INDAGIS_HOME/plugins/hermes-achievements/`:
...
**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.indagis/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$INDAGIS_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Indagis plugin — see [Build an Indagis Plugin](/developer-guide/plugins). The only differences are:
