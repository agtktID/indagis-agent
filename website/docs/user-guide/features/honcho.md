...
title: "Honcho Memory"
sidebar_position: 21
description: "[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of…"
---

# Honcho Memory

[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Indagis's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Indagis instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
indagis memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `indagis` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Indagis syncs them back at session init.
...
The `indagis honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `indagis memory setup honcho` (or run `indagis memory setup` and pick it from the list); the `indagis honcho` subcommand then appears on the next invocation.

```bash
indagis memory setup honcho    # Configure Honcho directly (works before activation)
indagis honcho status          # Connection status, config, and key settings
indagis honcho setup           # Redirects to `indagis memory setup` (post-activation alias)
indagis honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
indagis honcho peer            # Show or update peer names + dialectic reasoning level
indagis honcho mode            # Show or set recall mode (hybrid/context/tools)
indagis honcho tokens          # Show or set token budget for context and dialectic
indagis honcho identity        # Seed or show the AI peer's Honcho identity
indagis honcho sync            # Sync Honcho config to all existing profiles
indagis honcho peers           # Show peer identities across all profiles
indagis honcho sessions        # List known Honcho session mappings
indagis honcho map             # Map current directory to a Honcho session name
indagis honcho enable          # Enable Honcho for the active profile
indagis honcho disable         # Disable Honcho for the active profile
indagis honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `indagis honcho`

If you previously used the standalone `indagis honcho setup`:
...
No re-login or re-setup needed. Run `indagis memory setup` and select "honcho" — the wizard detects your existing config.
