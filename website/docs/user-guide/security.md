...
title: "Security"
sidebar_position: 16
description: "Indagis Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container…"
---

# Security

Indagis Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Indagis checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.indagis/config.yaml`:
...
| `timeout` | `300` | Seconds Indagis waits for an approval reply before timing out. |
...
When YOLO is active, Indagis shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Indagis refuses to run them **regardless** of:
...
Configure the timeout in `~/.indagis/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.indagis/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.indagis/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` indagis/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.indagis/config.yaml`:
...
| Indagis credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under HERMES_HOME (active profile and global root) |
...
- Set automatically in the [official Docker image](https://github.com/agtktID/indagis-agent) (`HERMES_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.indagis/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.indagis/cron/jobs.json`, profile skills, or other Indagis state outside that prefix

To allow both a workspace and Indagis home:
...
### Cron and other Indagis state

Do not ask the agent to `patch` `~/.indagis/cron/jobs.json` directly. Use the `cronjob` tool, [`indagis cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Indagis control files when write safety blocks direct edits.
...
When running the messaging gateway, Indagis controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.indagis/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.indagis/.env to allow open access,
...
For more flexible authorization, Indagis includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.
...
:::tip Docker users: run pairing commands as the `indagis` user
The official Docker image runs the gateway as the unprivileged `indagis` user
...
Always pass `-u indagis`:

```bash
docker exec -u indagis indagis-agent indagis pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/agtktID/indagis-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.indagis/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Indagis applies strict security hardening to every container.
...
Container resources are configurable in `~/.indagis/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.indagis/sandboxes/docker/<task_id>/`
...
When loaded, Indagis checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Indagis infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Indagis infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Indagis integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Indagis under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.indagis/.env` with proper file permissions
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.indagis/.env`:
...
Indagis ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Indagis installs these **lazily** on first use rather than eagerly under `indagis-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
