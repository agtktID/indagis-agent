...
- **Do not add to `~/.indagis/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.indagis/cron/jobs.json`, profile skills, or other Hermes state outside that prefix
...
When loaded, Hermes checks if these files exist in the active profile's `INDAGIS_HOME` and registers them for mounting:
