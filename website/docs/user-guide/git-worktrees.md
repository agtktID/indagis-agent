...
- CLI: the directory where you run `hermes` or `indagis chat`
...
- Hermes checkpoint data under `~/.indagis/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.
