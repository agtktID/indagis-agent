...
:::tip
Quickest setup: run `indagis setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::
...
Profiles use the `INDAGIS_HOME` environment variable. When you run `coder chat`, the wrapper script sets `INDAGIS_HOME=~/.indagis/profiles/coder` before launching indagis. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Indagis state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `INDAGIS_HOME` is the profile boundary. It controls Indagis config, `.env`,
