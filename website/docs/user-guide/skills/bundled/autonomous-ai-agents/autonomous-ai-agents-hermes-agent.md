...
| Version | `3.1.0` |
| Author | Indagis Agent + Teknium |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `indagis`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `themes`, `skins`, `desktop-plugins`, `tui-widgets`, `petdex`, `development` |
...
Indagis Agent is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Indagis works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Indagis different:

- **Self-improving through skills** — Indagis learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Indagis instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Indagis feature. If an Indagis feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets:

- CLI commands: `indagis --help`, `indagis <command> --help`, and `hermes_cli/main.py`
...
# Other surfaces
indagis desktop                 # launch the native desktop app (alias: indagis gui)
indagis dashboard               # web admin panel + embedded chat
indagis proxy                   # OpenAI-compatible local proxy backed by your OAuth provider
```
...
| MCP servers (add, catalog, `indagis mcp`) | `references/native-mcp.md` |
...
Two theming rules that hold even without loading the reference: **you apply skins yourself** (`indagis config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`indagis skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional Indagis Instances

Run additional Indagis processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `indagis` process |
|-|-----------------|--------------------------|
...
Indagis uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `indagis chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Indagis; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Desktop app** (`indagis desktop` / `indagis gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`indagis dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `indagis --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`indagis --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`indagis proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
...
- **Never hand-edit `config.yaml` for the user** — use `indagis config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
