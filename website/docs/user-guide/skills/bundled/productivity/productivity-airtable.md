...
4. Store the token in `${HERMES_HOME:-~/.indagis}/.env` (or via `indagis setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:-~/.indagis}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
