...
Everything operator-facing is a `indagis teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${INDAGIS_HOME:-~/.indagis}/.env`:
...
```bash
indagis teams-pipeline validate              # config snapshot — run first after any change
indagis teams-pipeline token-health          # Graph token status
indagis teams-pipeline token-health --force-refresh   # force a fresh token acquisition
indagis teams-pipeline list                  # recent meeting jobs
indagis teams-pipeline list --status failed  # only failed jobs
indagis teams-pipeline show <job-id>         # full detail of one job
indagis teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
indagis teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
indagis teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
indagis teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```
...
indagis teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
indagis teams-pipeline delete-subscription <sub-id>
indagis teams-pipeline maintain-subscriptions            # renew near-expiry ones
indagis teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `indagis teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `indagis cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
