...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/devops/watchers` |
...
All three live in `$INDAGIS_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$INDAGIS_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $INDAGIS_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${INDAGIS_HOME:-~/.indagis}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $INDAGIS_HOME/skills/devops/watchers/scripts/watch_github.py \
...
```bash
python $INDAGIS_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$INDAGIS_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $INDAGIS_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $INDAGIS_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$INDAGIS_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
