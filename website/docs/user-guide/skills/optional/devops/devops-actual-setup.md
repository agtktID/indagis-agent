---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Indagis"
...
Set up Actual Computer (actual.inc) inference in Indagis.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `indagis skills install official/devops/actual-setup` |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as an Indagis inference
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.indagis/.env`.
...
3. Select provider + model:
   ```bash
   indagis config set model.provider actual
   indagis config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   indagis chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
3. Point Indagis at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   ...
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.indagis/.env`, then:
   ```bash
   indagis config set model.provider actual
   indagis config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   indagis chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
```bash
# Relay:
indagis chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
indagis chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
