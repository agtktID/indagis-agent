...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Indagis Agent |
| License | MIT |
...
The script reads `${INDAGIS_HOME:-~/.indagis}/.env` for two optional defaults:
...
Helper script: `~/.indagis/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${INDAGIS_HOME:-~/.indagis}/.env`.
