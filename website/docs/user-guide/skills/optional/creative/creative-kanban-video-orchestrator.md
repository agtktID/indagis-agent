...
short to a music video to an ASCII loop — in an Indagis Kanban pipeline that
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${INDAGIS_HOME:-~/.indagis}/.env` or the user's secret store.
