...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Indagis Kanban pipeline that
...
3. **Generates** a setup script that creates Indagis profiles, project workspace, and the initial kanban task
...
**[references/intake.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/intake.md)**.
...
**[references/role-archetypes.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/role-archetypes.md)**.

For mapping role → which Indagis skills + toolsets it loads, see
**[references/tool-matrix.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/tool-matrix.md)**.
...
3. Creates each Indagis profile via `indagis profile create --clone`
...
7. Fires the initial `indagis kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
indagis kanban watch --tenant <project-tenant>     # live events
indagis kanban list  --tenant <project-tenant>     # board snapshot
indagis dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `indagis kanban create` call to a specialist profile. The kanban
