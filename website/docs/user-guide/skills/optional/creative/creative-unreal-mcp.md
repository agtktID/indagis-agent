...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Indagis MCP catalog. The
...
Two halves, in this order: the editor side must be up before Indagis connects.
...
### One-time, Indagis side

    indagis mcp install unreal-engine
...
`~/.indagis/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Indagis — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Indagis connects
...
2. Start the Indagis session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Indagis
they appear as:

| Indagis tool | Purpose |
|---|---|
...
then happens at `indagis mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Indagis runs batched calls
...
- **Start order matters.** Editor + server up first, then the Indagis
...
- **Timeouts on long operations.** Indagis' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.indagis/config.yaml` for
