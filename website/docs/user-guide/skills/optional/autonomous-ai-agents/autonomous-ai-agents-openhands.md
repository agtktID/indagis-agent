...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/autonomous-ai-agents/openhands` |
| Path | `optional-skills/autonomous-ai-agents/openhands` |
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), Indagis Agent |
| License | MIT |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Indagis.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Indagis-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (Indagis subagents via `delegate_task`).
