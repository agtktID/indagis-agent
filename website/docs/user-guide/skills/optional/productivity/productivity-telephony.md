...
- save provider credentials into `${HERMES_HOME:-~/.indagis}/.env`
...
3. Buy it and save it into `${HERMES_HOME:-~/.indagis}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${HERMES_HOME:-~/.indagis}/.env`
