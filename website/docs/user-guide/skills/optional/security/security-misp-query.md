...
Search a MISP threat-intelligence instance for indicators (IOCs), events, and related context via the REST API.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `indagis skills install official/security/misp-query` |
| Path | `optional-skills/security/misp-query` |
| Version | `1.0.0` |
| Author | Indagis Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `threat-intel`, `misp`, `ioc`, `sharing-community`, `threat-hunting` |
| Related skills | [`virustotal-lookup`](/docs/user-guide/skills/bundled/security/security-virustotal-lookup), [`shodan-search`](/docs/user-guide/skills/bundled/security/security-shodan-search), [`sigma-rule-search`](/docs/user-guide/skills/bundled/security/security-sigma-rule-search) |
