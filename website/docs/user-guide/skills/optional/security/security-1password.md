...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Indagis Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Indagis terminal calls (desktop app flow only)
...
### Service Account (recommended for Indagis)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${INDAGIS_HOME:-~/.indagis}/.env` (the skill will prompt for this on first load).
...
## Indagis Execution Pattern (desktop app flow)

Indagis terminal commands are non-interactive by default and can lose auth context between calls.
