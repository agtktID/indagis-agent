...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Indagis or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Indagis. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.indagis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.indagis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Indagis integration, either:

- configure the server in `~/.indagis/config.yaml` using the `native-mcp` skill, or
...
### Indagis cannot see the deployed server

The server-building part may be correct while the Indagis config is not. Load the `native-mcp` skill and configure the server in `~/.indagis/config.yaml`, then restart Indagis.
