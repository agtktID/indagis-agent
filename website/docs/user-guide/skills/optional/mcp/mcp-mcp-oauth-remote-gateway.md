...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Indagis Agent |
| License | MIT |
...
# MCP OAuth on a Remote Indagis Gateway

## Overview

Indagis' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Indagis process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Indagis runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Indagis.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Indagis' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Indagis is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Indagis** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Indagis process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the indagis user, an
empty or missing tokens directory (`$INDAGIS_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Indagis detects a remote session it prints two
...
Both require an interactive terminal to the Indagis host. The rest of this skill
is for when there is NO interactive TTY — Indagis running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Indagis Dashboard (try this BEFORE manual token surgery)

A remote Indagis gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `indagis dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'indagis dashboard'`). It exposes a connector/MCP console —
...
across restarts) and anything reading credentials from `$INDAGIS_HOME/.env`. Those
...
Indagis' `HermesTokenStorage` would have written, so on `/reload-mcp` Indagis finds
cached tokens and skips the browser flow entirely.
...
uses these same env vars, so if Indagis' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find INDAGIS_HOME and the config path

```bash
INDAGIS_HOME=$(python3 -c 'from hermes_constants import get_hermes_home; print(get_hermes_home())')
echo "config: $INDAGIS_HOME/config.yaml"
echo "tokens: $INDAGIS_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Indagis Agent (manual OAuth)",
...
### 8. Write tokens in Indagis' exact schema

`tools/mcp_oauth.py::HermesTokenStorage` expects two files under
`$INDAGIS_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
UA** — Cloudflare will 403 you even though Indagis (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Indagis sees `auth: oauth`, calls `HermesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Indagis process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Indagis needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Indagis does natively).
...
- **Don't claim Indagis doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Indagis. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Indagis' config.
