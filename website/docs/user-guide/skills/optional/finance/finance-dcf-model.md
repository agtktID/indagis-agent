...
| | |
|---|---|
| Source | Optional — install with `indagis skills install official/finance/dcf-model` |
...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   ...
   - If `status` is `"errors_found"` → Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/agtktID/indagis-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Indagis:

- **If you have any structured financial-data MCP configured** (Indagis supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
