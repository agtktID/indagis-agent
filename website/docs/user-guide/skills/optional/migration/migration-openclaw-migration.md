---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Indagis"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Indagis"
---
...
Import an OpenClaw setup (memories, skills) into Indagis.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `indagis skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Indagis Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Indagis`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Indagis Migration

Use this skill when a user wants to move their OpenClaw setup into Indagis Agent with minimal manual cleanup.
...
```bash
indagis claw migrate              # Full interactive migration
...
indagis claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `indagis setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Indagis home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Indagis memory entries
- merge OpenClaw command approval patterns into Indagis `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Indagis working-directory configuration
- copy OpenClaw skills into `~/.indagis/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Indagis workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.indagis/tts/`
- archive non-secret docs that do not have a direct Indagis destination
...
- `~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.indagis/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.indagis/skills/migration/openclaw-migration/`.
...
Indagis CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Indagis. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into an Indagis workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Indagis skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```
...
```bash
python3 ~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.indagis/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Indagis unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Indagis targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Indagis destination. Unsupported auth blobs must still be reported as skipped.
...
- Indagis persona state imported
- Indagis memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.indagis/skills/openclaw-imports/`
