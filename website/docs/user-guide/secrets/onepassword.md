...
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.indagis/.env` has loaded, Hermes runs `op read` for each reference and sets the resolved values into `os.environ`.
