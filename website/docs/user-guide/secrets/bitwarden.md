...
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.indagis/.env` has loaded, Indagis calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
