...
title: "Bot Mode"
sidebar_position: 1
description: "**Bot Mode** turns your [Indagis profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and…"
---

# Bot Mode

**Bot Mode** turns your [Indagis profiles](./profiles.md) into a roster of named **Bots**. Each Bot has its own role, model, memory, skills, and avatar; Bots run recurring routines, deliberate together in group chats, and message each other directly. Build a specialist Bot once and it is there forever, one click away.
...
There is no new primitive to learn: a Bot **is** a Indagis profile — isolated config, memory, skills, credentials, and chat history under `~/.indagis/profiles/<name>/`. Bot Mode is a UI over that primitive, so everything you do in it is visible from the CLI too: `indagis -p <bot> chat` opens the same agent, and Bot routines appear in `indagis cron list`. No core patches, no background daemons, no extra storage.
:::
...
- **Model & provider pin** — give the Bot its own model. Any provider/model pair Indagis knows about works, and different Bots can run on different models side by side. Leave it unset to inherit from the launch profile.
...
- **A pixel pet** — a companion from the [petdex gallery](./features/pets.md) that bounces beside the avatar while the Bot is busy. Run `indagis pets` in a terminal to explore the gallery.
...
Every gateway you register in **Settings → Connections** — local, remote URL, SSH, Indagis Cloud, docker — is a persistent line the Desktop holds open, and Bot Mode uses those lines for messaging automatically. No extra setup:
...
```bash
indagis peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
indagis peer list
indagis peer dm spark < /tmp/dm.txt        # message body from a file (nothing shell-interpreted)
indagis peer dm spark/researcher < /tmp/dm.txt   # named profile on a multiplexed peer
indagis peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
indagis peer status spark run_abc123
indagis peer stop spark run_abc123
```

`indagis peer dm` delivers into the remote agent's canonical Bot Chat over the peer's existing API server, runs one agent turn there, and prints the reply on stdout — the exact cross-machine twin of the local `indagis -p <bot> chat` command.
...
Requirements: the peer machine runs the `api_server` gateway platform with a strong `API_SERVER_KEY`; reachability is your network's business (LAN, Tailscale, VPN). The key is a credential and lives in `~/.indagis/.env` as `HERMES_PEER_<NAME>_KEY`; peer names/URLs live in `config.yaml` under `bot_peers`.
...
| --- | --- |
| Chat with a Bot | `indagis -p <bot> chat` |
| A Bot's files, skills, memory | `~/.indagis/profiles/<bot>/` |
| Routines | `indagis cron list` (jobs named `[bot:<name>] …`) |
| Create / inspect profiles | `indagis profile create`, `indagis profile list` |
