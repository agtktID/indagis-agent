---
id: import-from-other-agents
title: "Import from Other Agents"
sidebar_position: 9
description: "`indagis import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Indagis with one command. It follows the same…"
---
...
indagis import-agent claude-code        # import from ~/.claude
indagis import-agent codex              # import from ~/.codex
indagis import-agent claude-code --dry-run          # preview only
indagis import-agent codex --source /path/to/.codex # custom location
indagis import-agent claude-code --overwrite --yes  # replace conflicts, skip prompts
```
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.indagis/skills/claude-code-imports/<name>/` |
...
| `memories/*.md` | Memory entries in `~/.indagis/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.indagis/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `indagis setup` to configure providers, or add secrets to `~/.indagis/.env`.
