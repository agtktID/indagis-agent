...
title: "Local Models"
sidebar_position: 10
description: "Indagis can run open models entirely on your own machine. It downloads and"
---

# Local Models

Indagis can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Indagis does the rest.
...
2. Click **Install runtime**. Indagis downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Indagis, restarts
...
Models ship in several quality grades (quantizations). Indagis picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Indagis never
...
Local models live or die by memory placement, so Indagis manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Indagis deliberately places the
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Indagis
...
If a llama-server is already running on your machine, Indagis detects it
...
[Run Indagis Locally with Ollama](/guides/local-ollama-setup) and
