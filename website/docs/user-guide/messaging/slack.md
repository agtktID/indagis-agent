...
title: "Slack"
sidebar_position: 1
description: "Connect Indagis Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of"
---
# Slack
# Slack Setup

Connect Indagis Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Indagis instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Indagis uses the modern
...
The fastest path is to paste a manifest Indagis generates for you. It
...
### Option A: From a Indagis-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   indagis slack manifest --agent-view --write
   ```
   This writes `~/.indagis/slack-manifest.json` and prints paste-in
   ...
   ```bash
   indagis slack manifest --agent-view \
...
4. Enter an app name (e.g., "Indagis Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Indagis can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Indagis configuration issue.
:::
...
Indagis uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
indagis gateway setup    # Select Slack when prompted
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Indagis now
...
Every Indagis command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Indagis command with its description.

Under the hood: Indagis ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Indagis Labs/indagis-agent/blob/main/hermes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Indagis
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Indagis can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Indagis only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Indagis adds new commands (e.g. after `indagis update`), regenerate
the manifest and update your Slack app:

```bash
indagis slack manifest --write
```
...
3. Paste the new contents of `~/.indagis/slack-manifest.json`
...
`/hermes bg run the tests` — Indagis routes it the same way as `/bg
...
Slack never delivers them to Indagis.

As a workaround, Indagis recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Indagis treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@Indagis !stop`) and with
...
Indagis falls back to a text prompt, the prompt instructs you to reply
...
fails, Indagis retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Indagis /cmd` — reply as normal visible
messages instead.)
...
```bash
indagis slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Indagis behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Indagis Agent what time is it?`). In channels, Indagis replies in a thread attached to that message. |
| **Threads** | If you @mention Indagis inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.indagis/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # If Slack rejects the native stream, Indagis keeps one editable text
      ...
      # that message itself @mentions Indagis, and "all" accepts every
      # other bot. Indagis always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
name in the thread. By default Indagis sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Indagis is thinking…"`) —
...
`streaming.enabled` is on (transport `auto` or `draft`), Indagis uses native
streaming automatically wherever it's available:

- The stream starts on the first frame and appends only deltas (the API is
  append-only). The streamed message **is** the final message — Indagis seals
...
- If your Slack app doesn't have the AI features enabled (or lacks the
  `assistant:write` scope), the first failure is cached and Indagis falls back
...
- If the native stream can't start or update, Indagis falls back to a single
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Indagis in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Indagis will respond.
...
By default Indagis ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Indagis instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Indagis always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Indagis agents are filtered consistently across workspaces.
...
`client_msg_id` — the same signature Indagis uses to recognise app posts — so it
is dropped as bot traffic. This blocks a common pattern: a custom front-end (an
internal dashboard, a mobile shell, a kiosk) that sends messages to Indagis *as*
the logged-in user.
...
manifest — regenerate with `indagis slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Indagis will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Indagis Agent`).
...
Delivery works even when the cron process isn't co-located with the gateway — Indagis falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Indagis can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Indagis also loads tokens from an **OAuth token file** at:

```
~/.indagis/slack_tokens.json
```
...
- When a message arrives, Indagis uses the correct workspace-specific client to respond.
...
Indagis supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Indagis Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Indagis Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Indagis now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Indagis Agent`)
...
- Tokens should be stored in `~/.indagis/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Indagis config directory
