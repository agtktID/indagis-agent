...
title: "Google Chat"
sidebar_position: 1
description: "Connect Indagis Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub"
---
# Google Chat
# Google Chat Setup

Connect Indagis Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Indagis
...
> Run `indagis gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Indagis can read (e.g.,
`~/.indagis/google-chat-sa.json`, `chmod 600`).
...
Also grant `Pub/Sub Viewer` on the same subscription — Indagis calls
...
- **App name**: whatever you want users to see ("Indagis" is reasonable).
...
`ADDED_TO_SPACE` event that Indagis uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Indagis

Add the Google Chat section to `~/.indagis/.env`:
...
Send "hola" in the test DM. The bot posts a "Indagis is thinking…" marker, then
...
`~/.indagis/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Indagis is thinking…").
...
Thread support: when a user replies inside a thread, Indagis detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Indagis session.
...
3. Download the JSON. Move it onto the host that runs Indagis.
4. Register the client with Indagis (run under the profile you want it scoped to):
...
indagis -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Indagis home (e.g.
`~/.indagis/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.indagis/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.indagis/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Indagis isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `indagis gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Indagis will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.indagis/google_chat_user_tokens/<sanitized_email>.json` (filesystem
