...
sidebar_position: 1
title: "Telegram"
description: "Set up Indagis Agent as a Telegram bot"
---
...
If you have local or plugin commands that should stay visible in Telegram's `/` picker, prioritize them in `~/.indagis/config.yaml`:
...
When this flag is on, Indagis still generates an internal session title (used by `hermes sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.
...
Send `/topic off` in the root DM. Indagis flips the row off, clears the chat's `(thread_id → session_id)` bindings, and the root DM reverts to a normal Indagis chat. Existing topics in Telegram aren't deleted — they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
