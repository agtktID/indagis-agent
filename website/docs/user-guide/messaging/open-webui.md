...
`hermes config set` auto-routes the flag to `config.yaml` and the secret to `~/.indagis/.env`. If the gateway is already running, restart it so the change takes effect:
