...
title: "Yuanbao"
sidebar_position: 1
description: "Connect Indagis to Yuanbao, Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and support..."
---
# Yuanbao

Connect Indagis to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.indagis/.env`:
...
All standard Indagis commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.indagis/logs/gateway.log`
...
4. Consider enabling verbose logging: `indagis gateway run -vv`
...
Yuanbao has a maximum message size. Indagis automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
indagis chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
