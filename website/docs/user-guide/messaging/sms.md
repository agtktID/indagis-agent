...
title: "SMS (Twilio)"
sidebar_position: 1
description: "Indagis connects to SMS through the Twilio API. People text your Twilio phone number and get AI responses back — same conversational experience as T..."
---
# SMS (Twilio)
# SMS Setup (Twilio)

Indagis connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.indagis/indagis-agent && uv pip install -e ".[sms]"`
...
If you're running Indagis locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Indagis will respond via SMS.
...
Indagis validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Indagis gateway logs for Twilio API errors
