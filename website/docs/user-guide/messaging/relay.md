...
Indagis Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Indagis Cloud access token from your existing login
   ...
   air-gapped / self-hosted-IdP path, no Indagis Cloud involved): with
...
`indagis gateway enroll` refuses to run in managed/hosted installs — there the
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Indagis Cloud: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `indagis auth add indagis` (or `indagis setup`) and retry.
...
startup. Restart it (`indagis gateway restart`).
