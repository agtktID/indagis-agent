...
sidebar_position: 9
title: "Matrix"
description: "Set up Indagis Agent as a Matrix bot"
---
...
2. Choose a username like `indagis` — the full user ID will be `@indagis:your-server.org`.
...
```bash
MATRIX_USER_ID=@indagis:your-server.org
...
# MATRIX_USER_ID=@indagis:matrix.example.org
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@indagis:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@indagis:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@indagis:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@indagis:your-server';
   "
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@indagis:your-server.org"},
       "password": "***",
