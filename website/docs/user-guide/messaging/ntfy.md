...
title: "ntfy"
sidebar_position: 1
description: "ntfy is a simple HTTP-based pub-sub notification service. It works with the free public server at ntfy.sh or any self-hosted instance, and supports..."
---
# ntfy
...
ntfy makes a great lightweight push channel for Indagis: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Indagis adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Indagis replies:
