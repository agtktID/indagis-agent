...
sidebar_position: 5
description: "Indagis uses two kinds of model slots:"
---
...
:::tip Fastest path: Indagis Cloud
[Indagis Cloud](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `indagis setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `indagis portal info`.
...
```bash
indagis config set security.allow_data_training_tiers_noninteractive true
```

Indagis still prints the full data-policy warning and the acknowledgement key on every unattended startup, so worker logs retain an audit trail. This setting does not approve expensive-model or provider-routing warnings, and it does not replace the interactive confirmation prompt. Revoke it with `indagis config unset security.allow_data_training_tiers_noninteractive`.
...
|---|---|
| **Title Gen** | When title latency or cost matters more than matching the main model. Pin a known-good flash model, or set `auxiliary.title_generation.prefer_fast_model: true` to let Indagis choose the provider's fast tier. |
...
| **Skills Hub** | `indagis skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`indagis kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`indagis profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
Provider entries (`providers.<name>` in the `providers:` dict, or items in the legacy `custom_providers` list) accept knobs that shape how Indagis talks to the endpoint:
...
With discovery off, the model picker (`indagis model`, `/model`) shows the configured list instead of a live probe.
...
Indagis matches this declaration to the exact provider route and runtime model
...
omitted, Indagis keeps its normal provider and model capability detection.

:::note Legacy format
Older configs used a top-level `custom_providers:` list (with `base_url` instead of `api`). It still works and is auto-migrated to the `providers:` dict on `indagis update` (config v12).
:::

## When does it take effect?

- **CLI** (`indagis chat`): next `indagis chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`indagis gateway restart`) if you want to force all sessions to pick up the change.
...
Inside any `indagis chat` session:
...
Define your own short names for models you reach for often, then use `/model <alias>` in a running session or `indagis chat --model <alias>` at startup. There are two equivalent formats — pick whichever fits your workflow.
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `indagis config set` writes scalars and now also parses inline list/mapping literals, though this short alias form still can't carry a custom `base_url`:
...
> `indagis config set` also accepts inline **list/mapping literals** (JSON/YAML flow style). Quote them so your shell passes them through intact:
>
> ```bash
> indagis config set platform_toolsets.line '["clarify", "file", "web"]'
> indagis config set display.tool_progress_overrides '{"terminal": "off"}'
> ```
...
### `indagis model` subcommand

```bash
indagis model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`indagis model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.default` in `~/.indagis/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `indagis config get model --json` and `indagis status`.

### Direct config edit

Edit `~/.indagis/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Indagis-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Indagis-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Indagis-Session-Token: $TOKEN" \
