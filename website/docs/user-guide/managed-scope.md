...
title: "Managed Scope"
sidebar_position: 11
description: "**Managed scope** lets an administrator push a baseline of configuration and"
---
...
`~/.indagis/config.yaml`, `~/.indagis/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/indagis`:

```text
/etc/indagis/
├── config.yaml     # managed config layer (wins over ~/.indagis/config.yaml)
└── .env            # managed env layer (wins over ~/.indagis/.env + shell)
```
...
files. It is **never persisted** to any `.env` by Indagis.
...
left user-settable. `indagis doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/indagis/config.yaml` (managed) | `/etc/indagis/.env` (managed) |
| 2 | `~/.indagis/config.yaml` (user) | `~/.indagis/.env` (user) |
...
```bash
indagis config        # shows a header naming the managed source + the pinned keys
indagis doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Indagis refuses and names the source:

```bash
$ indagis config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/indagis/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `indagis config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/indagis

# Pin some config values for every user on this machine
sudo tee /etc/indagis/config.yaml >/dev/null <<'YAML'
model:
  provider: indagis
security:
  redact_secrets: true
...
sudo tee /etc/indagis/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/indagis
sudo chmod 0644 /etc/indagis/config.yaml /etc/indagis/.env
```

Changes take effect on the next Indagis start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`indagis doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Indagis as `root`), managed scope is advisory.
