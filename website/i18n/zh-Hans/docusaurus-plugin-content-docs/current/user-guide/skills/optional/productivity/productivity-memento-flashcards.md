...
| | |
|---|---|
| 来源 | 可选 — 使用 `indagis skills install official/productivity/memento-flashcards` 安装 |
...
:::info
以下是 Indagis 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时所看到的指令内容。
:::
...
```
~/.indagis/skills/productivity/memento-flashcards/data/cards.json
```
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py add \
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py due
```

返回 `next_review_at <= now` 的卡片 JSON 数组。如需集合过滤：

```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py due --collection "History"
```
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py rate \
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/youtube_quiz.py fetch VIDEO_ID
```
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py add-quiz \
  --video-id "VIDEO_ID" \
...
**导出：**
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py export \
...
**导入：**
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py import \
  --file ~/flashcards.csv \
  --collection "Imported"
```
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py stats
```
...
```bash
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py stats
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py add --question "Capital of France?" --answer "Paris" --collection "General"
python3 ~/.indagis/skills/productivity/memento-flashcards/scripts/memento_cards.py due
```
