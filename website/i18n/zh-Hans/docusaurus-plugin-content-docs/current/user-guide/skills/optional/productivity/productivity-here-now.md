...
| | |
|---|---|
| 来源 | 可选 — 通过 `indagis skills install official/productivity/here-now` 安装 |
...
bash "$PUBLISH" {file-or-dir} --client indagis
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client indagis
```
...
| `--client {name}` | 用于归因的 agent 名称（如 `indagis`） |
