...
| | |
|---|---|
| 来源 | 可选 — 通过 `indagis skills install official/creative/meme-generation` 安装 |
...
4. 找到 skill 的脚本目录：
   ```
   SKILL_DIR=$(dirname "$(find ~/.indagis/skills -path '*/meme-generation/SKILL.md' 2>/dev/null | head -1)")
   ```
5. 运行生成器：
   ```bash
