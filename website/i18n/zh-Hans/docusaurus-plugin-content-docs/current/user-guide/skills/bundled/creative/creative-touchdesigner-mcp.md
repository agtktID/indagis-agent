...
```
Indagis Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${INDAGIS_HOME:-$HOME/.hermes}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

脚本将：
1. 检查 TD 是否正在运行
2. 如果尚未缓存，下载 twozero.tox
3. 将 `twozero_td` MCP 服务器添加到 Indagis 配置（如果缺失）
