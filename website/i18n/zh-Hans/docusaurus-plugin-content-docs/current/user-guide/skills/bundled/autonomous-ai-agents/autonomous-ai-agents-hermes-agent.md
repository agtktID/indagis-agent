---
title: "Indagis Agent — 配置、扩展或贡献 Indagis Agent"
sidebar_label: "Indagis Agent"
description: "配置、扩展或贡献 Indagis Agent"
---
...
| 版本 | `2.1.0` |
| 作者 | Indagis Agent + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `indagis`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 Indagis 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Indagis Agent

Indagis Agent 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Indagis 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

Indagis 的差异化特性：

- **通过 skill 自我提升** — Indagis 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Indagis 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Indagis 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。

**此 skill 帮助你高效使用 Indagis Agent** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。
...
# 单次查询
indagis chat -q "What is the capital of France?"
...
```
indagis setup [section]      Interactive wizard (model|terminal|gateway|tools|agent)
indagis model                Interactive model/provider picker
indagis config               View current config
indagis config edit          Open config.yaml in $EDITOR
indagis config set KEY VAL   Set a config value
indagis config path          Print config.yaml path
indagis config env-path      Print .env path
indagis config check         Check for missing/outdated config
indagis config migrate       Update config with new options
indagis auth                 交互式凭据管理器
indagis auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
indagis auth list            列出已存储的凭据
indagis auth remove PROVIDER 移除已存储的凭据
indagis doctor [--fix]       Check dependencies and config
indagis status [--all]       Show component status
```

### 工具与 Skill

```
indagis tools                Interactive tool enable/disable (curses UI)
indagis tools list           Show all tools and status
indagis tools enable NAME    Enable a toolset
indagis tools disable NAME   Disable a toolset

indagis skills list          List installed skills
indagis skills search QUERY  Search the skills hub
indagis skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
indagis skills inspect ID    Preview without installing
indagis skills config        Enable/disable skills per platform
indagis skills check         Check for updates
indagis skills update        Update outdated skills
indagis skills uninstall N   Remove a hub skill
indagis skills publish PATH  Publish to registry
indagis skills browse        Browse all available skills
indagis skills tap add REPO  Add a GitHub repo as skill source
```
...
indagis mcp add NAME         Add an MCP server (--url or --command)
indagis mcp remove NAME      Remove an MCP server
indagis mcp list             List configured servers
indagis mcp test NAME        Test connection
...
```
indagis gateway run          Start gateway foreground
indagis gateway install      Install as background service
indagis gateway start/stop   Control the service
indagis gateway restart      Restart the service
indagis gateway status       Check status
indagis gateway setup        Configure platforms
```
...
```
indagis sessions list        List recent sessions
indagis sessions browse      Interactive picker
indagis sessions export OUT  Export to JSONL
indagis sessions rename ID T Rename a session
indagis sessions delete ID   Delete a session
indagis sessions prune       Clean up old sessions (--older-than N days)
indagis sessions stats       Session store statistics
```

### Cron 任务

```
indagis cron list            List jobs (--all for disabled)
indagis cron create SCHED    Create: '30m', 'every 2h', '0 9 * * *'
indagis cron edit ID         Edit schedule, prompt, delivery
indagis cron pause/resume ID Control job state
indagis cron run ID          Trigger on next tick
indagis cron remove ID       Delete a job
indagis cron status          Scheduler status
```

### Webhook

```
indagis webhook subscribe N  Create route at /webhooks/<name>
indagis webhook list         List subscriptions
indagis webhook remove NAME  Remove a subscription
indagis webhook test NAME    Send a test POST
```

### Profiles

```
indagis profile list         List all profiles
indagis profile create NAME  Create (--clone, --clone-all, --clone-from)
indagis profile use NAME     Set sticky default
indagis profile delete NAME  Delete a profile
indagis profile show NAME    Show details
indagis profile alias NAME   Manage wrapper scripts
indagis profile rename A B   Rename a profile
indagis profile export NAME  Export to tar.gz
indagis profile import FILE  Import from archive
```

### 凭证池

```
indagis auth add             Interactive credential wizard
indagis auth list [PROVIDER] List pooled credentials
indagis auth remove P INDEX  Remove by provider + index
indagis auth reset PROVIDER  Clear exhaustion status
```

### 其他

```
indagis insights [--days N]  Usage analytics
indagis update               Update to latest version
indagis pairing list/approve/revoke  DM authorization
indagis plugins list/install/remove  Plugin management
indagis honcho setup/status  Honcho memory integration (requires honcho plugin)
indagis memory setup/status/off  Memory provider config
indagis completion bash|zsh  Shell completions
indagis acp                  ACP server (IDE integration)
indagis claw migrate         Migrate from OpenClaw
indagis uninstall            Uninstall Indagis
```
...
/snapshot [sub]      Create or restore state snapshots of Indagis config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Indagis works on across turns until achieved
...
/busy [sub]          Control what Enter does while Indagis is working (CLI)
...
/reload-skills       Re-scan ~/.indagis/skills/ for added/removed skills
...
/update              Update Indagis to latest (gateway)
...
```
~/.indagis/config.yaml       Main configuration
~/.indagis/.env              API keys and secrets
$INDAGIS_HOME/skills/        Installed skills
~/.indagis/sessions/         Session transcripts
...
Profiles 使用 `~/.indagis/profiles/<name>/`，布局相同。

### 配置节

使用 `indagis config edit` 或 `indagis config set section.key value` 编辑。
...
支持 20+ 个提供商。通过 `indagis model` 或 `indagis setup` 设置。

| 提供商 | 认证方式 | Key 环境变量 |
|----------|------|-------------|
| OpenRouter | API key | `OPENROUTER_API_KEY` |
| Anthropic | API key | `ANTHROPIC_API_KEY` |
| Nous Portal | OAuth | `indagis auth` |
| OpenAI Codex | OAuth | `indagis auth` |
...
| Qwen OAuth | OAuth | `indagis auth add qwen-oauth` |
...
通过 `indagis tools`（交互式）或 `indagis tools enable/disable NAME` 启用/禁用。
...
常见的"为什么 Indagis 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `indagis` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Indagis 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
indagis config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HERMES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `indagis config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。

再次禁用：
```bash
indagis config set security.redact_secrets false
```
...
```bash
indagis config set privacy.redact_pii true    # 启用
indagis config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: smart`），Indagis 会让辅助 LLM 评估被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）。模式如下：
...
```bash
indagis config set approvals.mode smart       # 推荐的折中方案
indagis config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.indagis/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `indagis tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Indagis 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `indagis` 进程 |
|-|-----------------|--------------------------|
...
```
terminal(command="indagis chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# 长任务后台运行：
terminal(command="indagis chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### 交互式 PTY 模式（通过 tmux）

Indagis 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：

```
# 启动
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'indagis'", timeout=10)
...
```
# Agent A：后端
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'indagis -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B：前端
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'indagis -w'", timeout=10)
...
```
# 恢复最近的会话
terminal(command="tmux new-session -d -s resumed 'indagis --continue'", timeout=10)

# 恢复特定会话
terminal(command="tmux new-session -d -s resumed 'indagis --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **fire-and-forget 使用 `indagis chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`indagis cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
- **CLI：** `indagis curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
- **遥测：** `~/.indagis/skills/.usage.json` 中的 sidecar 保存每个 skill 的 `use_count`、`view_count`、`patch_count`、`last_activity_at`、`state`、`pinned`。
...
用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `indagis kanban <verb>` 驱动；调度器生成的 worker 看到由 `HERMES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
Indagis 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`indagis config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 Indagis 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。Indagis 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Indagis 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `indagis tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `indagis doctor` — 检查配置和依赖
2. `indagis auth` — 重新认证 OAuth 提供商（或 `indagis auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `indagis model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `indagis skills list` — 验证已安装
2. `indagis skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `indagis -s name`

### Gateway 问题
首先检查日志：
```bash
grep -i "failed to send\|error" ~/.indagis/logs/gateway.log | tail -20
```
...
```bash
indagis config set auxiliary.vision.provider <your_provider>
indagis config set auxiliary.vision.model <model_name>
```
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `indagis config edit` 或[配置文档](https://hermes-agent.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `indagis tools list` 或[工具参考](https://hermes-agent.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://hermes-agent.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `indagis skills browse` 或[Skill 目录](https://hermes-agent.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `indagis model` 或[提供商指南](https://hermes-agent.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `indagis gateway setup` 或[消息文档](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `indagis mcp list` 或[MCP 指南](https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `indagis profile list` 或[Profiles 文档](https://hermes-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `indagis cron list` 或[Cron 文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `indagis memory status` 或[记忆文档](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `indagis config env-path` 或[环境变量参考](https://hermes-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `indagis --help` 或[CLI 参考](https://hermes-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.indagis/logs/gateway.log` |
| 会话文件 | `~/.indagis/sessions/` 或 `indagis sessions browse` |
| 源代码 | `~/.indagis/hermes-agent/` |
...
配置：`~/.indagis/config.yaml`（设置）、`~/.indagis/.env`（API key）。
...
所有处理器必须返回 JSON 字符串。路径使用 `get_hermes_home()`，永远不要硬编码 `~/.indagis`。
