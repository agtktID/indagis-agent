...
uv run python3 "${INDAGIS_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif [ -f ~/.indagis/.env ] && grep -q "^GITHUB_TOKEN=" ~/.indagis/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.indagis/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${INDAGIS_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
