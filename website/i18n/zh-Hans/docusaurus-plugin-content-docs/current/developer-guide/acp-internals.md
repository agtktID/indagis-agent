...
ACP 适配器将 Indagis 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
`acp_adapter/tools.py` 将 Indagis 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
而是复用 Indagis 的运行时解析器：

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

因此 ACP 通告并使用当前配置的 Indagis provider/凭据。它还始终通告一个终端 setup 认证方法（`hermes-setup`，参数 `--setup`），以便首次运行的 ACP 客户端在启动正常 ACP 会话前可以打开 Indagis 的交互式模型/provider 配置。
...
- ACP 会话持久化至共享的 `~/.indagis/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `hermes_cli/main.py` — `indagis acp` CLI 子命令
