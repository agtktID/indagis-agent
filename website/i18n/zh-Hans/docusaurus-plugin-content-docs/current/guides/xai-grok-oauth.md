...
description: "使用 SuperGrok 或 X Premium+ 订阅登录，在 Indagis Agent 中使用 Grok 模型——无需 API 密钥"
---

# xAI Grok OAuth（SuperGrok / X Premium+）

Indagis Agent 通过基于浏览器的 OAuth 登录流程支持 xAI Grok，认证服务器为 [accounts.x.ai](https://accounts.x.ai)，支持 **SuperGrok 订阅**（[grok.com](https://x.ai/grok)）或 **X Premium+ 订阅**（已关联的 X 账号）。无需 `XAI_API_KEY`——登录一次后，Indagis 会在后台自动刷新会话。
...
同一 OAuth bearer token 也会被 Indagis 中所有直连 xAI 的功能复用——TTS、图像生成、视频生成和转录——因此单次登录即可覆盖全部四项功能。
...
# → Indagis 打开或打印 accounts.x.ai 验证 URL
...
首次登录后，凭据存储在 `~/.indagis/auth.json` 中，并在过期前自动刷新。
...
在没有浏览器的服务器、容器、仅限浏览器的远程控制台（Cloud Shell、Codespaces、EC2 Instance Connect）或 SSH 会话中，Indagis 会打印 xAI 验证 URL 和用户代码。在笔记本电脑或云控制台的任意浏览器中打开该 URL，如有提示则输入代码，Indagis 会持续轮询直到 xAI 批准登录。无需 SSH 隧道或本地回调监听器。
...
1. Indagis 向 `auth.x.ai` 请求设备代码。
2. 你打开验证 URL，登录，如有提示则输入显示的代码，并批准访问。
3. Indagis 轮询 xAI 直到批准，然后将 token 保存到 `~/.indagis/auth.json`。
4. 此后，Indagis 在后台刷新 access token——你将保持登录状态，直到执行 `indagis auth logout xai-oauth` 或在 xAI 账号设置中撤销访问。
...
```bash
indagis config set model.default grok-build-0.1
indagis config set model.provider xai-oauth
```

## 配置参考

登录后，`~/.indagis/config.yaml` 将包含：
...
```bash
indagis --provider xai-oauth        # 规范名称
indagis --provider grok-oauth       # 别名
indagis --provider x-ai-oauth       # 别名
indagis --provider xai-grok-oauth   # 别名
```
...
:::note 视频生成默认关闭
`video_gen` 工具集默认禁用。在 `indagis tools` → `🎬 Video Generation`（按空格键）中启用后，agent 才能调用 `video_generate`。否则 agent 可能回退到内置的 ComfyUI 技能，该技能同样标记为视频生成。
:::

:::note 配置 xAI 凭据后 X 搜索自动启用
只要配置了 xAI 凭据（SuperGrok / X Premium+ OAuth token 或 `XAI_API_KEY`），`x_search` 工具集就会自动启用。如不需要，请通过 `indagis tools` → `🐦 X (Twitter) Search`（按空格键）显式禁用。该工具通过 xAI 内置的 `x_search` Responses API 路由——支持 **SuperGrok / X Premium+ OAuth 登录**或付费 `XAI_API_KEY`，两者同时配置时优先使用 OAuth（消耗订阅配额而非 API 费用）。未配置任何 xAI 凭据时，无论工具集是否启用，工具 schema 都对模型隐藏。
:::
...
要将 xAI 设为活跃 provider，请在 `config.yaml` 中设置 `model.provider: xai-oauth`（使用 `indagis setup` 进行引导配置），或在单次调用时传入 `--provider xai-oauth`。

## 故障排查

### Token 过期——未自动重新登录

Indagis 在每次会话前刷新 token，并在收到 401 时响应式地再次刷新。如果刷新因 `invalid_grant` 失败（刷新 token 被撤销或账号已轮换），Indagis 会显示类型化的重新认证消息，而不是崩溃。

当刷新失败是终态时（HTTP 4xx、`invalid_grant`、授权被撤销等），Indagis 将刷新 token 标记为失效并在本地隔离——后续调用跳过注定失败的刷新尝试，而不是反复重放同一个 401。agent 显示一条"需要重新认证"消息，并在你再次登录前保持等待。

**修复方法：** 再次运行 `indagis auth add xai-oauth` 开始全新登录。下次成功交换后隔离状态自动清除。

### 授权超时

设备代码批准有有限的过期窗口（xAI 在设备代码响应中设置 `expires_in`，通常为数十分钟量级）。如果你未在时限内批准登录，Indagis 会抛出超时错误。

**修复方法：** 重新运行 `indagis auth add xai-oauth`（或 `indagis model`）。流程重新开始。

### 从远程服务器登录

在 SSH 或容器会话中，Indagis 打印验证 URL 和用户代码，而不是打开浏览器。在笔记本电脑或云控制台的浏览器中打开该 URL——xAI Grok OAuth 无需 SSH 端口转发。
...
这**不是** token 过期问题——重新运行 `indagis model` 不会改变结果。xAI 的后端已被观察到将 OAuth API 访问限制在特定 SuperGrok 套餐，即使应用内订阅处于激活状态（issue [#26847](https://github.com/NousResearch/hermes-agent/issues/26847)）。
...
**修复方法：** 运行 `indagis model` 并选择 xAI Grok OAuth provider，或运行 `indagis auth add xai-oauth`。
...
这会清除 `auth.json` 中的单例 OAuth 条目以及 `xai-oauth` 的所有凭据池行。如果只想删除单个池条目，请使用 `indagis auth remove xai-oauth <index|id|label>`（运行 `indagis auth list xai-oauth` 查看列表）。

## 另请参阅

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — 如果 Indagis 与浏览器不在同一台机器上，必读
