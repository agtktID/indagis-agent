...
sidebar_position: 12
title: "Cron 故障排查"
description: "诊断并修复常见的 Indagis cron 问题——任务未触发、投递失败、skill 加载错误及性能问题"
---
...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `indagis gateway`，安装为服务用 `indagis gateway start`）。如需单次调试，可手动触发一次 tick：`indagis cron tick`。
...
```bash
date
indagis cron list   # 将 next_run 时间与本地时间对比
```
...
| 目标 | 所需配置 |
|--------|----------|
| `telegram` | `~/.indagis/.env` 中的 `TELEGRAM_BOT_TOKEN` |
| `discord` | `~/.indagis/.env` 中的 `DISCORD_BOT_TOKEN` |
| `slack` | `~/.indagis/.env` 中的 `SLACK_BOT_TOKEN` |
| `whatsapp` | 已配置 WhatsApp gateway |
| `signal` | 已配置 Signal gateway |
| `matrix` | 已配置 Matrix homeserver |
| `email` | `config.yaml` 中已配置 SMTP |
| `sms` | 已配置 SMS 提供商 |
| `local` | 对 `~/.indagis/cron/output/` 有写权限 |
...
若投递失败，任务仍会执行——只是不会发送到任何地方。检查 `indagis cron list` 中的 `last_error` 字段（如有）。
...
Skill 必须先安装才能附加到 cron 任务。若 skill 缺失，先用 `indagis skills install <skill-name>` 安装，或在 CLI 中通过 `/skills` 安装。

### 检查 2：检查 skill 名称与 skill 文件夹名称

Skill 名称区分大小写，必须与已安装 skill 的文件夹名称完全匹配。若任务指定的是 `ai-funding-daily-report`，但 skill 文件夹也是 `ai-funding-daily-report`，请从 `indagis skills list` 确认确切名称。
...
1. 任务投递的聊天会话（若投递成功）
2. `~/.indagis/logs/agent.log`（调度器消息）或 `errors.log`（警告信息）
3. 通过 `indagis cron list` 查看任务的 `last_run` 元数据

### 检查 2：常见错误模式

**脚本报 "No such file or directory"**
`script` 路径必须为绝对路径（或相对于 Indagis 配置目录的路径）。验证：
```bash
ls ~/.indagis/scripts/your-script.py   # 必须存在
indagis cron edit <job_id> --script ~/.indagis/scripts/your-script.py
```

**任务执行时报 "Skill not found"**
Skill 必须安装在运行调度器的机器上。若你在不同机器间切换，skill 不会自动同步——请用 `indagis skills install <skill-name>` 重新安装。
...
任务存储在 `~/.indagis/cron/jobs.json`。若该文件对当前用户不可读写，调度器将静默失败：

```bash
ls -la ~/.indagis/cron/jobs.json
chmod 600 ~/.indagis/cron/jobs.json   # 应由你的用户拥有
```
...
```bash
indagis cron list                    # 显示所有任务、状态、next_run 时间
indagis cron run <job_id>            # 安排在下次 tick 执行（用于测试）
indagis cron edit <job_id>           # 修复配置问题
indagis logs                         # 查看近期 Indagis 日志
indagis skills list                  # 确认已安装的 skill
```
...
1. 使用 `indagis cron run <job_id>` 运行任务（在下次 gateway tick 时触发），观察聊天输出中的错误
2. 查看 `~/.indagis/logs/agent.log` 中的调度器消息和 `~/.indagis/logs/errors.log` 中的警告
