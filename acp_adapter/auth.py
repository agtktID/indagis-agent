...
def build_auth_methods() -> list[Any]:
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Indagis using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Indagis provider",
            description=(
                "Open Indagis' interactive model/provider setup in a terminal. "
                "Use this when Indagis has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
