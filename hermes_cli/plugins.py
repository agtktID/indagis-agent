...
from utils import env_var_enabled, env_with_legacy_alias, fast_safe_load
...
def get_bundled_plugins_dir() -> Path:
    ...
    """
    env_override = env_with_legacy_alias("INDAGIS_BUNDLED_PLUGINS", "HERMES_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = env_with_legacy_alias("INDAGIS_PLUGINS_DEBUG", "HERMES_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `indagis plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
