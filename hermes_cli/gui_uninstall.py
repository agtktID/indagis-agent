...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$INDAGIS_HOME/hermes-agent/``:
       ...
       - ``$INDAGIS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$INDAGIS_HOME``:
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under INDAGIS_HOME.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    config / sessions / .env under ``$INDAGIS_HOME``.
