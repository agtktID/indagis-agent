    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `indagis auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `indagis dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    indagis dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `indagis dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `indagis setup` (or `indagis auth add nous`) first, then retry.")
        else:
