...
``$INDAGIS_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $INDAGIS_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$INDAGIS_HOME``,
    ...
    ``$INDAGIS_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's INDAGIS_HOME (typically /opt/data).
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    the gateway and dashboard containers share a bind-mounted INDAGIS_HOME,
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $INDAGIS_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted INDAGIS_HOME, both race to flock() the same s6-log lock
