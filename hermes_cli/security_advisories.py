"""
Security advisory checker for Indagis Agent.
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.indagis/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: indagis doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'indagis doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_indagis_home()) / "cache"
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `indagis doctor` on the gateway host for details.")
