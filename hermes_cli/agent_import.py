...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `indagis config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`indagis config edit` (or move it aside), then re-run the import."
        )
    return data
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Indagis equivalent — "
...
def import_agent_command(args) -> None:
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: indagis import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: indagis import-agent claude-code   or   indagis import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Indagis — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: indagis import-agent "
    ...
    print_info("Secrets:     never imported — run 'indagis setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: indagis import-agent {agent} --yes")
            return
