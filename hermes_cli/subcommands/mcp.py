...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Indagis as an MCP server",
        description=(
            "Manage MCP server connections and run Indagis as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'indagis mcp add' to connect to a new server, or\n"
            "'indagis mcp serve' to expose Indagis conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Indagis as an MCP server (expose conversations to other agents)",
    )
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `indagis mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `indagis mcp install n8n`)",
    )
