...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Indagis Agent. Use 'indagis debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    indagis debug share             Upload debug report (asks for confirmation)
    indagis debug share --yes       Skip confirmation (for scripts/CI)
    indagis debug share --lines 500 Include more log lines
    indagis debug share --expire 30 Keep paste for 30 days
    indagis debug share --local     Print report locally (no upload)
    indagis debug share --no-redact Disable upload-time secret redaction
    indagis debug share --nous      Upload to Nous-internal storage (private)
    indagis debug delete <url>      Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'indagis debug share'",
    )
