...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `indagis auth` to manage credentials, "
            "`indagis model` to select a provider, or `indagis setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `indagis model`",
    )
    login_parser.add_argument(
