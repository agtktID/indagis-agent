...
from hermes_constants import get_default_indagis_root, get_indagis_home, display_indagis_home
...
# INDAGIS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under INDAGIS_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE INDAGIS_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside INDAGIS_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside INDAGIS_HOME.
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    print(f"Scanning {display_indagis_home()} ...")
    ...
    # outside INDAGIS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_indagis_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_indagis_home()}")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under INDAGIS_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_indagis_home()}."
            )
...
# Critical state files to include in quick snapshots (relative to INDAGIS_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $INDAGIS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside INDAGIS_HOME and the entry is silently
...
# Relative path of the cron job database inside INDAGIS_HOME. Kept in sync with
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_indagis_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of INDAGIS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<INDAGIS_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    """
    hermes_root = hermes_home or get_default_indagis_root()
    if not hermes_root.is_dir():
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of INDAGIS_HOME under ``backups/`` before a
    ...
    ``<INDAGIS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
