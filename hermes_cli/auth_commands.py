...
def auth_add_command(args) -> None:
    ...
    raise SystemExit(f"`indagis auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `indagis auth status spotify`.")
