...
_UPSTREAM_REPO_URL = "https://github.com/agtktID/indagis-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/agtktid/indagis-agent"
...
def check_for_updates() -> Optional[int]:
    ...
    if embedded_rev:
        ...
        # $INDAGIS_HOME/hermes-agent/ may be a stale copy from --clone-all;
...
_RELEASE_URL_BASE = "https://github.com/agtktID/indagis-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical agtktID/indagis-agent repo (forks don't get a link).
    """
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or indagis setup[/]"
            )
        else:
