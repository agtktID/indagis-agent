    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                # before the rebrand and "# Indagis Agent" after, and an
                # uninstall must clean up either one.
...
#
#   1. Sets User-scope env vars ``INDAGIS_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if INDAGIS_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete INDAGIS_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("INDAGIS_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _is_default_hermes_home(hermes_home: Path) -> bool:
    ...
    try:
        ...
        return hermes_home.resolve() == get_default_indagis_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its INDAGIS_HOME directory.
    ...
    current INDAGIS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's INDAGIS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$INDAGIS_HOME`` config/sessions/.env, and
    ...
    print(color("│         ⚕ Indagis Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Indagis Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Indagis agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Indagis agent at {hermes_home / 'hermes-agent'}")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Indagis Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their INDAGIS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Indagis data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Indagis code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Indagis PATH entries from shell configs / Windows User PATH")
    print("  • Indagis wrapper scripts and Indagis-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Indagis config/data: {hermes_home}")
        ...
        print(f"  • Keep Indagis config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    wipe ``$INDAGIS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Indagis-owned PATH entries in User environment")

        log_info("Removing INDAGIS_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Indagis-set User env vars to remove")
    ...
    log_info("Removing Indagis-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Indagis-managed node/npm/npx symlinks found")
    ...
    #     but the packaged app + Electron userData live OUTSIDE INDAGIS_HOME and
    ...
    #     under INDAGIS_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile INDAGIS_HOME dirs live under
    ...
    print()
    print("Thank you for using Indagis Agent! ⚕")
    print()
