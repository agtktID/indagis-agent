...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/agtktID/indagis-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for indagis-agent itself
        ...
        # GitHub ZIPs extract to indagis-agent-<branch>/
        ...
        print(
            ...
            "reinstall from https://github.com/agtktID/indagis-agent"
        )
        _m().sys.exit(1)
    finally:
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Indagis behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Indagis couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Indagis couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Indagis behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Indagis couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Indagis couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/agtktID/indagis-agent.git",
    "git@github.com:agtktID/indagis-agent.git",
    "https://github.com/agtktID/indagis-agent",
    "git@github.com:agtktID/indagis-agent",
}

OFFICIAL_REPO_URL = "https://github.com/agtktID/indagis-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Indagis repository.")
        print("  This means you may miss updates from agtktID/indagis-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/agtktID/indagis-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/agtktID/indagis-agent.git' to add later."
            )
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Indagis Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Indagis processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Indagis Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Indagis...")
    try:
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Indagis gateways, Desktop backends, or other "
        ...
        if not import_ok:
            ...
            print("    https://github.com/agtktID/indagis-agent")
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Indagis gateway unit(s) detected:")
