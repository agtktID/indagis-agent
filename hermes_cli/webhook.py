...
def _setup_hint() -> str:
  ...
  Then start the gateway: indagis gateway run
"""
...
def webhook_command(args):
    ...
    if not sub:
        print("Usage: indagis webhook {subscribe|list|remove|test}")
        print("Run 'indagis webhook --help' for details.")
        return
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: indagis webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from indagis webhook test"}'
