...
_REDACTION_BANNER = (
    "[indagis debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    ...
    """
    return get_indagis_home() / "pastes" / "pending.json"
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `indagis debug share` "
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    return (get_indagis_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Indagis Desktop on the machine running the app, not by this "
...
def build_debug_share(
    *,
    ...
    if redact:
        logger.info(
            "indagis debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `indagis debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  indagis debug delete <url>")

    print("\nShare these links with the Indagis team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Indagis version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    if redact:
        logger.info(
            "indagis debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `indagis debug share --local` to print the report instead, "
            "or `indagis debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: indagis debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'indagis debug share'.")
        return
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: indagis debug <command>")
        print()
        print("Commands:")
