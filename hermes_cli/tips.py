...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or indagis -c.",
    ...
    "Ctrl+Z suspends Indagis to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "indagis -c resumes your most recent CLI session. indagis -c \"project name\" resumes by title.",
    "indagis -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "indagis -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "indagis chat -t web,terminal enables only specific toolsets for a focused session.",
    "indagis chat -s github-pr-workflow preloads a skill at launch.",
    "indagis chat -q \"query\" runs a single non-interactive query and exits.",
    "indagis chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "indagis chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "indagis --yolo bypasses all dangerous command approval prompts for the entire session.",
    "indagis chat --source telegram tags the session for filtering in indagis sessions list.",
    "indagis -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "indagis doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "indagis dump outputs a compact setup summary — great for bug reports.",
    "indagis config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "indagis config edit opens config.yaml in your default editor.",
    "indagis config check scans for missing or stale configuration options.",
    "indagis sessions browse opens an interactive session picker with search.",
    "indagis sessions stats shows session counts by platform and database size.",
    "indagis sessions prune --older-than 30 cleans up old sessions.",
    "indagis skills search react --source skills-sh searches the skills.sh public directory.",
    "indagis skills check scans installed hub skills for upstream updates.",
    "indagis skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "indagis skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "indagis mcp add github --command npx adds MCP servers from the command line.",
    "indagis mcp serve runs Indagis itself as an MCP server for other agents.",
    "indagis auth add lets you add multiple API keys for credential pool rotation.",
    "indagis completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "indagis logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "indagis backup creates a zip backup of your entire Indagis home directory.",
    "indagis profile create coder creates an isolated profile that becomes its own command.",
    "indagis profile create work --clone copies your current config and keys to a new profile.",
    "indagis update syncs new bundled skills to ALL profiles automatically.",
    "indagis gateway install sets up Indagis as a system service (systemd/launchd).",
    "indagis memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "indagis webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: indagis tools disables unused tools, indagis skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "indagis models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as indagis -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Indagis tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'indagis profile create coder' creates the 'coder' command.",
    "indagis profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Indagis prints a resume command with session ID and stats.",
    "indagis sessions export backup.jsonl exports all sessions for backup or analysis.",
    "indagis -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "indagis skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: indagis cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Indagis runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "indagis gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Indagis via a local macOS server.",
    ...
    "Indagis loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "indagis mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "indagis mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "indagis mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — indagis tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.indagis/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "indagis chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "indagis plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "indagis acp runs Indagis as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "indagis auth add nous or indagis auth add openai-codex sets up OAuth-based providers.",
    ...
    "indagis status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.indagis/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Indagis your own.",
    ...
    "Before a gateway session reset, Indagis auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.indagis/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Indagis state via INDAGIS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "HERMES_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in INDAGIS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.indagis/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'indagis profile create ls' would be rejected.",
    "indagis profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "indagis claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via indagis tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.indagis/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.indagis/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Indagis auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Indagis config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Indagis is working.',
    ...
    '/reload reloads ~/.indagis/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.indagis/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.indagis/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'indagis curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "indagis curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'indagis curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'indagis auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'indagis dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'indagis dashboard embeds the full Indagis TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.indagis/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.indagis/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.indagis/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.indagis/skills/ so drop-in skills appear without restarting the session.',
    ...
    'indagis -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'indagis chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'indagis chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'indagis chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "indagis chat --source tool tags programmatic chats so they don't clutter indagis sessions list.",
    'indagis dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'indagis sessions rename <ID> "new title" renames any past session; indagis sessions delete <ID> removes one.',
    'indagis import restores a full Indagis backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'indagis fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'indagis pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'indagis setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'indagis status --deep runs the full health sweep across every component; plain indagis status is the quick view.',
    ...
    'INDAGIS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Indagis block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from indagis sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.indagis/dashboard-plugins/.',
]
