...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'indagis doctor'")
        and last.startswith("Run 'indagis setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="indagis", add_help=False)
...
class HermesConsoleEngine:
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Indagis Console does not run shell syntax. Use one supported "
                    "Indagis command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Indagis component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Indagis logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Indagis configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Indagis configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Indagis Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Indagis Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in Indagis Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Indagis Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Indagis Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Indagis Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Indagis Console.",
            ("skills", "publish"): "`skills publish` is not available in Indagis Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Indagis Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Indagis Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Indagis Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Indagis Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Indagis Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Indagis Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Indagis Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Indagis Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Indagis Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Indagis Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Indagis Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Indagis Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Indagis Console.")
...
def run_console_repl(
    *,
    ...
    engine = HermesConsoleEngine()
    if interactive:
        print("Indagis Console. Type `help` for commands, `exit` to quit.", file=stdout)
