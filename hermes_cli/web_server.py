...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same INDAGIS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
app = FastAPI(title="Indagis Agent", version=__version__, lifespan=_lifespan)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Indagis (no terminal prompt), "
...
# INDAGIS_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to INDAGIS_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no INDAGIS_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("INDAGIS_HOME", "").strip()
    if not raw:
        return False
    try:
        from hermes_constants import get_default_indagis_root

        root = get_default_indagis_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_indagis_home() at call time
    ...
    try:
        ...
        # INDAGIS_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_indagis_home())
            )
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_indagis_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
_ACTION_LOG_DIR: Path = get_indagis_home() / "logs"
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Indagis updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        ...
        if force:
            try:
                (get_indagis_home() / ".update_check").unlink()
            except OSError:
                pass
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_indagis_home() / provider.name / "config.json"
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_indagis_home()))
            return
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Indagis from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Indagis to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Indagis from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Indagis to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Indagis in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Indagis through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Indagis via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Indagis through an IMAP/SMTP mailbox.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Indagis to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Indagis inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Indagis to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Indagis through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Indagis to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Indagis to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Indagis.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Indagis from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Indagis over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Indagis through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Indagis over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Indagis to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Indagis as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Indagis via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://agtktid.github.io/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Indagis Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
def _messaging_platform_payload(
    ...
    # INDAGIS_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    ...
    return get_indagis_dir("platforms/whatsapp/session", "whatsapp/session")
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the INDAGIS_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized INDAGIS_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Indagis PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "indagis auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "indagis auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "indagis auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "indagis auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "indagis auth add xai-oauth",
        "docs_url": "https://agtktid.github.io/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "indagis auth add anthropic",
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Indagis — run the disconnect command to remove it."
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"indagis auth add {d.slug}",
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Indagis "
...
def _prune_sessions(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_indagis_home()
...
async def get_logs(
    ...
    log_path = get_indagis_home() / "logs" / log_name
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (INDAGIS_HOME already
    ...
    INDAGIS_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, INDAGIS_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_indagis_home_override(flow.hermes_home)
...
def _dashboard_backup_dir() -> Path:
    return get_indagis_home() / "backups"
...
async def list_checkpoints():
    ...
    cp_dir = get_indagis_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_setup_command(name: str) -> str:
    ...
    return "indagis setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local INDAGIS_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local INDAGIS_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the INDAGIS_HOME override. Returns the
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_indagis_home()`` at call
       time — the context-local override from ``set_indagis_home_override``
    ...
    retargeted to the *current* ``get_indagis_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a INDAGIS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_indagis_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_indagis_home()`` at call time
...
def _resolve_chat_argv(
    ...
    ``INDAGIS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_indagis_home()`` from that env var at its own import, so the child
    ...
    # An explicit profile scope below still overrides INDAGIS_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    if profile_dir is not None:
        env["INDAGIS_HOME"] = str(profile_dir)
    ...
    # inherits the profile INDAGIS_HOME set above.
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Indagis Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Indagis inside WSL2 to use the dashboard's /chat "
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_indagis_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``INDAGIS_HOME``.
    """
    themes_dir = get_process_indagis_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.indagis/plugins/ or the legacy ./.hermes/plugins/ (only if INDAGIS_ENABLE_PROJECT_PLUGINS or the legacy HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # INDAGIS_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_indagis_home() / "plugins", "user"),
    ...
    if env_var_enabled("INDAGIS_ENABLE_PROJECT_PLUGINS") or env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_indagis_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"indagis auth {name}"
                        break
            except Exception:
                pass
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Indagis backend listening on {host}:{actual_port}")
            else:
                print(f"  Indagis Web UI → http://{host}:{actual_port}")
