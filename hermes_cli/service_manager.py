...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's INDAGIS_HOME blindly;
    derive the shared profile root from the current INDAGIS_HOME and map the
    ...
    hermes_home = Path(os.environ.get("INDAGIS_HOME", "/opt/data"))
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources INDAGIS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e INDAGIS_HOME=/data/hermes`` is honored at run
        ...
        root INDAGIS_HOME profile" (the implicit profile that exists at
        the top of $INDAGIS_HOME, not under profiles/). It must be
        ...
        would instead look up ``$INDAGIS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``INDAGIS_HOME``,
        ...
        # profile's INDAGIS_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-INDAGIS_HOME PID lock first;
        ...
        # Each profile is scoped to its own INDAGIS_HOME and s6 guarantees a
        ...
        if profile == "default":
            ...
            gateway_cmd = f"indagis -p {shlex.quote(profile)} gateway run --replace"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${INDAGIS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the INDAGIS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e INDAGIS_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{INDAGIS_HOME:=/opt/data}}"\n'
            f'log_dir="$INDAGIS_HOME/logs/gateways/{prof}"\n'
