...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_indagis_home()}/config.yaml and run `indagis setup` to "
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_indagis_home = _c.get_indagis_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$INDAGIS_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `indagis plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    #      on top of ensure_indagis_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_indagis_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `indagis config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `indagis config set`"
                )
