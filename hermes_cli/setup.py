"""
Interactive setup wizard for Indagis Agent.
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Indagis Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.indagis/.env."
            )
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Indagis runs shell commands and code.")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.indagis/logs/tool_calls.log (gateway only)")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Indagis to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://agtktid.github.io/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://agtktid.github.io/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Indagis from anywhere.")
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Indagis profile and are not uploaded.")
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Indagis to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Indagis at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Indagis at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Indagis at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Indagis at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Indagis equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Indagis config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Indagis.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Indagis's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Indagis can preview what would be imported before making any changes.")
    print()
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Indagis Setup — Nous Portal (one-shot)            │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Indagis Setup — {label:<33s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Indagis Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Indagis Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Indagis configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Indagis?",
            [
