...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`indagis curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `indagis curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`indagis curator status` and run `indagis curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `indagis curator adopt <name>`, "
          "or all with `indagis curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`indagis curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`indagis curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.indagis/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.indagis/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.indagis/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="indagis curator")
    register_cli(parser)
