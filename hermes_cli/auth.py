"""
Multi-provider authentication system for Indagis Agent.
...
SPOTIFY_DOCS_URL = "https://agtktid.github.io/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://agtktid.github.io/docs/guides/oauth-over-ssh"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `indagis model` to re-authenticate."
...
def _auth_file_path() -> Path:
    path = get_indagis_home() / "auth.json"
    # Seat belt: if pytest is running and INDAGIS_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch INDAGIS_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set INDAGIS_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom INDAGIS_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_indagis_root()
    except Exception:
        return None
    profile_home = get_indagis_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.hermes/auth.json`` even when INDAGIS_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_indagis_root()`` for a profile-shaped INDAGIS_HOME can
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'indagis doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'indagis model' for available providers, or run 'indagis doctor' to diagnose config issues."
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: indagis auth add nous",
        provider="nous",
        code=reason,
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `indagis auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `indagis auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `indagis auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `indagis auth spotify` again.",
            provider="spotify",
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Indagis:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Indagis is waiting for the OAuth callback on {redirect_uri}")
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `indagis auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `indagis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `indagis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `indagis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `indagis auth` to re-authenticate."
            )
            relogin_required = True
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `indagis model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `indagis model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `indagis model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `indagis model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `indagis model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `indagis model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
...
# ``get_default_indagis_root()`` returns — ``~/.hermes`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's INDAGIS_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    from hermes_constants import get_default_indagis_root
    return get_default_indagis_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from hermes_constants import get_default_indagis_root
        real_home_shared = (
            get_default_indagis_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No INDAGIS_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "instead.\n"
            "Re-authenticate with: indagis auth add nous"
        )
        relogin = True
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Indagis is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Indagis is not logged into Nous Portal.",
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Indagis' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `indagis doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def login_command(args) -> None:
    ...
    print("Use 'indagis auth' to manage credentials,")
    print("'indagis model' to select a provider, or 'indagis setup' for full setup.")
    raise SystemExit(0)
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Indagis auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Indagis will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Indagis will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(Indagis creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Indagis auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Indagis creates its own local OAuth session)")
    print()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Indagis login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `indagis model` and select "
                "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Indagis login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `indagis model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `indagis model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Indagis will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `indagis model` or configure an API key to use Indagis.")
        else:
