...
def _plugins_dir() -> Path:
    ...
    plugins = get_indagis_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_indagis_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_indagis_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_indagis_home()}/.env later)[/dim]")
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Indagis.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `indagis plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Indagis plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `indagis plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  indagis gateway restart[/dim]")
    console.print()
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`indagis plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] indagis plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] indagis plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] indagis plugins")
    console.print("[dim]Enable/disable:[/dim] indagis plugins enable/disable <name>")
