...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: indagis gateway install", Colors.DIM))
    print(color("                    sudo indagis gateway install --system  # Linux servers", Colors.DIM))
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: indagis gateway restart")
        ...
        print("    indagis gateway install    # Install as a user service")
        print("    sudo indagis gateway install --system  # Linux servers: boot-time system service")
        print("    indagis gateway            # Or run in foreground")
