...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Indagis session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    print()
    print(color(
        "  Install: indagis mcp install <name>    Picker: indagis mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Indagis — run `indagis update` "
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        print(color(
            ...
            "Run `indagis mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
