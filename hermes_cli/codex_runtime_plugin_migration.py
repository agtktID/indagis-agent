...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Indagis config.")
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Indagis key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (INDAGIS_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Indagis)")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a INDAGIS_HOME pointing
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    INDAGIS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # INDAGIS_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_indagis_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever INDAGIS_HOME its launcher sets
    ...
    # — that would override the launcher's INDAGIS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("INDAGIS_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("INDAGIS_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["INDAGIS_HOME"] = hermes_home
    ...
    *,
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Indagis config is not a dict; cannot migrate."
        )
        return report
