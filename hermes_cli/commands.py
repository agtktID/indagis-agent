...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Indagis config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Indagis works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Indagis' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Indagis is working", "Configuration",
    ...
    CommandDef("watch", "Signal Watch — proactive alerts on IOC/target changes", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "create", "show", "pause", "resume", "run", "remove", "status")),
    CommandDef("case", "Case Memory — cross-investigation IOC correlation index", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "ingest", "correlate", "lookup", "investigations", "stats")),
    CommandDef("dossier", "Dossier Builder — Markdown investigation report from an evidence store", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("build",)),
    CommandDef("attribution", "Attribution Confidence Scorer — NATO/Admiralty source-reliability rating", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("score", "matrix")),
    CommandDef("puppet", "Sock Puppet Manager — OSINT persona bookkeeping (footprint, rotation, isolation)", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "create", "show", "add-platform", "use", "burn", "retire")),
    CommandDef("bounty", "Bounty Ledger — payout/ROI tracker for bug bounty submissions", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
    ...
    CommandDef("custody", "Custody Chain — Ed25519 signing and verification for evidence exports", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("keys", "keygen", "sign", "verify", "export")),
    CommandDef("intel", "Threat-intel lookups — AbuseIPDB, GreyNoise, OTX, MalwareBazaar, crt.sh, CISA KEV/EPSS, breach exposure", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("sources", "abuseipdb", "greynoise", "otx", "malwarebazaar", "crtsh", "kev", "breach-email", "breach-domain")),
    CommandDef("surface", "Surface Diff — continuous recon with automatic diffing", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("targets", "snapshot", "diff", "history", "schedule")),
    CommandDef("rules", "Rule Forge — auto-generate Sigma/YARA rules from Case Memory findings", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("forge",)),
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.indagis/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Indagis Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Indagis Agent version", "Info", aliases=("v",),
...
Telegram only displays a small BotCommand menu in practice.  The full Indagis
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "Talk to Indagis or run a subcommand", "[subcommand] [args]"))
    seen.add("hermes")
