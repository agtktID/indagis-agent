...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        # Both product names: "Indagis" is what electron-builder emits now,
        # "Hermes" covers a pre-rebrand build still on disk.
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Indagis.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Indagis.exe from backup.")
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Indagis.exe, close any")
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "indagis dashboard",
            # A dashboard launched by a pre-rebrand install still runs under
            # the `hermes` entry point — keep matching it so status/stop see it.
            "hermes dashboard",
