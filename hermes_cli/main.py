...
Usage:
    ...
    hermes uninstall           Uninstall Indagis Agent
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Indagis?")
        print()
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Indagis only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Indagis URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Indagis will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: indagis slack <subcommand>\n"
            "\n"
            "subcommands:\n"
...
def cmd_backup(args):
    """Back up Indagis home directory to a zip file."""
...
def cmd_import(args):
    """Restore an Indagis backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Indagis Agent (or just the Chat GUI with --gui)."""
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Indagis Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Indagis Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode:
        print("→ Launching Indagis Desktop from source build...")
    ...
    print(f"→ Launching packaged Indagis Desktop: {' '.join(launch_command)}")
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        ...
        if self_locked:
            print(
                "  Indagis is still running from the launcher that needs "
                "replacing. Close other Indagis windows, restart from a "
...
def cmd_update(args):
    """Update Indagis Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Indagis Agent")
        return
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Indagis {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Indagis {mf.hermes_requires}")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    if choice == "2":
        ...
        print(
            ...
            "  Docs: https://agtktid.github.io/docs/"
...
def cmd_logs(args):
    """View and filter Indagis log files."""
...
def cmd_console(args):
    """Open the safe Indagis command console."""
...
def cmd_acp(args):
    """Launch Indagis Agent as an ACP server."""
    try:
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agtktid.github.io/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agtktid.github.io/docs/user-guide/secrets/"
        ),
    )
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agtktid.github.io/docs/user-guide/egress/iron-proxy"
        ),
    )
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Indagis shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Indagis app. `status` reports the driver's grant\n"
