...
def _root() -> Path:
    ...
    # helper's token, including custom INDAGIS_HOME layouts.
    return get_default_indagis_root() / "desktop-ssh"
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isabs(configured_path):
        raise ValueError("Indagis path must be absolute")
    ...
    if not os.path.isfile(python_entry):
        raise ValueError("Indagis Python runtime was not found")
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    if not os.path.isabs(hermes_path) or not os.path.isfile(path):
        raise ValueError("Indagis path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "hermesHome": str(get_default_indagis_root()), "python": sys.executable}
