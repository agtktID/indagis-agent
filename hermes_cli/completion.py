...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Indagis Agent bash completion
# Add to ~/.bashrc:
#   eval "$(indagis completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.indagis/profiles"
...
complete -F _hermes_completion indagis
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef indagis
# Indagis Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.indagis/profiles" ]]; then
        profiles+=($HOME/.indagis/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'indagis command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Indagis Agent fish completion",
        "# Add to your config:",
        "#   indagis completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.indagis/profiles",
        "        for d in $HOME/.indagis/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c indagis -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c indagis -f -s p -l profile"
