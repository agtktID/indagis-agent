...
# Configuration
REPO_URL_SSH="git@github.com:agtktID/indagis-agent.git"
REPO_URL_HTTPS="https://github.com/agtktID/indagis-agent.git"
# Indagis home resolution: P1 INDAGIS_HOME → P2 ~/.indagis → P3 INDAGIS_HOME
...
if [ -n "${INDAGIS_HOME:-}" ]; then
    ...
    : # P2: ~/.indagis already exists — current default, no warning.
...
#   data still at /root/.hermes (INDAGIS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$INDAGIS_HOME/.no-bundled-skills so future"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Indagis.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.indagis/hermes-agent"
            echo "                   default (root, Linux): /usr/local/lib/hermes-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.indagis, or \$INDAGIS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Indagis Agent installs the code under"
            echo "  /usr/local/lib/hermes-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$INDAGIS_HOME"
            echo "  (default /root/.indagis).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$INDAGIS_HOME/hermes-agent are preserved in-place."
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Indagis Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $INDAGIS_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $INDAGIS_HOME/hermes-agent
...
#                              $INDAGIS_HOME/hermes-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in INDAGIS_HOME.
    if is_termux; then
        INSTALL_DIR="$INDAGIS_HOME/hermes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$INDAGIS_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$INDAGIS_HOME/hermes-agent"
        ...
        log_info "  Data:    $INDAGIS_HOME (unchanged)"
...
# so global package binaries land in $INDAGIS_HOME/node/bin — which is NOT on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$INDAGIS_HOME/node/etc/npmrc"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://github.com/agtktID/indagis-agent/raw/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $INDAGIS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$INDAGIS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $INDAGIS_HOME/bin ..."
    mkdir -p "$INDAGIS_HOME/bin"
    ...
    # directly into $INDAGIS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$INDAGIS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$INDAGIS_HOME/node/bin/node" ] && node_satisfies_build "$("$INDAGIS_HOME/node/bin/node" --version)"; then
        export PATH="$INDAGIS_HOME/node/bin:$PATH"
        log_success "Node.js $("$INDAGIS_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    mv "$extracted_dir" "$INDAGIS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$INDAGIS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$INDAGIS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$INDAGIS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Indagis Agent itself does not require or retain root access."
...
copy_config_templates() {
    ...
    # Create ~/.indagis directory structure (config at top level, code in subdir)
    mkdir -p "$INDAGIS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.indagis/.env (top level, easy to find)
    if [ ! -f "$INDAGIS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$INDAGIS_HOME/.env"
            log_success "Created ~/.indagis/.env from template"
        else
            touch "$INDAGIS_HOME/.env"
            log_success "Created ~/.indagis/.env"
        fi
    else
        log_info "~/.indagis/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$INDAGIS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.indagis/config.yaml (top level, easy to find)
    if [ ! -f "$INDAGIS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$INDAGIS_HOME/config.yaml"
            log_success "Created ~/.indagis/config.yaml from template"
        fi
    else
        log_info "~/.indagis/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$INDAGIS_HOME/SOUL.md" ]; then
        cat > "$INDAGIS_HOME/SOUL.md" << 'SOUL_EOF'
You are Indagis Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.indagis/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.indagis/"

    # Seed bundled skills into ~/.indagis/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$INDAGIS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $INDAGIS_HOME/.no-bundled-skills"
        log_info "  Future 'hermes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.indagis/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.indagis/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$INDAGIS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$INDAGIS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.indagis/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$INDAGIS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Indagis Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Indagis Agent browser tools — explicit browser override."
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$INDAGIS_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$INDAGIS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.indagis/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $INDAGIS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $INDAGIS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $INDAGIS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}indagis${NC}             Start chatting"
    echo -e "   ${GREEN}indagis setup${NC}       Configure API keys & settings"
    echo -e "   ${GREEN}indagis config${NC}      View/edit configuration"
    echo -e "   ${GREEN}indagis config edit${NC} Open config in editor"
    echo -e "   ${GREEN}indagis gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}indagis update${NC}      Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'indagis' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'indagis' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use the 'indagis' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$INDAGIS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$INDAGIS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$INDAGIS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$INDAGIS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$INDAGIS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
install_desktop() {
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Indagis" ]; then
            app="$desktop_dir/release/linux-unpacked/Indagis"
        elif [ -x "$desktop_dir/release/linux-unpacked/indagis" ]; then
            app="$desktop_dir/release/linux-unpacked/indagis"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Indagis.app" \
            "$desktop_dir/release/mac/Indagis.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # importing Python cannot reverse it by loading INDAGIS_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if INDAGIS_HOME="$INDAGIS_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $INDAGIS_HOME/node/bin)
        ...
        complete)
            ...
            # $INDAGIS_HOME. $INDAGIS_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $INDAGIS_HOME.
    # $INDAGIS_HOME is a shared data dir (it can be bind-mounted into a Docker
