...
#   data still at /root/.indagis (INDAGIS_HOME).  Matches Claude Code / Codex CLI
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/agtktID/indagis-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_node() {
    ...
    log_info "Extracting to ~/.indagis/node/..."
    ...
    # Place into ~/.indagis/node/ and symlink binaries into the same bin dir
    ...
    log_success "Node.js $installed_ver installed to ~/.indagis/node/"
    HAS_NODE=true
}
