...
1. Spawns a SCRATCH dashboard (``hermes dashboard``) bound to loopback on a
   free port, with an ISOLATED ``INDAGIS_HOME`` (temp dir, minimal seeded state).
...
the scratch INDAGIS_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["INDAGIS_HOME"] = str(self.home)
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch INDAGIS_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch INDAGIS_HOME on exit")
