...
12. **execute_code sandbox has no env vars** — When Indagis runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Indagis `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes")), ".env"))`
