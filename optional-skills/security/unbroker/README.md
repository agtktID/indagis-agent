...
The design is **Indagis-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
indagis skills install official/security/unbroker
```

Then start a new Indagis session and drive it (below). The skill works zero-config; a few optional
...
Drive it from an Indagis session:
...
- **PII handling.** Dossiers live under the Indagis home directory (`0600`, optionally
