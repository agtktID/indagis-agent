...
Usage:
  ...
  <server_name> matches the files in $INDAGIS_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Indagis' own resolver (profile-safe); fall back to env then ~/.hermes.
    try:
        from hermes_constants import get_indagis_home
        return str(get_indagis_home())
    except Exception:
        return os.environ.get("INDAGIS_HOME") or os.path.expanduser("~/.hermes")
