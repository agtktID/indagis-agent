...
The server-building part may be correct while the Indagis config is not. Load the `native-mcp` skill and configure the server in `~/.hermes/config.yaml`, then restart Indagis.
