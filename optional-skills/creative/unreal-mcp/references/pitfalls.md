...
### 1. Start order: editor first, Indagis second

Indagis probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Indagis session.
...
restart the editor, restart the Indagis session.
...
server fails to bind (Output Log) or Indagis' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.indagis/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Indagis

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Indagis' connection
lives in `~/.indagis/config.yaml` via `indagis mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Indagis — just don't
mistake it for the Indagis setup step.
...
explicitly warns against overlapping calls. Indagis executes same-turn tool
...
### 9. Timeouts: Indagis gives up before Unreal does

Indagis' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Indagis runs elsewhere (SSH backend,
