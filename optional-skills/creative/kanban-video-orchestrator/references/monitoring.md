...
indagis kanban watch --tenant <project-slug>

# Snapshot of the board
indagis kanban list --tenant <project-slug>
indagis kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
indagis kanban stats --tenant <project-slug>

# Visual dashboard (browser)
indagis dashboard

# Inspect a specific task (includes comments + events)
indagis kanban show <task-id>

# Follow a single task's event stream
indagis kanban tail <task-id>
```

Verify available subcommands with `indagis kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `indagis kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `indagis kanban block <id>` then `indagis kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `indagis kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `indagis kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `indagis profile list` (profile exists?), `indagis status` (gateway/dispatcher up?), and verify tenant. |
...
indagis kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
indagis kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(indagis kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
indagis kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
indagis kanban block <task-id>
indagis kanban archive <task-id>

# Diagnose what happened
indagis kanban show <task-id>      # task body, comments, recent events
indagis kanban tail <task-id>      # follow the live event stream
indagis kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `indagis kanban create` for the director
...
while true; do
    clear
    indagis kanban list --tenant <slug>
    echo "---"
    indagis kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `indagis kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `indagis profile list` and start
