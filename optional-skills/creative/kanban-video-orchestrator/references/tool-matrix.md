...
Maps each role archetype to the Indagis skills it should `always_load` and the
...
## Indagis skills relevant to video production
...
These are **not** Indagis skills but external CLIs / APIs that profiles invoke.
...
## Built-in Indagis tools for media review

These are native Indagis tools — not invoked via terminal but through their own
...
key is present in `${INDAGIS_HOME:-~/.indagis}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Indagis profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${INDAGIS_HOME:-~/.indagis}/.env` → environment variable.
