...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: indagis config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$INDAGIS_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_indagis_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$INDAGIS_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / INDAGIS_HOME
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `indagis model` to "
                        "select a provider, or run `indagis setup` for first-time "
                        "configuration."
                    )
    ...
    # title write past the first prompt build (e.g. a gateway's pending-title
    # bookkeeping) set this so the gate doesn't depend on write ordering.
    # MUST only ever be derived from an already-DB-persisted title, never
    # from client-supplied input — the message_agent containment gate trusts
    # this value ahead of the DB read (tools/bot_mode_dm.py::_session_title).
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Indagis Agent.  Choose a model with at least "
