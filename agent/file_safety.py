...
def _hermes_home_path() -> Path:
    """Resolve the active INDAGIS_HOME (profile-aware) without circular imports."""
    try:
        from hermes_constants import get_indagis_home  # local import to avoid cycles
        return get_indagis_home()
    except Exception:
        return Path(os.path.expanduser("~/.indagis"))


def _hermes_root_path() -> Path:
    """Resolve the Hermes root dir (always the parent of any profile, never per-profile)."""
    try:
        from hermes_constants import get_default_indagis_root  # local import to avoid cycles
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``INDAGIS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under INDAGIS_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Resolve BOTH the active INDAGIS_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (INDAGIS_HOME points at
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Indagis credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Indagis MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Indagis MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Hermes profiles are separate INDAGIS_HOME dirs under
...
# Profile-scoped directories under INDAGIS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from INDAGIS_HOME.
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"belongs to Indagis profile {info['target_profile']!r}, but the "
...
#
#   <INDAGIS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
#
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    INDAGIS_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Indagis process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real INDAGIS_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host INDAGIS_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Indagis "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real INDAGIS_HOME. Use the "
