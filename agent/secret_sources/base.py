...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per INDAGIS_HOME) by the orchestrator in
...
class SecretSource(ABC):
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved INDAGIS_HOME.
        """
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `indagis secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `indagis secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `indagis secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `indagis secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
