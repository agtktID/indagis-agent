...
Follow the Indagis skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Indagis`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Indagis.
...
4. "## How to Run" — the canonical invocation, framed through Indagis tools.
...
Indagis-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Indagis tools by name in backticks: `terminal`, `read_file`,
