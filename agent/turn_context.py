...
def build_turn_context(
    ...
    *,
    ...
    # section in agent/system_prompt.py). The gate is stable for a session's
    # lifetime, so the tool list is byte-identical every turn: prompt-cache
    # safe. Every other session (CLI, gateway chats, group-room member
    # sessions, cron, subagents) fails the gate and never sees the schema.
    try:
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Indagis preflight compression for codex app-server "
                "(mode=%s); Indagis will not start thread compaction here.",
