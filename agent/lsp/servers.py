...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<INDAGIS_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<INDAGIS_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    inner = (
        ...
        f"-HostName Indagis -HostProfileId hermes -HostVersion 1.0.0 "
