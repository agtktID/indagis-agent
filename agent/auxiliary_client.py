...
from utils import base_url_host_matches, base_url_hostname, env_float, env_with_legacy_alias, model_forces_max_completion_tokens, normalize_proxy_env_vars
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `indagis update` (or "
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/agtktID/indagis-agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = env_with_legacy_alias("INDAGIS_OPENROUTER_CACHE", "HERMES_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = env_with_legacy_alias("INDAGIS_OPENROUTER_CACHE_TTL", "HERMES_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_AUTH_JSON_PATH = get_indagis_home() / "auth.json"
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(env_with_legacy_alias("INDAGIS_NOUS_MIN_KEY_TTL_SECONDS", "HERMES_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `indagis setup` or `indagis model` and enter a valid http(s) base URL."
        ) from exc
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: indagis auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: indagis model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: indagis model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: indagis doctor for "
                "diagnostics)"
            )
            return None, None
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `indagis model`."
                    )
