...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    hermes_home = get_indagis_home().resolve()
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Indagis path and cannot be attached")
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Indagis path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
