...
This document describes Indagis Agent's trust model, names the one
...
[GitHub Security Advisories](https://github.com/agtktID/indagis-agent/security/advisories/new).
Do not open public issues for security vulnerabilities.
**Indagis Agent does not operate a bug bounty program.**

If the issue is in the upstream engine rather than in Indagis-specific
code, it can also be reported to Nous Research via
[their advisories](https://github.com/NousResearch/hermes-agent/security/advisories/new)
or **security@nousresearch.com** — see the credits in the README for
which directories the engine owns.
...
- Environment details (`indagis version`, commit SHA, OS, Python
  version).
...
Indagis Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Indagis Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Indagis Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Indagis Agent's documentation
...
Indagis Agent supports two OS-level isolation postures. They address
...
Indagis Agent supports this in two ways:

- **Indagis Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Indagis Agent's in-process heuristics
...
Indagis Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Indagis Agent
itself. Bugs in Indagis Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Indagis Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Indagis Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Indagis Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Indagis Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Indagis Agent. Bugs in Indagis
...
- Indagis Agent includes supply-chain guards for MCP server
...
- **Channel:** the GHSA thread on this repository (or, for an
  engine-level report escalated upstream, correspondence with
