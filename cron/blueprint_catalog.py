...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + an ``indagis://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``indagis://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"indagis://blueprint/{quote(blueprint.key)}{qs}"
