...
``<INDAGIS_HOME>/state/gateway.lifecycle.json``:
...
def _process_hermes_home() -> Path:
    """INDAGIS_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<INDAGIS_HOME>/state/gateway.lifecycle.json``."""
