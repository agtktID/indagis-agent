...
The PID file lives at ``{INDAGIS_HOME}/gateway.pid``.  INDAGIS_HOME defaults to
``~/.hermes`` but can be overridden via the environment variable.  This means
separate INDAGIS_HOME directories naturally get separate PID files — a property
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_indagis_home() / "gateway-starts.log"
...
def _get_process_hermes_home() -> Path:
    """Return the process-level INDAGIS_HOME, skipping context-local overrides.
    ...
    ``get_indagis_home()`` honors ``_HERMES_HOME_OVERRIDE`` contextvar used for
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute INDAGIS_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare INDAGIS_HOME paths with the host platform's case semantics."""
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting INDAGIS_HOME."""
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    has_gateway_entry = (
        ...
        or any(
            t.rsplit("/", 1)[-1] in ("hermes", "hermes.exe", "indagis", "indagis.exe")
            for t in tokens
        )
    )
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a INDAGIS_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.hermes`` or ``$INDAGIS_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    explicit ``INDAGIS_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a INDAGIS_HOME= value on the argv
    ...
    # a non-matching explicit INDAGIS_HOME= on the argv. INDAGIS_HOME is usually
...
def _build_pid_record() -> dict:
    return {
        ...
        # INDAGIS_HOME-local.  Persist the owning gateway's process home so an
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``INDAGIS_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    INDAGIS_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different INDAGIS_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-INDAGIS_HOME --replace while
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's INDAGIS_HOME rather than the replacer's.  Same-home callers omit
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target INDAGIS_HOME's gateway PID record, and the live
